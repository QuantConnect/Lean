...
namespace QuantConnect.Tests.Engine
{
    ...
    public class DefaultOptionAssignmentModelTests
    {
        ...
        public void SimulatesAssignment(SecurityType securityType)
        {
            ...
            {
                expected.Add(
                    optionDef(def.Right, def.StrikePrice, def.BidPrice, def.AskPrice),
                    def.Exercise
                );
            }
            ...
            // checking results
            foreach (
                var option in algorithm
                    .Securities.Values.Where(security => security.Symbol.SecurityType.IsOption())
            ...
            )
            {
                ...
                Assert.AreEqual(
                    ...
                    result.Quantity > 0,
                    $"Failed on strike: {option.Symbol.ID.StrikePrice}"
                );
            }
        }

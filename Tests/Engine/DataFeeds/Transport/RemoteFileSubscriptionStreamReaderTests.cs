...
namespace QuantConnect.Tests.Engine.DataFeeds.Transport
{
    ...
    public class RemoteFileSubscriptionStreamReaderTests
    {
        ...
        {
            using var cacheProvider = new SingleEntryDataCacheProvider(
            ...
            using var cacheProvider2 = new SingleEntryDataCacheProvider(
                TestGlobals.DataProvider,
                isDataEphemeral: isDataEphemeral
            );
        ...
        {
            using var cacheProvider = new ZipDataCacheProvider(
                TestGlobals.DataProvider,
                isDataEphemeral: isDataEphemeral
            );
            using var remoteReader = new RemoteFileSubscriptionStreamReader(
            ...
            using var cacheProvider2 = new ZipDataCacheProvider(
                TestGlobals.DataProvider,
                isDataEphemeral: isDataEphemeral
            );
            using var remoteReader2 = new RemoteFileSubscriptionStreamReader(
        ...
        public void GetsZippedDataForUrlNotEndingWithZipExtension(
            bool withQuery,
            string entryName,
            int expectedLines
        )
        {
            ...
            while (!remoteReader.EndOfStream)
            {
                ...
                Assert.IsTrue(
                    int.TryParse(csv[0], NumberStyles.Number, CultureInfo.InvariantCulture, out _)
                );
                Assert.IsTrue(
                    decimal.TryParse(
                        csv[1],
        ...
        public void InvalidDataSource()
        {
            ...
            Assert.Throws<WebException>(
                () =>
                    new RemoteFileSubscriptionStreamReader(
        ...
        private class TestDownloadProvider : QuantConnect.Api.Api
        {
            ...
            public override byte[] DownloadBytes(
                string address,

...
#pragma warning restore CS0162
        ...
        public void FillForwardProducesCorrectResults(
            string scenarioName,
            BaseData[] inputData,
            ...
            Symbol symbol)
        {
            ...
            // Use always-open hours for OpenInterest, regular market hours for other types
            ...
            using var ffEnumerator = new FillForwardEnumerator(
                new List<BaseData>(inputData).GetEnumerator(),
                exchange,
            ...
            int count = 0, ffCount = 0;
            BaseData previousValue = null;
            while (ffEnumerator.MoveNext())
            {
                count++;
                var current = ffEnumerator.Current;
                if (current.IsFillForward)
                {
                    Assert.AreEqual(previousValue.Value, current.Value);
                    if (current is CustomData customData)
                    {
                        Assert.AreEqual(123, customData.CustomValue);
                    }
                    ffCount++;
                }

                // Ensure data points fall within regular trading days (Monday-Friday)
                Assert.AreNotEqual(DayOfWeek.Saturday, current.EndTime.DayOfWeek);
                Assert.AreNotEqual(DayOfWeek.Sunday, current.EndTime.DayOfWeek);

                // Type-specific validations
                switch (current)
                {
                    case TradeBar trade:
                        Assert.IsTrue(trade.Period > TimeSpan.Zero, "Invalid period in TradeBar");
                        break;

                    case QuoteBar quote:
                        Assert.IsNotNull(quote.Bid, "Bid should not be null");
                        Assert.IsNotNull(quote.Ask, "Ask should not be null");
                        break;

                    case OpenInterest openInterest:
                        Assert.GreaterOrEqual(openInterest.Value, 0, "OpenInterest cannot be negative");
                        break;
                }
            ...
            // Final assertions
            Assert.AreEqual(expectedCount, count, $"Total count mismatch in {scenarioName}");
            Assert.AreEqual(expectedFF, ffCount, $"FF count mismatch in {scenarioName}");
        }
        ...
        private static object[] TestCases =
        {
            ...
            new object[]
            {
                ...
                new BaseData[]
                {
                    new TradeBar { Time = new DateTime(2023, 1, 1), Value = 1, Period = Time.OneDay },
                    new TradeBar { Time = new DateTime(2023, 1, 5), Value = 2, Period = Time.OneDay }
                },
                Time.OneDay,
            ...
            new object[]
            {
                ...
                new BaseData[]
                {
                    ...
                    new QuoteBar
                    {
                        ...
                        Bid = new Bar(1.1m, 1.1m, 1.1m, 1.1m),
                        Ask = new Bar(2.1m, 2.1m, 2.1m, 2.1m),
                        Period = Time.OneDay
                    }
                },
                Time.OneDay,
            ...
            new object[]
            {
                ...
                new BaseData[]
                {
                    new OpenInterest { Time = new DateTime(2023, 1, 1), EndTime = new DateTime(2023, 1, 2), Value = 1000 },
                    new OpenInterest { Time = new DateTime(2023, 1, 3), EndTime = new DateTime(2023, 1, 4), Value = 1500 }
                },
                Time.OneDay,
                new DateTime(2023, 1, 10),
                7,
            ...
            new object[]
            {
                ...
                new BaseData[]
                {
                    new CustomData { Time = new DateTime(2023, 1, 1), EndTime = new DateTime(2023, 1, 2), Value = 10, CustomValue = 123},
                },
                Time.OneDay,
                new DateTime(2023, 1, 10),
                6,

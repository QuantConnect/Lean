...
namespace QuantConnect.Tests.Common.Parameters
{
    ...
    public class ParameterAttributeTests
    {
        ...
        public void FindsParameters()
        {
            ...
            foreach (
                var field in typeof(Instance<int>)
                    .GetFields(ParameterAttribute.BindingFlags)
                    .Where(x => !x.Name.Contains(">k__"))
            )
            {
                Assert.IsTrue(parameters.ContainsKey(field.Name), "Failed on Field: " + field.Name);
            }
            foreach (
                var property in typeof(Instance<int>).GetProperties(ParameterAttribute.BindingFlags)
            )
            {
                Assert.IsTrue(
                    parameters.ContainsKey(property.Name),

...
namespace QuantConnect.Tests.Common.Securities
{
    ...
    public class SecurityCacheTests
    {
        ...
        public void AlwaysReturnTheLastData(
        ...
        public void UseLatestTradebarIfThereIsntAvailableQuotebar()
        {
            ...
            var laterTrade = GenerateData(
                    MarketDataType.TradeBar,
                    1,
                    true,
                    quotes.Last().Time.AddSeconds(1)
                )
                .First();
            // Act
        ...
        public void AddData_SecurityCacheHasTradeAndQuoteTick()
        {
            ...
            Assert.True(
                securityCache
                    .GetAll<Tick>()
                    .LastOrDefault(x => x.TickType == TickType.Quote)
                    .Equals(quote)
            );
            Assert.True(
                securityCache
                    .GetAll<Tick>()
                    .LastOrDefault(x => x.TickType == TickType.Trade)
                    .Equals(trade)
            );
        }
        ...
        public void ResetTests(SecurityCache cache, SecuritySeedData seedType)
        {
            switch (seedType)
            {
                ...
                case SecuritySeedData.QuoteTick:
                    ...
                    Assert.Greater(
                        cache.GetAll<Tick>().Count(x => x.TickType == TickType.Quote),
                        0
                    );
                    cache.Reset();
                ...
                case SecuritySeedData.TradeTick:
                    Assert.IsNotNull(cache.GetData());
                    Assert.Greater(
                        cache.GetAll<Tick>().Count(x => x.TickType == TickType.Trade),
                        0
                    );
                    cache.Reset();
                    Assert.IsFalse(cache.HasData(typeof(Tick)));
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsOpenInterestValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
            var map = new Dictionary<string, string> { { "OpenInterest", "Value" } };
            AddDataAndAssertChanges(
                cache,
                seedType,
                SecuritySeedData.OpenInterest,
                new OpenInterest { Value = 101, EndTime = ReferenceTime },
                map
            );
        }
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsOpenInterestTickValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
            ...
            AddDataAndAssertChanges(
                cache,
                seedType,
                SecuritySeedData.OpenInterestTick,
                new Tick
                {
                    Value = 101,
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsQuoteTickValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsTradeTickValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsTradeBarValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataWithSameEndTime_SetsQuoteBarValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
        ...
        [Test]
        [TestCaseSource(nameof(GetSecurityCacheInitialStates))]
        public void AddDataFundamentals_DoesNotChangeCacheValues(
            SecurityCache cache,
            SecuritySeedData seedType
        )
        {
            ...
            AddDataAndAssertChanges(
                cache,
                seedType,
                SecuritySeedData.Fundamentals,
                new Fundamental(ReferenceTime, Symbols.AAPL)
                {
                    Value = 111,
                    EndTime = ReferenceTime
                },
        ...
        private void AddDataAndAssertChanges(
            SecurityCache cache,
            SecuritySeedData seedType,
            SecuritySeedData dataType,
            BaseData data,
            Dictionary<string, string> cacheToBaseDataPropertyMap = null
        )
        {
            ...
            if (
                seedType == SecuritySeedData.QuoteBar
                && (dataType == SecuritySeedData.QuoteBar || dataType == SecuritySeedData.TradeBar)
            )
            {
                // these properties aren't updated when previous data is quote bar at same time as a new IBar
                updatedCacheProperties = updatedCacheProperties
                    .Where(p =>
                        p != "Open" && p != "High" && p != "Low" && p != "Close" && p != "Price"
                    )
                    .ToArray();
            }
            ...
            foreach (var property in before.Properties())
            {
                ...
                {
                    if (
                        cacheToBaseDataPropertyMap?.TryGetValue(property.Name, out dataPropertyName)
                        == true
                    )
                    {
        ...
        private static TestCaseData[] GetSecurityCacheInitialStates()
        {
            ...
            openInterest.AddData(new OpenInterest { Value = 23, EndTime = ReferenceTime, });
            ...
            fundamentals.AddData(new Fundamental(ReferenceTime, Symbols.AAPL) { Value = 23 });

            return new[]
            {
                new TestCaseData(defaultInstance, SecuritySeedData.None).SetName(
                    "Default Instance"
                ),
                new TestCaseData(tradeTick, SecuritySeedData.TradeTick).SetName(
                    "Seeded w/ Trade Tick"
                ),
                new TestCaseData(quoteTick, SecuritySeedData.QuoteTick).SetName(
                    "Seeded w/ Quote Tick"
                ),
                new TestCaseData(openInterestTick, SecuritySeedData.OpenInterestTick).SetName(
                    "Seeded w/ OpenInterest Tick"
                ),
                new TestCaseData(openInterest, SecuritySeedData.OpenInterest).SetName(
                    "Seeded w/ OpenInterest"
                ),
                ...
                new TestCaseData(fundamentals, SecuritySeedData.Fundamentals).SetName(
                    "Seeded w/ Fundamentals"
                )
            };
        }

        private IReadOnlyCollection<BaseData> GenerateData(
            MarketDataType type,
            int quantity,
            bool sameTime,

...
namespace QuantConnect.Tests.Common.Securities
{
    public class SecurityServiceTests : ISecurityInitializerProvider
    {
        ...
        public ISecurityInitializer SecurityInitializer =>
            QuantConnect.Securities.SecurityInitializer.Null;
        ...
        public void CanCreate_ForexOrCrypto_WithCorrectSubscriptions(
            string ticker,
            SecurityType type,
            string market
        )
        {
            ...
            var configs = _subscriptionManager.SubscriptionDataConfigService.Add(
                typeof(QuoteBar),
                symbol,
                Resolution.Second,
                false,
        ...
        public void CanCreate_Cfd_WithCorrectSubscriptions()
        {
            ...
            _marketHoursDatabase.SetEntryAlwaysOpen(
                Market.USA,
                "abc",
                SecurityType.Cfd,
                TimeZones.NewYork
        ...
        public void CanCreate_CustomSecurities_WithCorrectSubscriptions()
        {
            var symbol = new Symbol(
                SecurityIdentifier.GenerateBase(null, "BTC", Market.USA),
                "BTC"
            );
            _marketHoursDatabase.SetEntryAlwaysOpen(
                Market.USA,
            ...
            var configs = _subscriptionManager.SubscriptionDataConfigService.Add(
                typeof(LiveTradingFeaturesAlgorithm.Bitcoin),
                symbol,
                Resolution.Second,
                false,
                false,
            ...
            Assert.AreEqual(
                security.Subscriptions.First().Type,
                typeof(LiveTradingFeaturesAlgorithm.Bitcoin)
            );
        ...
        public void CanCreate_ConcreteOptions_WithCorrectSubscriptions()
        {
            ...
            Assert.IsTrue(
                security.Subscriptions.Any(x =>
                    x.TickType == TickType.OpenInterest && x.Type == typeof(OpenInterest)
                )
            );
            Assert.IsTrue(
                security.Subscriptions.Any(x =>
                    x.TickType == TickType.Quote && x.Type == typeof(QuoteBar)
                )
            );
            Assert.IsTrue(
                security.Subscriptions.Any(x =>
                    x.TickType == TickType.Trade && x.Type == typeof(TradeBar)
                )
            );
        }
        ...
        {
            var identifier = SecurityIdentifier.GenerateFuture(
        ...
        public void CreatesEquityOptionWithContractMultiplierEqualsToContractUnitOfTrade()
        {
            ...
            var equityOptionSecurity = (QuantConnect.Securities.Option.Option)
                _securityService.CreateSecurity(equityOption, configs, 1.0m);
        ...
        public void CreatesFutureOptionWithContractMultiplierEqualsToFutureContractMultiplier()
        {
            ...
            var futureOptionSecurity = (QuantConnect.Securities.Option.Option)
                _securityService.CreateSecurity(futureOption, configs, 1.0m);
        ...
        public void AddPrimaryExchangeToSecurityObject()
        {
            ...
            mockedPrimaryExchangeProvider
                .Setup(pep => pep.GetPrimaryExchange(equitySymbol.ID))
                .Returns(Exchange.NASDAQ);
            ...
            Assert.AreEqual(equity.Subscriptions.First().TickType, TickType.Trade);
            Assert.AreEqual(
                ((QuantConnect.Securities.Equity.Equity)equity).PrimaryExchange,
                Exchange.NASDAQ
            );
        }
    }

...
namespace QuantConnect.Tests.Common.Securities
{
    ...
    public class SecurityTransactionModelTests
    {
        ...
        public void PerformsMarketOnCloseUsingClosingPrice()
        {
            ...
            var model = new SecurityTransactionModel();
            var order = new MarketOnCloseOrder(Symbols.SPY, SecurityType.Equity, 100, reference);
            var config = CreateTradeBarConfig(Symbols.SPY);
            var security = new Security(SecurityExchangeHoursTests.CreateUsEquitySecurityExchangeHours(), config, 1);
            security.SetLocalTimeKeeper(TimeKeeper.GetLocalTimeKeeper(TimeZones.NewYork));
            var time = reference;
        ...
        {
            return new SubscriptionDataConfig(typeof(TradeBar), SecurityType.Equity, symbol, Resolution.Minute, Market.USA, TimeZones.NewYork, true, true, false);
        }
    }
}

    ...
    public class OptionSecurityTests
    {
        ...
        public void EquityOptionSecurityUsesOptionMarginModel()
        {
            ...
            var equityOptionSecurity = new Option(
                ...
                MarketHoursDatabase
                    .FromDataFolder()
                    .GetExchangeHours(Market.USA, equityOption, equityOption.SecurityType),
                new Cash("USD", 100000m, 1m),
        ...
        public void AlgorithmSendsOneTimeWarningAboutOptionModelsConsistency(
            [Values(
                nameof(OptionModelsConsistencyRegressionAlgorithm),
                nameof(IndexOptionModelsConsistencyRegressionAlgorithm)
            )]
        ...
        )
        {
            ...
            Assert.IsTrue(
                result.Logs.Any(message =>
                    message.Contains("Debug: Warning: Security ")
                    && message.EndsWith(
                        "To avoid this, consider using a security initializer to set the right models to each security type according to your algorithm's requirements."
                    )
                )
            );
        }

...
namespace QuantConnect.Tests.Common.Securities.Futures
{
    ...
    public class FutureSettlementModelTests
    {
        ...
        private static readonly TimeKeeper TimeKeeper =
            new(Noon.ConvertToUtc(TimeZones.NewYork), new[] { TimeZones.NewYork });
        ...
        public void Setup()
        {
            ...
            _portfolio = new SecurityPortfolioManager(
                securities,
                transactions,
            ...
            var entry = MarketHoursDatabase
                .FromDataFolder()
                .GetEntry(
                    Symbols.Fut_SPY_Feb19_2016.ID.Market,
        ...
        public void DailySettlement(
            ...
            decimal quantity,
            decimal futurePriceStep1,
            decimal futurePriceStep2
        )
        {
            ...
            _model.Scan(
                new ScanSettlementModelParameters(_portfolio, _future, _timeKeeper.LocalTime)
            );
            _portfolio.InvalidateTotalPortfolioValue();
            ...
            Assert.AreEqual(expectedTpv, _portfolio.TotalPortfolioValue);
            Assert.AreEqual(
                startCash + _future.Holdings.UnrealizedProfit,
        ...
        [TestCase(1400, -10, 1300, 20)]
        [TestCase(1300, -10, 1400, 20)]
        public void HoldingsQuantityChange(
            decimal averagePrice,
            decimal quantity,
            decimal futurePrice,
            decimal newQuantity
        )
        {
            ...
            var absoluteQuantityClosed = Math.Min(
                Math.Abs(fillQuantity),
                _future.Holdings.AbsoluteQuantity
            );
            ...
            var funds = new CashAmount(
                _future.Holdings.TotalCloseProfit(
                    includeFees: false,
                    exitPrice: fillPrice,
                    _future.Holdings.AveragePrice,
                    closedQuantity
                ),
                Currencies.USD
            );
            var fill = new OrderEvent(
                1,
                _future.Symbol,
                _timeKeeper.LocalTime,
                OrderStatus.Filled,
                Extensions.GetOrderDirection(fillQuantity),
                fillPrice,
                fillQuantity,
                OrderFee.Zero
            );
            _future.SettlementModel.ApplyFunds(
                new ApplyFundsSettlementModelParameters(
                    _portfolio,
                    _future,
                    _timeKeeper.LocalTime.ConvertToUtc(_timeKeeper.TimeZone),
                    funds,
                    fill
                )
            );
        ...
        [TestCase(10, 10, -5, -15)]
        [TestCase(5, 15, -5, -15)]
        public void DifferentAveragePrice(
            decimal fillQuantityA,
            decimal fillQuantityB,
            decimal fillQuantityC,
            decimal fillQuantityD
        )
        {
            ...
            _future.PortfolioModel.ProcessFill(
                _portfolio,
                _future,
                new OrderEvent(
                    1,
                    ...
                    _timeKeeper.LocalTime,
                    OrderStatus.Filled,
                    Extensions.GetOrderDirection(fillQuantityA),
                    initialAveragePrice,
                    fillQuantityA,
                    OrderFee.Zero
                )
            ...
            var profit = _future.Holdings.TotalCloseProfit(
                includeFees: false,
                exitPrice: _future.Price,
                _future.Holdings.AveragePrice,
                _future.Holdings.Quantity
            );
            Assert.AreEqual(0, _futureHoldings.SettledProfit);
            ...
            _future.PortfolioModel.ProcessFill(
                ...
                new OrderEvent(
                    ...
                    _timeKeeper.LocalTime,
                    OrderStatus.Filled,
                    Extensions.GetOrderDirection(fillQuantityB),
                    secondAveragePrice,
                    fillQuantityB,
                    OrderFee.Zero
                )
            ...
            _future.PortfolioModel.ProcessFill(
                ...
                new OrderEvent(
                    ...
                    _timeKeeper.LocalTime,
                    OrderStatus.Filled,
                    Extensions.GetOrderDirection(fillQuantityC),
                    exitFillPrice,
                    fillQuantityC,
                    OrderFee.Zero
                )
            ...
            var currentlyStettledProfit =
                profit * ((fillQuantityA + fillQuantityC) / fillQuantityA);
            Assert.AreEqual(currentlyStettledProfit, _futureHoldings.SettledProfit);
            var closeProfit = new CashAmount(
                _future.Holdings.TotalCloseProfit(
                    includeFees: false,
            ...
            Assert.AreEqual(
                startCash + closeProfit.Amount + currentlyStettledProfit,
                _portfolio.CashBook[Currencies.USD].Amount
            );
            Assert.AreEqual(
                startTpv
                    + closeProfit.Amount
                    + _futureHoldings.UnsettledProfit
                    + currentlyStettledProfit,
                _portfolio.TotalPortfolioValue
            );
            Assert.AreEqual(
                _futureHoldings.UnsettledProfit + currentlyStettledProfit,
                _future.Holdings.TotalCloseProfit(
                    includeFees: false,
                    exitPrice: _future.Price,
                    averagePrice,
            ...
            _future.PortfolioModel.ProcessFill(
                ...
                new OrderEvent(
                    ...
                    _timeKeeper.LocalTime,
                    OrderStatus.Filled,
                    Extensions.GetOrderDirection(fillQuantityD),
                    exitFillPrice,
                    fillQuantityD,
                    OrderFee.Zero
                )
            ...
            Assert.AreEqual(0, _futureHoldings.SettledProfit);
            Assert.AreEqual(0, _futureHoldings.UnsettledProfit);
            var closeProfit2 = new CashAmount(
                _future.Holdings.TotalCloseProfit(
                    includeFees: false,
                    exitPrice: exitFillPrice,
                    averagePrice,
            ...
            Assert.AreEqual(
                startCash + closeProfit.Amount + closeProfit2.Amount,
                _portfolio.CashBook[Currencies.USD].Amount
            );
            Assert.AreEqual(
                startTpv + closeProfit.Amount + closeProfit2.Amount,
                _portfolio.TotalPortfolioValue
            );
        }
        ...
        private static void SetPrice(Security security, decimal price)
        {
            security.SetMarketPrice(
                new Tick(Noon, security.Symbol, string.Empty, Exchange.UNKNOWN, quantity: 1, price)
            );
        }
    }
}

...
namespace QuantConnect.Tests.Common.Orders.Fills
{
    ...
    public partial class EquityFillModelTests
    {
        ...
        )
        {
            ...
            var orderTime = new DateTime(2014, 6, 24, orderHour, 0, 0).ConvertToUtc(
                equity.Exchange.TimeZone
            );
            ...
            Assert.IsTrue(
                fill.Message.StartsWith(message, StringComparison.InvariantCultureIgnoreCase)
            );
        }
        ...
        public void PerformsMarketOnOpenUsingOpenPriceWithMinuteSubscription(int quantity)
        {
            ...
            var fill = model
                .Fill(new FillModelParameters(equity, order, configProvider, Time.OneHour, null))
                .Single();
            ...
            equity.SetMarketPrice(
                new QuoteBar(
                    time,
                    Symbols.SPY,
                    new Bar(1.45m, 1.99m, 1.09m, 1.39m),
                    100,
                    new Bar(1.46m, 2.01m, 1.11m, 1.41m),
                    100
                )
            );
            ...
            // Fill with trade bar
            equity.SetMarketPrice(
                new TradeBar(time, Symbols.SPY, expected, 2.0m, 1.1m, 1.40m, 100)
            );
            fill = model.MarketOnOpenFill(equity, order);
            Assert.AreEqual(order.Quantity, fill.FillQuantity);
        ...
        {
            Func<DateTime, decimal, TradeBar> getTradeBar = (t, o) =>
                new TradeBar(
                    t.RoundDown(Time.OneDay),
                    Symbols.SPY,
                    o,
                    2m,
                    0.5m,
        ...
        [TestCase(
            -100,
            TradeConditionFlags.OfficialOpen
                | TradeConditionFlags.Regular
                | TradeConditionFlags.Cash
                | TradeConditionFlags.Cross
        ...
        [TestCase(
            -100,
            TradeConditionFlags.OpeningPrints
                | TradeConditionFlags.Regular
                | TradeConditionFlags.Cash
                | TradeConditionFlags.Cross
                | TradeConditionFlags.DerivativelyPriced
        ...
        [TestCase(-100, 0.9)]
        [TestCase(100, 1.1)]
        public void PerformsMarketOnOpenUsingOpenPriceWithTickSubscriptionButNoSalesCondition(
            int quantity,
            decimal expected
        )
        {
            ...
            equity.Cache.Reset();
            equity.Update(
                new List<Tick> { new Tick(time, Symbols.SPY, 1m, 0.9m, 1.1m), },
                typeof(Tick)
            );
            ...
            Assert.AreEqual(expected, fill.FillPrice);
            Assert.IsTrue(
                fill.Message.Contains(
                    "Fill with last Quote data.",
        ...
        public void PerformsMarketOnOpenUsingOpenPriceWithDifferentOrderSubmissionDateTime(
        ...
        )
        {
            ...
            var orderTime = new DateTime(2015, 6, day, hour, minute, 0).ConvertToUtc(
                TimeZones.NewYork
            );
            ...
            TimeKeeper.SetUtcDateTime(time.ConvertToUtc(TimeZones.NewYork));
            equity.SetMarketPrice(
                new TradeBar(time, Symbols.SPY, 1m, 2m, 0.5m, 1.33m, 100, period)
            );
        ...
        {
            Func<DateTime, decimal, TradeBar> getTradeBar = (t, c) =>
                new TradeBar(
                    t.RoundDown(Time.OneDay * 2),
                    Symbols.SPY,
                    1.33m,
                    2m,
                    0.5m,
        ...
        [TestCase(-100)]
        [TestCase(100)]
        public void PerformsMarketOnCloseUsingClosingPriceWithMinuteTradeBarSubscription(
            int quantity
        )
        {
            var reference = new DateTime(2015, 06, 05, 15, 0, 0); // before market close
        ...
        [TestCase(-100)]
        [TestCase(100)]
        public void PerformsMarketOnCloseUsingClosingPriceWithMinuteQuoteBarSubscription(
            int quantity
        )
        {
            var reference = new DateTime(2015, 06, 05, 15, 0, 0); // before market close
            ...
            equity.SetMarketPrice(
                new QuoteBar(
                    time,
                    Symbols.SPY,
                    new Bar(1.45m, 1.99m, 1.09m, expected),
                    100,
                    new Bar(1.46m, 2.01m, 1.11m, expected),
                    100,
                    config.Increment
                )
            );
            ...
            Assert.AreEqual(expected, fill.FillPrice);
            Assert.IsTrue(
                fill.Message.StartsWith(
                    "Warning: No trade information available",
                    StringComparison.InvariantCulture
                )
            );
        }
        ...
        [TestCase(
            -100,
            true,
            TradeConditionFlags.OfficialClose
                | TradeConditionFlags.Regular
                | TradeConditionFlags.Cash
                | TradeConditionFlags.Cross
                | TradeConditionFlags.DerivativelyPriced
        ...
        [TestCase(
            -100,
            true,
            TradeConditionFlags.ClosingPrints
                | TradeConditionFlags.Regular
                | TradeConditionFlags.Cash
                | TradeConditionFlags.Cross
                | TradeConditionFlags.DerivativelyPriced
        ...
        public void PerformsMarketOnCloseUsingClosingPriceWithTickSubscription(
            int quantity,
            bool extendedHours,
            long numericSaleCondition
        )
        {
            ...
            if (!extendedHours)
            {
                ...
                Assert.AreEqual(expected, fill.FillPrice);
                Assert.AreEqual(
                    "No trade with the OfficialClose or ClosingPrints flag for data that does not include extended market hours. Fill with last Trade data.",
                    fill.Message
                );
                return;
            }
        ...
        [TestCase(-100)]
        [TestCase(100)]
        public void PerformsMarketOnCloseUsingClosingPriceWithTickSubscriptionButNoSalesCondition(
            int quantity
        )
        {
            var reference = new DateTime(2015, 06, 05, 15, 0, 0); // before market close
        ...
        private SubscriptionDataConfig CreateQuoteBarConfig(
            Symbol symbol,
        ...
        private FillModelParameters GetFillModelParameters(Order order)
        {
            ...
            return new FillModelParameters(security, order, configProvider, Time.OneHour, null);
        }

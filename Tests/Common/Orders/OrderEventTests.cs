    ...
    public class OrderEventTests
    {
        ...
        public void JsonIgnores()
        {
            ...
            json = JsonConvert.SerializeObject(
                new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero, "This is a message")
                {
                    LimitPrice = 1,
                    StopPrice = 2,
                    IsInTheMoney = true
        ...
        public void RoundTripSerialization([Values] bool isInTheMoney)
        {
            ...
            Assert.IsFalse(
                serializeObject.Contains(Currencies.NullCurrency, StringComparison.InvariantCulture)
            );
            Assert.IsFalse(
                serializeObject.Contains("order-fee-amount", StringComparison.InvariantCulture)
            );
            Assert.IsFalse(
                serializeObject.Contains("order-fee-currency", StringComparison.InvariantCulture)
            );
            Assert.AreEqual(
                isInTheMoney,
                serializeObject.Contains("isInTheMoney", StringComparison.InvariantCulture)
            );

            var deserializeObject = JsonConvert.DeserializeObject<OrderEvent>(
                serializeObject,
                converter
            );
            ...
            Assert.AreEqual(
                ...
                deserializeObject.AbsoluteFillQuantity
            );
            ...
            Assert.AreEqual(
                ...
                deserializeObject.OrderFee.Value.Amount
            );
            Assert.AreEqual(
                orderEvent.OrderFee.Value.Currency,
                deserializeObject.OrderFee.Value.Currency
            );
        ...
        public void NonNullOrderFee()
        {
            ...
            var orderEvent = new OrderEvent(
                order,
                DateTime.UtcNow,
                new OrderFee(new CashAmount(88, Currencies.USD))
            );
            ...
            Assert.IsTrue(
                serializeObject.Contains("orderFeeAmount", StringComparison.InvariantCulture)
            );
            Assert.IsTrue(
                serializeObject.Contains("orderFeeCurrency", StringComparison.InvariantCulture)
            );
        ...
        {
            var serializeObject =
                "{\"id\":\"id-0-0\",\"algorithm-id\":\"id\",\"order-id\":0,\"order-event-id\":0,\"symbol\":\"BTCUSD 2XR\",\"time\":1711565119.684036,\"status\":\"none\","

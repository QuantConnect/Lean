...
{
    [TestFixture]
    public class StopLimitOrderTests
    {
        [Test]
        public void GetValuesReturnsQuantityTimesPrice()
        {
            ...
            var order = new StopLimitOrder(Symbols.SPY, 100, price, price, time);
        ...
        public void GetValueUsesBetterPrice()
        {
            ...
            var time = new DateTime(2016, 2, 4, 16, 0, 0).ConvertToUtc(TimeZones.NewYork);
            var order = new StopLimitOrder(Symbols.SPY, 100, price - 2 * delta, price - delta, time);
            var value = order.GetValue(price);
            ...
            order.StopPrice = price + 2 * delta;//unused, the limit price should be used in this calc
            value = order.GetValue(price);
            Assert.AreEqual((price + delta) * order.Quantity, value);
        }
    }

...
namespace QuantConnect.Tests.Common.Data.Market
{
    ...
    public class OptionChainSelectionTests
    {
        ...
        public void PickIsASynonymOfSelect()
        {
            ...
            Assert.AreEqual(chain.Select(right: OptionRight.Call, moneyness: 0.05m).Symbol, chain.Pick(right: OptionRight.Call, moneyness: 0.05m).Symbol);
        ...
        [TestCase(8, 110)]
        [TestCase(-4, 100)]
        public void SelectByStrikeDistance(double strikeFromAtm, double expectedStrike)
        {
            var chain = CreateDefaultChain();
            var contract = chain.SelectByStrikeDistance((decimal)strikeFromAtm, OptionRight.Put, targetDte: 8);
        ...
        public void SelectByDeltaIsSignInsensitive(double targetDelta)
        {
            ...
            var contract = chain.SelectByDelta((decimal)targetDelta, OptionRight.Put, targetDte: 8);
        ...
        public void SelectByDeltaIgnoresContractsWithoutGreeks()
        {
            ...
            var contract = chain.SelectByDelta(0.05m, OptionRight.Call);
            Assert.AreEqual(100m, contract.Strike);
            ...
            Assert.IsNull(noGreeks.SelectByDelta(0.05m, OptionRight.Call));
        }
        ...
        [Test]
        public void SelectByStrikeDistanceAndDeltaAreNullSafe()
        {
            var chain = CreateDefaultChain();
            Assert.IsNull(chain.SelectByStrikeDistance(-5m, minDte: 40, maxDte: 60));
            Assert.IsNull(chain.SelectByDelta(0.3m, minDte: 40, maxDte: 60));
            Assert.IsNull(CreateEmptyChain().SelectByStrikeDistance(-5m));
            Assert.IsNull(CreateEmptyChain().SelectByDelta(0.3m));

            // delta needs no underlying price, a strike distance does
            var noUnderlying = CreateChain(new[] { (Expiry1, 100m, OptionRight.Call, 0.5m) }, underlyingPrice: null);
            Assert.IsNull(noUnderlying.SelectByStrikeDistance(-5m, OptionRight.Call));
            Assert.AreEqual(100m, noUnderlying.SelectByDelta(0.5m, OptionRight.Call).Strike);
        }
...
def pick(chain):
    return chain.select_by_strike_distance(-10, OptionRight.PUT, target_dte=8)

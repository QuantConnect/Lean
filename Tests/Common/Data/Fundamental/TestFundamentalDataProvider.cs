...
namespace QuantConnect.Tests.Common.Data.Fundamental
{
    public class TestFundamentalDataProvider : IFundamentalDataProvider
    {
        ...
        private readonly Dictionary<string, double> _pERatio =
            new()
            {
        ...
        private readonly Dictionary<string, string> _industryTemplateCode =
            new()
            {
                { "AAPL R735QTJ8XC9X", "N" },
                { "IBM R735QTJ8XC9X", "N" },
        ...
        private readonly Dictionary<string, double> _equityPerShareGrowthOneYear =
            new()
            {
        ...
        private readonly Dictionary<string, long> _marketCap =
            new()
            {
        ...
        private readonly Dictionary<string, long> _sharesOutstanding =
            new() { { "SPY R735QTJ8XC9X", 1331000000 }, { "AAPL R735QTJ8XC9X", 22337000000000 }, };
        ...
        private dynamic Get(DateTime time, SecurityIdentifier securityIdentifier, string name)
        {
            switch (name)
            {
                ...
                case "CompanyProfile_SharesOutstanding":
                    if (
                        _sharesOutstanding.TryGetValue(
                            securityIdentifier.ToString(),
                            out var sharesOutstanding
                        )
                    )
                    {
                ...
                case "CompanyReference_IndustryTemplateCode":
                    if (
                        _industryTemplateCode.TryGetValue(
                            securityIdentifier.ToString(),
                            out var industryTemplateCode
                        )
                    )
                    {
                ...
                case "EarningRatios_EquityPerShareGrowth_OneYear":
                    if (
                        _equityPerShareGrowthOneYear.TryGetValue(
                            securityIdentifier.ToString(),
                            out var ePSG
                        )
                    )

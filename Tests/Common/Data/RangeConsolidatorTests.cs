...
#pragma warning restore CS0618
            ...
            for (int index = 0; index < returnedBars.Count; index++)
            {
                ...
                Assert.AreEqual(
                    1,
                    Math.Round(returnedBars[index].High - returnedBars[index].Low, 2)
                );
                ...
                {
                    Assert.IsTrue(
                        returnedBars[index].Open < lastRangeBar.Low
                            || returnedBars[index].Open > lastRangeBar.High
                    );
                }
        ...
        public virtual void PriceGapBehaviorIsTheExpectedOne(
            Symbol symbol,
        ...
        )
        {
            ...
            for (int index = 0; index < returnedBars.Count; index++)
            {
                ...
                {
                    Assert.IsTrue(
                        returnedBars[index].Open
                            == (lastRangeBar.High + (decimal)minimumPriceVariation)
                            || returnedBars[index].Open
                                == (lastRangeBar.Low - (decimal)minimumPriceVariation)
                    );
                }
                lastRangeBar = returnedBars[index];
            }
        ...
        [TestCase(
            new double[] { 94, 94.1, 94.2, 94.3, 94.4, 94.5, 94.6, 94.7, 94.8, 94.9, 95, 95.1 },
            new double[] { 94, 95, 94, 95, 110 }
        )]
        [TestCase(
            new double[] { 94, 93.9, 93.8, 93.7, 93.6, 93.5, 93.4, 93.3, 93.2, 93.1, 93, 92.9 },
            new double[] { 94, 94, 93, 93, 110 }
        )]
        [TestCase(
            new double[] { 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 94, 95.1 },
            new double[] { 94, 95, 94, 95, 160 }
        )]
        [TestCase(
            new double[] { 94, 93.9, 94.1, 93.8, 94.2, 93.7, 94.3, 93.6, 94.4, 93.5, 94.5, 93.4 },
            new double[] { 94, 94.5, 93.5, 93.5, 110 }
        )]
        public void ConsolidatorUpdatesTheVolumeOfTheBarsAsExpected(
            double[] testValues,
            double[] expectedBar
        )
        {
            using var consolidator = CreateConsolidator(100);
            var returnedBars = UpdateConsolidator(
                consolidator,
                new List<decimal>(testValues.Select(x => (decimal)x)),
                Symbols.IBM
            );
        ...
        private List<RangeBar> UpdateConsolidator(
            ...
            new object[]
            {
                ...
                new RangeBar[]
                {
                    new RangeBar
                    {
                        ...
                        Close = 93.5m,
                        Volume = 110,
                        EndTime = new DateTime(2016, 1, 12)
                    }
                }
            },
            new object[]
            {
                new List<decimal> { 94m, 94m, 94m, 94m, 94m, 95.1m },
                new RangeBar[]
                {
                    new RangeBar
                    {

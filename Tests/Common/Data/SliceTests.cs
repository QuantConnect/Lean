...
namespace QuantConnect.Tests.Common.Data
{
    ...
    public class SliceTests
    {
        ...
        public void AccessesByDataType()
        {
            ...
            var marginInterest = new MarginInterestRate
            {
                Symbol = Symbols.SPY,
                Time = now,
                InterestRate = 0.08m
        ...
        public void AccessesByDataTypeAndSymbol()
        {
...

            {
                ...
                Assert.IsTrue(
                    slice.TryGet<MarginInterestRate>(Symbols.SPY, out var foundMarginInterest)
                );
...

            {
                ...
                Assert.AreEqual(foundTradeBar, tradeBar);
                Assert.IsTrue(
                    slice.TryGet(typeof(UnlinkedData), Symbols.SPY, out var foundUnlinkedData)
                );
                Assert.AreEqual(foundUnlinkedData, unlinkedData);
                ...
                Assert.AreEqual(foundDelisting, delisting);
                Assert.IsTrue(
                    slice.TryGet(typeof(OpenInterest), Symbols.SPY, out var foundOpenInterest)
                );
                Assert.AreEqual(foundOpenInterest, openInterest);
                Assert.IsTrue(
                    slice.TryGet(
        ...
        public void DifferentCollectionsAreCorrectlyGeneratedSameSymbol()
        {
            var quoteBar = new QuoteBar(
                DateTime.Now,
                Symbols.SPY,
                new Bar(3100, 3100, 3100, 3100),
                0,
                new Bar(3101, 3101, 3101, 3101),
                0,
                Time.OneMinute
            );
            ...
            var slice = new Slice(
                DateTime.Now,
                new BaseData[] { quoteBar, tradeBar },
                DateTime.Now
            );
        ...
        public void AccessesCustomGenericallyByTypeOtherTypesPresent()
        {
            ...
            var slice = new Slice(
                DateTime.Now,
                new BaseData[] { unlinkedDataSpy, tradeBar },
                DateTime.Now
            );
        ...
        public void AccessesCustomGenericallyByType()
        {
            ...
            var slice = new Slice(
                DateTime.Now,
                new[] { unlinkedDataSpy, unlinkedDataAapl },
                DateTime.Now
            );
        ...
        public void MergeSlice()
        {
            ...
            var marginInterestRate1 = new MarginInterestRate
            {
                Time = _dataTime,
                Symbol = Symbols.SPY,
                InterestRate = 8
            ...
            var marginInterestRate2 = new MarginInterestRate
            {
                Time = _dataTime,
                Symbol = Symbols.SBIN,
                InterestRate = 18
        ...
        public void CheckMergeUpdatePrivateAttributes()
        {
            ...
            var slice3 = new Slice(
                _dataTime,
                new BaseData[] { tradeBar3, tradeBar4, tradeBar3_4 },
                _dataTime
            );
            ...
            var testTradeBar = (TradeBar)
                slice1
                    .Values.Where(datum =>
                        datum.DataType == MarketDataType.TradeBar && datum.Symbol.Value == "AAPL"
                    )
                    .Single();
        ...
        public void MergeOptionsAndFuturesChain()
        {
            ...
            var slice4 = new Slice(
                _dataTime,
                new List<BaseData>(),
                new TradeBars(_dataTime),
                new QuoteBars(),
                new Ticks(),
                optionChain1,
                futuresChain1,
                new Splits(),
                new Dividends(_dataTime),
                new Delistings(),
                new SymbolChangedEvents(),
                new MarginInterestRates(),
                _dataTime
            );
            var slice5 = new Slice(
                _dataTime,
                new List<BaseData>(),
                new TradeBars(_dataTime),
                new QuoteBars(),
        ...
        public void MergeCustomData()
        {
            ...
            var slice6 = new Slice(
                _dataTime,
                new BaseData[] { custom1, custom2, custom3, tradeBar2 },
                _dataTime
            );
...
def Test(slice, symbol):
    ...
    return data[symbol][""TimeTest""]"
                ...
                Assert.AreEqual(
                    ...
                    (DateTime)test(new PythonSlice(slice), Symbols.SPY)
                );
            }
        }
...
def Test(slice):
    ...
    return data"
                ...
                var config2 = new SubscriptionDataConfig(
                    config,
                    Extensions.CreateType(testModule.GetAttr("CustomDataTest2"))
                );
                ...
                var slice = new Slice(
                    DateTime.UtcNow,
                    new[] { unlinkedDataSpy, data2, data1 },
                    DateTime.UtcNow
                );
...
def Test(slice):
    ...
    return data"
                ...
                var slice = new Slice(
                    DateTime.Now,
                    new BaseData[] { unlinkedDataSpy, TradeBarAapl, unlinkedDataAapl, TradeBarSpy },
                    DateTime.Now
                );
...
def Test(slice):
    ...
    if count != 2:
        raise Exception('Unexpected value')"
                ...
                var slice = new Slice(
                    DateTime.Now,
                    new BaseData[] { indexedLinkedDataSpy, tradeBarAapl, indexedLinkedDataAapl },
                    DateTime.Now
                );
...
def Test(slice):
    ...
    for singleData in data.Values:
        raise Exception('Unexpected iteration')"
                ...
                var slice = new Slice(
                    DateTime.Now,
                    new List<BaseData> { tradeBarAapl },
                    DateTime.Now
                );
...
def Test(slice):
    ...
    if count != 2:
        ...
        public void AccessesTradeBarAndQuoteBarForSameSymbol()
        {
            ...
            var slice = new Slice(
                DateTime.Now,
                new List<BaseData>() { tradeBar, quoteBar },
                tradeBars,
                quoteBars,
                null,
                null,
...
def Test(slice):
    slice.popitem()"
                ...
                Assert.That(
                    ...
                    Throws
                        .InstanceOf<ClrBubbledException>()
                        .With.InnerException.InstanceOf<NotSupportedException>(),
...
def Test(slice, symbol):
    slice.pop(symbol)"
                ...
                Assert.That(
                    () => test(GetPythonSlice(), Symbols.SPY),
                    Throws
                        .InstanceOf<ClrBubbledException>()
                        .With.InnerException.InstanceOf<InvalidOperationException>(),
                    $"Slice is read-only: cannot pop the value for {Symbols.SPY} from the collection"
...
def Test(slice, symbol, default_value):
    slice.pop(symbol, default_value)"
                    )
                    .GetAttr("Test");

                Assert.That(
                    () => test(GetPythonSlice(), Symbols.SPY, null),
                    Throws
                        .InstanceOf<ClrBubbledException>()
                        .With.InnerException.InstanceOf<InvalidOperationException>(),
                    $"Slice is read-only: cannot pop the value for {Symbols.SPY} from the collection"
...
def Test(slice, symbol):
    copy = slice.copy()
    return ', '.join([f'{k}: {v.Value}' for k,v in copy.items()])"
                    )
                    .GetAttr("Test");
...
def Test(slice):
    return ', '.join([f'{k}: {v.Value}' for k,v in slice.items()])"
                    )
                    .GetAttr("Test");
...
def Test(slice, keys):
    ...
    return ', '.join([f'{k}: {v.Value}' for k,v in newDict.items()])"
                    )
                    .GetAttr("Test");
...
def Test(slice, keys, default_value):
    ...
    return ', '.join([f'{k}: {v.Value}' for k,v in newDict.items()])"
                ...
                var result = string.Empty;
                Assert.DoesNotThrow(
                    () => result = test(GetPythonSlice(), new[] { Symbols.EURUSD }, new Tick())
                );
...
def Test(slice, symbol, default_value):
    return slice.get(symbol, default_value)"
                    )
                    .GetAttr("Test");
                ...
                PyObject result = null;
                Assert.DoesNotThrow(
                    () => result = test(GetPythonSlice(), Symbols.EURUSD, expected)
                );
                BaseData actual;
                Assert.IsTrue(result.TryConvert(out actual));
                Assert.AreEqual(expected.Symbol, actual.Symbol);
...
def Test(slice, symbol):
    return slice.setdefault(symbol)"
                    )
                    .GetAttr("Test");
...
def Test(slice, symbol, default_value):
    return slice.setdefault(symbol, default_value)"
                    )
                    .GetAttr("Test");

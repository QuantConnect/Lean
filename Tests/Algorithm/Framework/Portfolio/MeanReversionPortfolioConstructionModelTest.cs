...
namespace QuantConnect.Tests.Algorithm.Framework.Portfolio
{
    ...
    public class MeanReversionPortfolioConstructionModelTests
    {
        ...
        private double[] _simplexExpectedArray1,
            _simplexExpectedArray2;
        ...
        public void CumulativeSum()
        {
            ...
            var result = MeanReversionPortfolioConstructionModel
                .CumulativeSum(list)
        ...
        public void GetPriceRelatives()
        {
            ...
            var result = model
                .TestGetPriceRelatives(insights)
        ...
        public void GetPriceRelativesPython()
        {
            ...
            using (Py.GIL())
            {
                ...
                var model = Py.Import(name)
                    .GetAttr(name)
                    .Invoke(
                        ((int)Resolution.Daily).ToPython(),
                        ((int)PortfolioBias.LongShort).ToPython(),
                        1.ToPython(),
                        2.ToPython()
                    );
                model.InvokeMethod(
                    "OnSecuritiesChanged",
                    _algorithm.ToPython(),
                    SecurityChangesTests.AddedNonInternal(aapl, spy).ToPython()
                );

                var result = PyList.AsList(
                    model.InvokeMethod("GetPriceRelatives", insights.ToPython())
                );
        ...
        public void GetPriceRelativesWithInsightMagnitudePython()
        {
            ...
            using (Py.GIL())
            {
                ...
                var model = Py.Import(name)
                    .GetAttr(name)
                    .Invoke(((int)Resolution.Daily).ToPython());
                model.InvokeMethod(
                    "OnSecuritiesChanged",
                    _algorithm.ToPython(),
                    SecurityChangesTests.AddedNonInternal(aapl, spy).ToPython()
        ...
        public void SimplexProjection(double regulator)
        {
            if (regulator <= 0)
            {
                var exception = Assert.Throws<ArgumentException>(
                    () =>
                        MeanReversionPortfolioConstructionModel.SimplexProjection(
                            _simplexTestArray,
                            regulator
                        )
                );
                Assert.That(
                    exception.Message,
                    Is.EqualTo("Total must be > 0 for Euclidean Projection onto the Simplex.")
                );
                return;
            }
            ...
            var result = MeanReversionPortfolioConstructionModel.SimplexProjection(
                _simplexTestArray,
                regulator
            );
                ...
                if (regulator <= 0)
                {
                    Assert.That(
                        () =>
                            model.InvokeMethod(
                                "SimplexProjection",
                                _simplexTestArray.ToPython(),
                                new PyFloat(regulator)
                            ),
                        Throws
                            .InstanceOf<ClrBubbledException>()
                            .With.InnerException.InstanceOf<ArgumentException>()
        ...
        )
        {
            ...
            {
                return new MeanReversionPortfolioConstructionModel(
                    resolution,
                    bias,
                    1,
                    1,
        ...
        private class TestMeanReversionPortfolioConstructionModel

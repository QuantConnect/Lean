...
namespace QuantConnect.Tests.Algorithm.Framework.Portfolio
{
    ...
    public class AccumulativeInsightPortfolioConstructionModelTests
    {
        ...
        public void EmptyInsightsReturnsEmptyTargets(Language language)
        {
            ...
            var actualTargets = _algorithm.PortfolioConstruction.CreateTargets(
                _algorithm,
                new Insight[0]
            );
        ...
        public void InsightsReturnsTargetsConsistentWithDirection(
            Language language,
        ...
        )
        {
            ...
            var expectedTargets = _algorithm.Securities.Select(x => new PortfolioTarget(
                x.Key,
                (int)direction * Math.Floor(amount / x.Value.Price)
            ));

            var insights = _algorithm.Securities.Keys.Select(x =>
                GetInsight(x, direction, _algorithm.UtcTime)
            );
            var actualTargets = _algorithm.PortfolioConstruction.CreateTargets(
                _algorithm,
                insights.ToArray()
        ...
        public void LongTermInsightCanceledByNew(Language language)
        {
            ...
            insights = new[]
            {
                GetInsight(Symbols.SPY, InsightDirection.Up, _algorithm.UtcTime, Time.OneMinute)
            };
            ...
            actualTargets = _algorithm.PortfolioConstruction.CreateTargets(
                _algorithm,
                new Insight[0]
            );
            AssertTargets(expectedTargets, actualTargets);
        ...
        public void LongTermInsightAccumulatesByNew(Language language, InsightDirection direction)
        {
            ...
            var expectedTargets = new List<IPortfolioTarget>
            {
                PortfolioTarget.Percent(
                    _algorithm,
                    Symbols.SPY,
                    (int)direction * 1m * (decimal)DefaultPercent
                )
            };
            AssertTargets(expectedTargets, targets);
            ...
            insights = new[]
            {
                GetInsight(Symbols.SPY, direction, _algorithm.UtcTime, Time.OneMinute)
            };
            targets = _algorithm.PortfolioConstruction.CreateTargets(_algorithm, insights).ToList();
            ...
            expectedTargets = new List<IPortfolioTarget>
            {
                PortfolioTarget.Percent(
                    _algorithm,
                    Symbols.SPY,
                    (int)direction * 1m * 2 * (decimal)DefaultPercent
                )
            };
            AssertTargets(expectedTargets, targets);
        ...
        [TestCase(Language.CSharp, InsightDirection.Up)]
        [TestCase(Language.CSharp, InsightDirection.Down)]
        public void InsightExpirationUndoesAccumulationBySteps(
            Language language,
            InsightDirection direction
        )
        {
            ...
            targets = _algorithm
                .PortfolioConstruction.CreateTargets(_algorithm, new Insight[0])
                .ToList();
        ...
        {
            PortfolioConstructionModel model = new AccumulativeInsightPortfolioConstructionModel(
                Resolution.Daily
            );
            ...
            var insights = new[]
            {
                GetInsight(Symbols.SPY, direction, _algorithm.UtcTime, TimeSpan.FromDays(10))
            };
            AssertTargets(
                new List<IPortfolioTarget>(),
                model.CreateTargets(_algorithm, insights).ToList()
            );
        ...
        [TestCase(Language.CSharp, InsightDirection.Up)]
        [TestCase(Language.CSharp, InsightDirection.Down)]
        public void InsightExpirationUndoesAccumulation(
            Language language,
            InsightDirection direction
        )
        {
        ...
        public void PortfolioBiasIsRespected(Language language, PortfolioBias bias)
        {
            ...
            _algorithm.PortfolioConstruction.OnSecuritiesChanged(
                _algorithm,
                SecurityChangesTests.AddedNonInternal(appl, spy, ibm, aig, qqq)
            );
        ...
        private Insight GetInsight(
            Symbol symbol,
            InsightDirection direction,
            DateTime generatedTimeUtc,
            TimeSpan? period = null,
            double? confidence = DefaultPercent
        )
        {
        ...
        {
            algorithm.SetPortfolioConstruction(
            ...
            if (language == Language.Python)
            {
                using (Py.GIL())
                {
                    ...
                    var instance = Py.Import(name)
                        .GetAttr(name)
                        .Invoke(((object)null).ToPython(), ((int)bias).ToPython());

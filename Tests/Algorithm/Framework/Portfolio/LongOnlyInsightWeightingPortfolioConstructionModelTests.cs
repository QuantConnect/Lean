    ...
    public class LongOnlyInsightWeightingPortfolioConstructionModelTests
    {
        ...
        public void InsightsReturnsTargetsConsistentWithDirection(Language language, InsightDirection direction)
        {
            ...
            var expectedTargets = _algorithm.Securities
                .Select(
                    x => new PortfolioTarget(
                        x.Key,
                        direction == InsightDirection.Down
                            ? 0
                            : (int) direction * Math.Floor(
                                  amount * (1 - _algorithm.Settings.FreePortfolioValuePercentage) / x.Value.Price
                              )
                    )
                );
        ...
        public void FlatDirectionNotAccountedToAllocation(Language language, InsightDirection direction)
        {
            ...
            var expectedTargets = _algorithm.Securities.Select(x =>
            {
                ...
                // Expected target for InsightDirection.Down is zero, because of Long bias 
                var quantity = x.Key.Value == "SPY" || direction == InsightDirection.Down
                    ? 0
                    : (int)direction * Math.Floor(
                          amount * (1 - _algorithm.Settings.FreePortfolioValuePercentage) / x.Value.Price
                      );
        ...
        public void LongTermInsightPreservesPosition(Language language)
        {
            ...
            var expectedTargets = new[] {new PortfolioTarget(Symbols.SPY, 0)};
        ...
        public void WeightsProportionally(Language language)
        {
            ...
            var insights = new[]
            {
                GetInsight(Symbols.SPY, InsightDirection.Up, _algorithm.UtcTime, weight:1),
                GetInsight(Symbol.Create("IBM", SecurityType.Equity, Market.USA),
                    InsightDirection.Up, _algorithm.UtcTime, weight:1)
            };
        ...
        private void SetPortfolioConstruction(Language language, QCAlgorithm algorithm, dynamic paramenter = null)
        {
            ...
            algorithm.SetPortfolioConstruction(InsightWeightingPortfolioConstructionModel.LongOnly(paramenter));

    ...
    public class EulerSearchOptimizationStrategyTests : OptimizationStrategyTests
    {
        ...
        public class EulerSearchTests
        {
            ...
            public void Depth(int _defaultSegmentAmount)
            {
                ...
                Queue<OptimizationResult> _pendingOptimizationResults =
                    new Queue<OptimizationResult>();
                int depth = -1;
                ...
                Assert.AreEqual(
                    Math.Ceiling(
                        Math.Log((double)(param.Step / param.MinStep), _defaultSegmentAmount)
                    ),
                    depth
                );
            }
            ...
            public void Reduce(int amountOfSegments)
            {
                ...
                _strategy.NewParameterSet += (s, parameterSet) =>
                {
                    ...
                    pendingOptimizationResults.Enqueue(
                        new OptimizationResult(
                            _stringify(_profit(parameterSet), _drawdown(parameterSet)),
                            parameterSet,
                            ""

    ...
    public class GridSearchOptimizationStrategyTests : OptimizationStrategyTests
    {
        ...
        public class GridSearchTests
        {
            ...
            public void Step1D(decimal min, decimal max, decimal step)
            {
                ...
                using (var enumerator = new EnqueueableEnumerator<ParameterSet>())
                {
                    ...
                    using (var paramEnumerator = new OptimizationStepParameterEnumerator(param))
                    {
                        while (paramEnumerator.MoveNext())
                        {
                            ...
                            Assert.IsTrue(
                                suggestion.Value.All(s => set.Any(arg => arg.Name == s.Key))
                            );
                ...
                Assert.AreEqual(
                    Math.Floor((param.MaxValue - param.MinValue) / param.Step.Value) + 1,
                    counter
                );
            }
            ...
            public void Estimate1D(decimal min, decimal max, decimal step)
            {
                ...
                Assert.AreEqual(
                    Math.Floor(Math.Abs(max - min) / Math.Abs(step)) + 1,
                    _strategy.GetTotalBacktestEstimate()
                );
            }

            private static TestCaseData[] OptimizationStepParameter2D =>
                new[]
                {
                    new TestCaseData(
                        new decimal[,]
            ...
            public void Step2D(decimal[,] data)
            {
                ...
                using (var enumerator = new EnqueueableEnumerator<ParameterSet>())
                {
                    ...
                    var fastParam =
                        args.First(arg => arg.Name == "ema-fast") as OptimizationStepParameter;
                    var slowParam =
                        args.First(arg => arg.Name == "ema-slow") as OptimizationStepParameter;

                    using (var fastEnumerator = new OptimizationStepParameterEnumerator(fastParam))
                    {
                        using (
                            var slowEnumerator = new OptimizationStepParameterEnumerator(slowParam)
                        )
                        {
            ...
            public void Step3D()
            {
                ...
                using (var enumerator = new EnqueueableEnumerator<ParameterSet>())
                {
                    ...
                    var slowParam =
                        args.First(arg => arg.Name == "ema-slow") as OptimizationStepParameter;
                    var customParam =
                        args.First(arg => arg.Name == "ema-custom") as OptimizationStepParameter;
                    using (var fastEnumerator = new OptimizationStepParameterEnumerator(fastParam))
                    {
                        using (
                            var slowEnumerator = new OptimizationStepParameterEnumerator(slowParam)
                        ...
                        {
                            using (
                                var customEnumerator = new OptimizationStepParameterEnumerator(
                                    customParam
                                )
                            )
                            {
        ...
        private static TestCaseData[] StrategySettings =>
            new[]
            {
                new TestCaseData(
                    new Maximization(),
                    OptimizationStepParameters,
                    new ParameterSet(
                        -1,
                        new Dictionary<string, string> { { "ema-slow", "5" }, { "ema-fast", "5" } }
                    )
                ),
                new TestCaseData(
                    new Minimization(),
                    ...
                    new ParameterSet(
                        -1,
                        new Dictionary<string, string> { { "ema-slow", "1" }, { "ema-fast", "3" } }
                    )
                ),
                new TestCaseData(
                    new Maximization(),
                    OptimizationMixedParameters,
                    new ParameterSet(
                        -1,
        ...
        public override void StepInsideNoTargetNoConstraints(
            Extremum extremum,
        ...
        private static TestCaseData[] OptimizeWithConstraint =>
            new[]
            {
                new TestCaseData(
                    0.05m,
                ...
                new TestCaseData(
                    ...
                    new ParameterSet(
                        -1,
                        new Dictionary<string, string> { { "ema-slow", "2" }, { "ema-fast", "3" } }
                    )
                ),
                new TestCaseData(
                    0.05m,
        ...
        public override void StepInsideWithConstraints(
            decimal drawdown,
            HashSet<OptimizationParameter> optimizationParameters,
            ParameterSet solution
        )
        ...
        private static TestCaseData[] OptimizeWithTarget =>
            new[]
            {
                new TestCaseData(
                    0m,
                ...
                new TestCaseData(
                    ...
                    new ParameterSet(
                        -1,
                        new Dictionary<string, string> { { "ema-slow", "1" }, { "ema-fast", "5" } }
                    )
                ),
                new TestCaseData(
                    8m,
                    ...
                    new ParameterSet(
                        -1,
                        new Dictionary<string, string> { { "ema-slow", "3" }, { "ema-fast", "5" } }
                    )
                ),
                new TestCaseData(
                    0m,
        ...
        public override void StepInsideWithTarget(
            decimal targetValue,
            HashSet<OptimizationParameter> optimizationParameters,
            ParameterSet solution
        )
        ...
        private static TestCaseData[] OptimizeWithTargetNotReached =>
            new[]
            {
                new TestCaseData(
                    15m,
        ...
        public override void TargetNotReached(
            decimal targetValue,
            HashSet<OptimizationParameter> optimizationParameters,
            ParameterSet solution
        )

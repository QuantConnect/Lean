...
namespace QuantConnect.Tests.Optimizer.Parameters
{
    ...
    public class OptimizationParameterEnumeratorTests
    {
        ...
        public class StepParameter
        {
            ...
            {
                using var enumerator = new OptimizationStepParameterEnumerator(
                    optimizationParameter
                );
                int total = 0;

                for (
                    decimal value = optimizationParameter.MinValue;
                    value <= optimizationParameter.MaxValue;
                    value += optimizationParameter.Step.Value
                )
                {
                    total++;
                ...
                Assert.AreEqual(
                    Math.Floor(
                        (optimizationParameter.MaxValue - optimizationParameter.MinValue)
                            / optimizationParameter.Step.Value
                    ) + 1,
                    total
                );

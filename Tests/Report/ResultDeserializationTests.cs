...
namespace QuantConnect.Tests.Report
{
    ...
    public class ResultDeserializationTests
    {
        ...
        private const string ValidBacktestResultJson2 =
            "{\"backtest\":{\"note\":null,\"name\":\"Emotional Black Mosquito\",\"organizationId\":99568,\"projectId\":15863659,\"completed\":true,\"optimizationId\":null,\"backtestId\""
        ...
        private const string ValidBacktestResultJson =
            "{\"RollingWindow\":{\"M1_20131011\":{\"TradeStatistics\":{\"StartDateTime\":null,\"EndDateTime\":null,\"TotalNumberOfTrades\":0,\"NumberOfWinningTrades\":0,\"NumberOfLosingTrades\":0,\"TotalProfitLoss\":\"0\",\"TotalProfit\":\"0\",\"TotalLoss\":\"0\",\"LargestProfit\":\"0\",\"LargestLoss\":\"0\",\"AverageProfitLoss\":\"0\","
            ...
            + "\"ema-slow\":\"20\"}}}";
        private const string InvalidBacktestResultJson =
            "{\"RollingWindow\":{},\"TotalPerformance\":null,\"Charts\":{\"Equity\":{\"Name\":\"Equity\",\"ChartType\":0,\"Series\":{\"Performance\":{\"Name\":\"Performance\",\"Unit\":\"$\",\"Index\":0,\"Values\":[{\"x\":1583704925,\"y\":5.0},{\"x\":1583791325,\"y\":null},{\"x\":1583877725,\"y\":7.0},{\"x\":1583964125,\"y\":8.0},{\"x\":1584050525,\"y\":9.0}],\"SeriesType\":0,\"Color\":\"\",\"ScatterMarkerSymbol\":\"none\"}}}},\"Orders\":"
            + OrderStringReplace
            + ",\"ProfitLoss\":{},\"Statistics\":{},\"RuntimeStatistics\":{}}";
        private const string InvalidLiveResultJson =
            "{\"Holdings\":{},\"Cash\":{\"USD\":{\"SecuritySymbol\":{\"Value\":\"\",\"ID\":\" 0\",\"Permtick\":\"\"},\"Symbol\":\"USD\",\"Amount\":0.0,\"ConversionRate\":1.0,\"CurrencySymbol\":\"$\",\"ValueInAccountCurrency\":0.0}},\"ServerStatistics\":{\"CPU Usage\":\"0.0%\",\"Used RAM (MB)\":\"68\",\"Total RAM (MB)\":\"\",\"Used Disk Space (MB)\":\"1\",\"Total Disk Space (MB)\":\"5\",\"Hostname\":\"LEAN\",\"LEAN Version\":\"v2.4.0.0\"},\"Charts\":{\"Equity\":{\"Name\":\"Equity\",\"ChartType\":0,\"Series\":{\"Performance\":{\"Name\":\"Performance\",\"Unit\":\"$\",\"Index\":0,\"Values\":[{\"x\":1583705127,\"y\":5.0},{\"x\":1583791527,\"y\":null},{\"x\":1583877927,\"y\":7.0},{\"x\":1583964327,\"y\":8.0},{\"x\":1584050727,\"y\":9.0}],\"SeriesType\":0,\"Color\":\"\",\"ScatterMarkerSymbol\":\"none\"}}}},\"Orders\":"
            + OrderStringReplace
            + ",\"ProfitLoss\":{},\"Statistics\":{},\"RuntimeStatistics\":{}}";
        private const string OrderJson =
            @"{'1': {
    'Type':"
            + OrderTypeStringReplace
            + @",
    ...
    'GroupOrderManager': {
        ...
        public void ValidBacktestResultDefaultSerializer(string chartKey)
        {
            var result = JsonConvert
                .DeserializeObject<BacktestResponseWrapper>(
                    ValidBacktestResultJson2.Replace(
                        "charts",
                        chartKey,
            ...
            Assert.IsTrue(
                result.Charts.Where(x => x.Key == "Drawdown").All(kvp => !kvp.Value.IsEmpty())
            );
        }
        ...
        public void ValidBacktestResult()
        {
            ...
            var result = JsonConvert.DeserializeObject<BacktestResult>(
                ValidBacktestResultJson,
                settings
            );
        ...
        public void BacktestResult_NullChartPoint_IsSkipped()
        {
            ...
            var deWithoutConverter = JsonConvert.DeserializeObject<BacktestResult>(
                InvalidBacktestResultJson.Replace(OrderStringReplace, EmptyJson),
                settings
            );
            var deWithConverter = JsonConvert.DeserializeObject<BacktestResult>(
                InvalidBacktestResultJson.Replace(OrderStringReplace, EmptyJson),
                converter
            );
            ...
            var roundtripDeserialization = JsonConvert.DeserializeObject<BacktestResult>(
                convertedSerialized
            );

            Assert.IsTrue(
                withConverterPoints.SequenceEqual(GetChartPoints(roundtripDeserialization).ToList())
            );
        }
        ...
        public void LiveResult_NullChartPoint_IsSkipped()
        {
            ...
            var deWithoutConverter = JsonConvert.DeserializeObject<LiveResult>(
                InvalidLiveResultJson.Replace(OrderStringReplace, EmptyJson),
                settings
            );
            var deWithConverter = JsonConvert.DeserializeObject<LiveResult>(
                InvalidLiveResultJson.Replace(OrderStringReplace, EmptyJson),
                converter
            );
            ...
            var convertedSerialized = JsonConvert.SerializeObject(deWithConverter);
            var roundtripDeserialization = JsonConvert.DeserializeObject<LiveResult>(
                convertedSerialized
            );
        ...
        public void NullCandleStickPoint_IsSkipped()
        {
            ...
            var nullCandlestick =
                "{\"Charts\":{\"Strategy Equity\":{\"Name\":\"Strategy Equity\",\"ChartType\":0,\"Series\":{\"Equity\":{\"Name\":\"Equity\",\"Values\":[[1695991900,null,null,null,null],[1695992500,10000000,10000000,10000000,10000000]],\"SeriesType\":2,\"Index\":2,\"Unit\":\"\"}}}}}";
            var deWithoutConverter = JsonConvert.DeserializeObject<LiveResult>(
                nullCandlestick,
                settings
            );
            ...
            var noConverterPoints = deWithoutConverter
                .Charts["Strategy Equity"]
                .Series["Equity"]
                .Values.Cast<Candlestick>()
                .ToList();
            var withConverterPoints = deWithConverter
                .Charts["Strategy Equity"]
                .Series["Equity"]
                .Values.Cast<Candlestick>()
                .ToList();
            ...
            var roundTripCandlestick = roundtripDeserialization
                .Charts["Strategy Equity"]
                .Series["Equity"]
                .Values.Cast<Candlestick>();
            ...
            Assert.IsTrue(
                withConverterPoints
                    .Select(x => x.Close)
                    .SequenceEqual(roundTripCandlestick.Select(x => x.Close))
            );
        }
        ...
        public void OrderTypeEnumStringAndValueDeserialization()
        {
            ...
            foreach (var orderType in (OrderType[])Enum.GetValues(typeof(OrderType)))
            {
                ...
                var intValueJson = OrderJson.Replace(
                    OrderTypeStringReplace,
                    ((int)orderType).ToStringInvariant()
                );
                var upperCaseJson = OrderJson.Replace(
                    OrderTypeStringReplace,
                    $"'{orderType.ToStringInvariant().ToUpperInvariant()}'"
                );
                var camelCaseJson = OrderJson.Replace(
                    OrderTypeStringReplace,
                    $"'{orderType.ToStringInvariant().ToCamelCase()}'"
                );

                var intValueLiveResult = InvalidLiveResultJson.Replace(
                    OrderStringReplace,
                    intValueJson
                );
                var upperCaseLiveResult = InvalidLiveResultJson.Replace(
                    OrderStringReplace,
                    upperCaseJson
                );
                var camelCaseLiveResult = InvalidLiveResultJson.Replace(
                    OrderStringReplace,
                    camelCaseJson
                );

                var intInstance = JsonConvert
                    .DeserializeObject<LiveResult>(intValueLiveResult, settings)
                    .Orders.Values.Single();
                var upperCaseInstance = JsonConvert
                    .DeserializeObject<LiveResult>(upperCaseLiveResult, settings)
                    .Orders.Values.Single();
                var camelCaseInstance = JsonConvert
                    .DeserializeObject<LiveResult>(camelCaseLiveResult, settings)
                    .Orders.Values.Single();
                ...
                Assert.AreEqual(
                    intInstance.TimeInForce.GetType(),
                    upperCaseInstance.TimeInForce.GetType()
                );
                ...
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.AskPrice,
                    upperCaseInstance.OrderSubmissionData?.AskPrice
                );
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.BidPrice,
                    upperCaseInstance.OrderSubmissionData?.BidPrice
                );
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.LastPrice,
                    upperCaseInstance.OrderSubmissionData?.LastPrice
                );
                ...
                Assert.AreEqual(
                    intInstance.TimeInForce.GetType(),
                    camelCaseInstance.TimeInForce.GetType()
                );
                ...
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.AskPrice,
                    camelCaseInstance.OrderSubmissionData?.AskPrice
                );
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.BidPrice,
                    camelCaseInstance.OrderSubmissionData?.BidPrice
                );
                Assert.AreEqual(
                    intInstance.OrderSubmissionData?.LastPrice,
                    camelCaseInstance.OrderSubmissionData?.LastPrice
                );
            }
        }

        [TestCaseSource(nameof(CreatesReportParametersTableCorrectlyTestCases))]
        public void CreatesReportParametersTableCorrectly(
            string parametersTemplate,
            Dictionary<string, string> parameters,
            string expectedParametersTable
        )
        {
            ...
            var parametersReportElment = new ParametersReportElement(
                "parameters",
                "",
...
"
        ...
        [TestCase(
            @"<!--crisis<div class=""col-xs-4""><table class=""crisis-chart table compact""><thead><tr><th style=""display: block; height: 75px;"">{{$TEXT-CRISIS-TITLE}}</th></tr></thead><tbody><tr><td style=""padding:0;""><img src=""{{$PLOT-CRISIS-CONTENT}}""></td></tr></tbody></table></div>crisis-->",
            @"<!--crisis(\r|\n)*((\r|\n|.)*?)crisis-->",
            @"<div class=""col-xs-4""><table class=""crisis-chart table compact""><thead><tr><th style=""display: block; height: 75px;"">{{$TEXT-CRISIS-TITLE}}</th></tr></thead><tbody><tr><td style=""padding:0;""><img src=""{{$PLOT-CRISIS-CONTENT}}""></td></tr></tbody></table></div>"
        )]
        [TestCase(
            @"<!--parameters<tr><td class = ""title""> {{$FIRST-KPI-NAME}} </td><td> {{$FIRST-KPI-VALUE}} </td></tr>parameters-->",
            @"<!--parameters(\r|\n)*((\r|\n|.)*?)parameters-->",
            @"<tr><td class = ""title""> {{$FIRST-KPI-NAME}} </td><td> {{$FIRST-KPI-VALUE}} </td></tr>"
        )]
        public void GetsExpectedCrisisAndParametersHTMLCodes(
            string input,
            string pattern,
            string expected
        )
...
parameters-->",
        ...
        [TestCase(
            @"<div class=""col-xs-4""><table class=""crisis-chart table compact""><thead><tr><th style=""display: block; height: 75px;"">{{$TEXT-CRISIS-TITLE}}</th></tr></thead><tbody><tr><td style=""padding:0;""><img src=""{{$PLOT-CRISIS-CONTENT}}""></td></tr></tbody></table></div>crisis-->",
            @"<!--crisis(\r|\n)*((\r|\n|.)*?)crisis-->"
        )]
        [TestCase(
        ...
        [TestCase(
            @"<tr><td class = ""title""> {{$FIRST-KPI-NAME}} </td><td> {{$FIRST-KPI-VALUE}} </td></tr>parameters-->",
            @"<!--parameters(\r|\n)*((\r|\n|.)*?)parameters-->"
        )]
        [TestCase(
        ...
        public IEnumerable<KeyValuePair<long, decimal?>> GetChartPoints(Result result)
        {
            return result
                ...
                .Select(point => new KeyValuePair<long, decimal?>(point.x, point.y));
        }

        private const string htmlExampleCode =

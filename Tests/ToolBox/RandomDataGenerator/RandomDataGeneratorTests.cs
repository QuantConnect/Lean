...
namespace QuantConnect.Tests.ToolBox.RandomDataGenerator
{
    ...
    public class RandomDataGeneratorTests
    {
        ...
        public void NextRandomGeneratedData(DateTime start, DateTime end, DateTime expectedMidPoint)
        {
            ...
            var midPoint =
                QuantConnect.ToolBox.RandomDataGenerator.RandomDataGenerator.GetDateMidpoint(
                    start,
                    end
                );
            var delistDate =
                QuantConnect.ToolBox.RandomDataGenerator.RandomDataGenerator.GetDelistingDate(
                    start,
                    end,
        ...
        public void RandomGeneratorProducesValuesBoundedForEquitiesWhenSplit(
            string start,
            string end
        )
        {
            ...
            var randomValueGenerator = new RandomValueGenerator();
            var tickGenerator = new TickGenerator(
                settings,
                new TickType[1] { TickType.Trade },
                security,
                randomValueGenerator
            )
                .GenerateTicks()
            ...
            Assert.IsTrue(
                0.099m <= dividendsSplitsMaps.FinalSplitFactor
                    && dividendsSplitsMaps.FinalSplitFactor <= 1.5m
            );
            ...
            foreach (var tick in tickHistory)
            {
                ...
                Assert.IsTrue(
                    0.001m <= tick.Value && tick.Value <= 1000000000,
                    $"The tick value was {tick.Value} but should have been bounded by 0.001 and 1 000 000 000"
                );
            }
        }

    ...
    public partial class PandasConverterTests
    {
        ...
        private static bool IsNewerPandas()
        {
            ...
            using (Py.GIL())
            {
                ...
                local["pd"] = pandas;
                newPandas = PythonEngine
                    .Eval("int(pd.__version__.split('.')[0]) >= 1", locals: local)
                    .As<bool>();
            }
            return newPandas;
        }
        ...
        public void HandlesQuoteBars()
        {
            ...
            var rawBars = Enumerable
                .Range(0, 10)
                .Select(i => new QuoteBar(
                    DateTime.UtcNow.AddMinutes(i),
                    symbol,
                    new Bar(i + 1.01m, i + 1.02m, i + 1.00m, i + 1.01m),
                    0m,
                    new Bar(i + 1.01m, i + 1.02m, i + 1.00m, i + 1.01m),
                    0m
                ))
...
def Test(dataFrame):
    ...
    lowData = data.low
    if lowData.empty:
        raise Exception('LOW history low data is empty')"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, symbol):
    ...
    data = data[['lastprice', 'quantity']][:-1]"
                    )
                    .GetAttr("Test");
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame, symbol):
    data = dataFrame.at[({index},), 'lastprice']"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, symbol):
    ...
    if {index} not in axes.levels[0]:
        raise ValueError('SPY was not found')"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, other, symbol):
    ...
    if not np.isnan(data):
        raise Exception('Data should be NaN')"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, symbol):
    ...
    data = data.loc[{index}]
    if data != np.float64:
        raise Exception('Data type is ' + str(data))"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, other, symbol):
    if not dataFrame.equals(other):
        raise Exception('Dataframe are not equal')"
                    )
                    .GetAttr("Test");
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame, symbol):
    data = dataFrame['lastprice'][{index}]"
                    )
                    .GetAttr("Test");
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_get_value_index(
            string index,
            bool cache = false
        )
        {
...
def Test(dataFrame, symbol):
    ...
    if data is 0:
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_get_value_column(
            string index,
            bool cache = false
        )
        {
    ...
    if data is 0:
        raise Exception('Data is zero')"
                ...
                Assert.DoesNotThrow(
                    () =>
                        test(
                            GetTestDataFrame(Symbols.SPY, 10),
                            GetTestDataFrame(Symbols.AAPL, 10),
                            Symbols.SPY
                        )
                );
            }
...
def Test(dataFrame, symbol):
    ...
    if data is 0:
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_index_levels_contains(
            string index,
            bool cache = false
        )
        {
...
def Test(dataFrame, symbol):
    if {index} not in dataFrame.index.levels[0]:
        ...
        public void BackwardsCompatibilityDataFrame_items(
            string method,
            string index,
            bool cache = false
        )
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame, symbol):
    data = dataFrame.lastprice.loc[{index}]"
                    )
                    .GetAttr("Test");
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_loc_SubDataFrame(
            string index,
            bool cache = false
        )
        {
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame, symbol):
    data = dataFrame.loc[{index}].loc['2013-10-07 04:00:00']"
                    )
                    .GetAttr("Test");
...
"
                ...
                Assert.DoesNotThrow(
                    () => indexedDataFrame = indexDataFrameBySymbols(dataFrame, looupkSymbols)
                );
                Assert.IsNotNull(indexedDataFrame);

                dynamic aaplDataFrame = null;
                Assert.DoesNotThrow(
                    () => aaplDataFrame = indexDataFrameBySymbol(indexedDataFrame, Symbols.AAPL)
                );
                Assert.IsNotNull(aaplDataFrame);

                dynamic googDataFrame = null;
                Assert.DoesNotThrow(
                    () => googDataFrame = indexDataFrameBySymbol(indexedDataFrame, Symbols.GOOG)
                );
                ...
                Assert.Throws<PythonException>(
                    () => spyDataFrame = indexDataFrameBySymbol(indexedDataFrame, Symbols.SPY)
                );
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame):
    return dataFrame.loc[(slice(None), '{index}'), 'lastprice']"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, dataFrame2, symbol):
    ...
    if len(data) is 0:
        ...
        public void BackwardsCompatibilityDataFrame_nlarg_smallest(
            string method,
            string index,
            bool cache = false
        )
...
def Test(dataFrame, symbol):
    ...
    data = series.shift(freq=d(1))"
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_tz_localize_convert(
            string index,
            bool cache = false
        )
        {
...
def Test(dataFrame, symbol):
    ...
    if data is 0:
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_unstack_lastprice(
            string index,
            bool cache = false
        )
        {
            ...
            using (Py.GIL())
            {
                dynamic test = PyModule
                    .FromString(
                        ...
                        $@"
def Test(dataFrame, symbol):
    data = dataFrame.unstack(level=0).lastprice[{index}]"
                    )
                    .GetAttr("Test");
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilityDataFrame_unstack_loc_loc(
            string index,
            bool cache = false
        )
        {
...
def Test(dataFrame, symbol):
    ...
    return str([result[x] for x in [symbol]])"
                    )
                    .GetAttr("Test");
                var result = "Remapper";
                Assert.DoesNotThrow(
                    () => result = test(GetTestDataFrame(Symbols.SPY), Symbols.SPY)
                );
...
def Test(dataFrame, symbol):
    ...
    to_append = pd.Series([100], name={symbol}, index=pd.Index([dt.now()], name='time'))
    result = pd.concat([close, to_append], ignore_index=True)
    return repr([result[x] for x in [symbol]])"
                    )
                    .GetAttr("Test");
                var result = "Remapper";
                Assert.DoesNotThrow(
...
def Test(dataFrame, symbol):
    series = dataFrame.lastprice.droplevel(0)
    data = series.between_time('02:00', '06:00')"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, symbol):
    series = dataFrame.lastprice
    data = series.resample('2S', level=1).mean()"
                    )
                    .GetAttr("Test");
...
def Test(dataFrame, symbol):
    ...
    if data is 0:
        ...
        [TestCase("symbol")]
        [TestCase("str(symbol.ID)")]
        public void BackwardsCompatibilitySeries_tz_localize_convert(
            string index,
            bool cache = false
        )
        {
...
def Test(dataFrame, symbol):
    if 'SPY' not in dataFrame.index.levels[0]:
        raise ValueError('SPY was not found')"
                    ...
                    .GetAttr("Test");
                Assert.Throws<PythonException>(
                    () => test(GetTestDataFrame(Symbols.SPY), Symbols.SPY)
                );
            }
        }
        ...
        public void HandlesTradeTicks()
        {
            ...
            var rawBars = Enumerable
                ...
                .Select(i => new Tick(
                    symbol,
                    $"1440{i:D2}00,167{i:D2}00,1{i:D2},T,T,0",
        ...
        public void HandlesQuoteTicks()
        {
            ...
            var rawBars = Enumerable
                .Range(0, 10)
                .Select(i => new Tick(
                    DateTime.UtcNow.AddMilliseconds(100 * i),
                    symbol,
                    0.99m,
                    1.01m
                ))
        ...
        private static Resolution[] ResolutionCases =
        {
            Resolution.Tick,
            Resolution.Minute,
            Resolution.Second
        };
        private static Symbol[] SymbolCases =
        {
        ...
        [Test]
        public void HandlesOpenInterestTicks(
            [ValueSource(nameof(ResolutionCases))] Resolution resolution,
            [ValueSource(nameof(SymbolCases))] Symbol symbol
        )
        {
            // Arrange
            var converter = new PandasConverter();
            ...
            var subcriptionDataConfig = new SubscriptionDataConfig(
                dataType,
                symbol,
                resolution,
                TimeZones.Chicago,
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                var openInterestTicks = new OpenInterest(
                    subcriptionDataConfig,
                    symbol,
                    line,
            ...
            using (Py.GIL())
            {
                ...
                {
                    var index = subDataFrame.index[i];
                    var value = subDataFrame
                        .loc[index]
                        .openinterest.AsManagedObject(typeof(decimal));
        ...
        public void HandlesCustomDataBars(Type type, string format)
        {
            ...
            if (type == typeof(CustomData))
                custom.Reader(
                    config,
                    "date,open,high,low,close,transactions",
                    DateTime.UtcNow,
                    false
                );
            ...
            var rawBars = Enumerable
                ...
                {
                    var line =
                        $"{DateTime.UtcNow.AddDays(i).ToStringInvariant(format)},{i + 101},{i + 102},{i + 100},{i + 101},{i + 101}";
            ...
            using (Py.GIL())
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    var transactions = subDataFrame
                        .loc[index]
                        .transactions.AsManagedObject(typeof(decimal));
        ...
        public void HandlesCustomDataWithValueColumn()
        {
            ...
            var rawBars = Enumerable
                ...
                {
                    var line =
                        $"{DateTime.UtcNow.AddDays(i).ToStringInvariant("yyyy-MM-dd")},{i + 40000}";
                    return custom.Reader(config, line, DateTime.UtcNow.AddDays(i), false);
                })
                .ToArray();
        ...
        public void HandlesCustomDataBarsInheritsFromTradeBar(Type type, string propertyName)
        {
            ...
            var rawBars = Enumerable
                ...
                {
                    var line =
                        $"{DateTime.UtcNow.AddDays(i).ToStringInvariant("yyyyMMdd HH:mm")},{i + 101},{i + 102},{i + 100},{i + 101},{i + 101}";
                    return custom.Reader(config, line, DateTime.UtcNow.AddDays(i), false)
                        as BaseData;
                })
                .ToArray();
            ...
            using (Py.GIL())
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    var transactions = subDataFrame
                        .loc[index][propertyName.ToLowerInvariant()]
                        .AsManagedObject(typeof(decimal));
        ...
        public void HandlesLeanDataReaderOutputForSpotMarkets(
            string securityType,
            string market,
            string resolution,
            string ticker,
        ...
        )
        {
            using (Py.GIL())
            {
                // Arrange
                var dataFolder = "../../../Data";
                var filepath = LeanDataReaderTests.GenerateFilepathForTesting(
                    dataFolder,
                    securityType,
                    market,
                    resolution,
                ...
                if (columnsNumber == 3 || columnsNumber == 6)
                {
                    Assert.AreEqual(
                        sumValue,
                        df.get("lastprice").sum().AsManagedObject(typeof(double)),
                        1e-4
                    );
                }
                else
                {
                    Assert.AreEqual(
                        sumValue,
                        df.get("close").sum().AsManagedObject(typeof(double)),
                        1e-4
                    );
                }
            }
        ...
        public void HandlesLeanDataReaderOutputForOptionAndFutures(
        ...
        )
        {
            using (Py.GIL())
            {
                ...
                if (lastPrice != null)
                {
                    Assert.AreEqual(
                        sumValue,
                        lastPrice.sum().AsManagedObject(typeof(double)),
                        1e-4
                    );
                }
                else if (columnsNumber == 1)
                {
                    Assert.AreEqual(
                        sumValue,
                        df.get("openinterest").sum().AsManagedObject(typeof(double)),
                        1e-4
                    );
                }
                else
...
def Test():
    ...
    return pdConverter.GetIndicatorDataFrame({{'ind': [IndicatorDataPoint()]}})"
                    )
                    .GetAttr("Test");
        ...
        public void DoesntAcceptOtherPythonObjectRatherThanDict()
        {
            using (Py.GIL())
            {
                dynamic tests = PyModule.FromString(
                    "testModule",
                    $@"
from QuantConnect.Python import PandasConverter
from QuantConnect.Indicators import IndicatorDataPoint;
...
def DataFrameIsEmpty():
    ...
    return dataFrame.empty"
                ...
                dynamic dataFrameContainsCorrectData = tests.GetAttr(
                    "DataFrameContainsCorrectData"
                );
        ...
        public void ReturnsTheCorrectDataInTheDataFrameFromDictionary()
        {
            using (Py.GIL())
            {
                ...
                Func<int, IndicatorDataPoint> makePoint = i => new IndicatorDataPoint(
                    dateTime.AddMinutes(i),
                    i
                );
                var indicator1DataPoints = Enumerable
                    .Range(0, 10)
                    .Select(i => makePoint(i))
                    .ToList();
                var indicator2DataPoints = Enumerable
                    .Range(0, 5)
                    .Select(i => makePoint(i))
                    .ToList();
                ...
                var dataFrame = pdConverter.GetIndicatorDataFrame(
                    new Dictionary<string, List<IndicatorDataPoint>>
                    {
                        { "ind1", indicator1DataPoints },
                        { "ind2", indicator2DataPoints },
                        { "ind3", indicator3DataPoints }
                ...
                Func<string, List<double>> getIndicatorData = key =>
                    dataFrame.GetItem(key).GetAttr("tolist").Invoke().As<List<double>>();
                CollectionAssert.AreEqual(
                    new int[] { 10, 3 },
                    dataFrame.GetAttr("shape").As<int[]>()
                );
                CollectionAssert.AreEqual(
                    new string[] { "ind1", "ind2", "ind3" },
                    dataFrame.GetAttr("columns").As<string[]>()
                );
                CollectionAssert.AreEqual(
                    indicator1DataPoints.Select(point => point.EndTime).ToList(),
                    dataFrame.GetAttr("index").GetAttr("tolist").Invoke().As<List<DateTime>>()
                );
                CollectionAssert.AreEqual(
                    indicator1DataPoints.Select(point => point.Value),
                    getIndicatorData("ind1").GetRange(0, indicator1DataPoints.Count)
                );
                CollectionAssert.AreEqual(
                    indicator2DataPoints.Select(point => point.Value),
                    getIndicatorData("ind2").GetRange(0, indicator2DataPoints.Count)
                );
                CollectionAssert.AreEqual(
                    indicator3DataPoints.Select(point => point.Value),
                    getIndicatorData("ind3").GetRange(0, indicator3DataPoints.Count)
                );
            }
        }

        [TestCaseSource(nameof(GetHistoryWithDuplicateTimes))]
        public void HandlesSlicesWithDuplicateTimeStamps(
            Symbol symbol,
            IEnumerable<Slice> data,
            string expectedDataFrameString
        )
        {
            var converter = new PandasConverter();
            ...
            {
                Assert.AreEqual(
                    expectedDataFrameString.Replace("\n", string.Empty).Replace("\r", string.Empty),
                    dataFrame
                        .to_string()
                        .As<string>()
                        .Replace("\n", string.Empty)
        ...
        public IEnumerable<Slice> GetHistory<T>(
            Symbol symbol,
                    ...
                    .Create(
                        ...
                        new List<DataFeedPacket>
                        {
                            new DataFeedPacket(
                                ...
                                new List<BaseData>() { t as BaseData }
                            )
                        },
        ...
        private dynamic GetTestDataFrame(IEnumerable<Symbol> symbols, int count = 1)
        {
            var slices = Enumerable
                ...
                .Select(i =>
                {
                    ...
                    return new Slice(
                        time,
                        symbols.Select(symbol => new Tick
                        {
                            Time = time,
                            Symbol = symbol,
                            Value = 167 + i / 10,
                        ...
                        new Slice(
                            ...
                            new BaseData[]
                            {
                                ...
                                new QuoteBar(
                                    time,
                                    symbol,
                                    new Bar(101m, 102m, 100m, 101m),
                                    99m,
                                    new Bar(110m, 112m, 105m, 110m),
                                    98m
                                )
                            },
                            time
                ...
                new TestCaseData(
                    ...
                    new[]
                    {
                        ...
                        new Slice(
                            ...
                            {
                                new QuoteBar(
                                    time.AddMinutes(1).AddSeconds(1),
                                    Symbols.BTCUSD,
                                    new Bar(101m, 102m, 100m, 101m),
                                    99m,
                                    new Bar(110m, 112m, 105m, 110m),
        ...
        internal class SubTradeBar : TradeBar
        {
            ...
            public SubTradeBar(TradeBar tradeBar)
                : base(tradeBar) { }
        ...
        internal class SubSubTradeBar : SubTradeBar
        {
            ...
            public SubSubTradeBar(TradeBar tradeBar)
                : base(tradeBar) { }

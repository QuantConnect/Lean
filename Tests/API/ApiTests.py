...
from Api import Api, ApiConnection
import unittest
import os

class TestApi(unittest.TestCase):
    
    def setUp(self, testToken = "", userId):
        self.testToken = testToken
        self.userId = userId
        self.api = Api(userId, testToken)
    ...
    def test_CRUD_ProjectFiles_Successfully(self):
        ...
        with open(os.getcwd()+ '/../../Algorithm.Python/BasicTemplateAlgorithm.py', 'r') as myfile:
            real_file_code = myfile.read().decode("utf-8-sig").encode("utf-8")
        
        with open(os.getcwd()+ '/../../Algorithm.Python/BasicTemplateForexAlgorithm.py', 'r') as myfile:
            second_real_file_code = myfile.read().decode("utf-8-sig").encode("utf-8")

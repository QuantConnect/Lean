...
namespace QuantConnect.Tests.Brokerages.LevelOneOrderBook
{
    ...
    public class LevelOneMarketDataTests
    {
        ...
        {
            var openInterestDateTime = new DateTime(2026, 06, 20);
            var levelOneMarketData = new LevelOneMarketData(_aapl);
            var eventRaised = default(bool);

            levelOneMarketData.BaseDataReceived += (_, e) => eventRaised = true;

            levelOneMarketData.UpdateOpenInterest(openInterestDateTime, openInterest);
        ...
        {
            var lastTradeDateTime = new DateTime(2026, 06, 20);
            var levelOneMarketData = new LevelOneMarketData(_aapl);
            var eventRaised = default(bool);

            levelOneMarketData.BaseDataReceived += (_, e) => eventRaised = true;

            levelOneMarketData.UpdateLastTrade(lastTradeDateTime, lastQuantity, lastPrice);
        ...
        public void UpdateLastTradeShouldTrackLatestTradeCorrectlyOnMultipleUpdates(bool shouldRaiseEvent, decimal? firstLastQuantity, decimal? firstLastPrice, decimal? secondLastQuantity, decimal? secondLastPrice)
        {
            ...
            levelOneMarketData.UpdateLastTrade(lastTradeDateTime, firstLastQuantity, firstLastPrice);
            ...
            levelOneMarketData.UpdateLastTrade(lastTradeDateTime, secondLastQuantity, secondLastPrice);
        ...
        {
            var lastQuoteDateTime = new DateTime(2026, 06, 20);
            var levelOneMarketData = new LevelOneMarketData(_aapl);
            var eventRaised = default(bool);

            levelOneMarketData.BaseDataReceived += (_, e) => eventRaised = true;

            levelOneMarketData.UpdateQuote(lastQuoteDateTime, bidPrice, bidSize, askPrice, askSize);
            ...
            decimal? secondBidPrice, decimal? secondBidSize, decimal? secondAskPrice, decimal? secondAskSize)
        {
            ...
            levelOneMarketData.UpdateQuote(lastQuoteDateTime, firstBidPrice, firstBidSize, firstAskPrice, firstAskSize);
            Assert.IsTrue(eventRaised);

            // Reset event flag for second update
            eventRaised = false;

            levelOneMarketData.UpdateQuote(lastQuoteDateTime, secondBidPrice, secondBidSize, secondAskPrice, secondAskSize);
        ...
        public void UpdateQuoteShouldIgnoreZeroSizeUpdatesCorrectly(bool ignoreZeroSizeUpdates, decimal? bidPrice, decimal? bidSize, decimal? askPrice, decimal? askSize)
        {
            ...
            levelOneMarketData.UpdateQuote(lastQuoteDateTime, 1, expectedBestBidSize, 1, expectedBestAskSize);

...
class LeanOutputReader(object):
    ...
    def cumulative_return(self, name = "cumulative-return.png", width = 11.5, height = 2.5):
        ...
        plt.xlabel("")
        leg = ax.legend(["Strategy","Benchmark"],prop = {'weight':'bold'},frameon=False, loc = "upper left")
    ...
    def annual_returns(self, name = "annual-returns.png",width = 3.5*2, height = 2.5*2):
        ...
        fig = ax.get_figure()
        plt.xticks(rotation = 0,ha = 'center')
        plt.axvline(x = 0, color = 'grey', linewidth = 0.75)
        vline = plt.axvline(x = np.mean(df_this),color = "red", ls = "dashed", label = "mean", linewidth = 0.75)
        plt.legend([vline],["mean"],loc='upper right', frameon=False)
        plt.setp(ax.spines.values(), color='grey')
        plt.setp([ax.get_xticklines(), ax.get_yticklines()], color='grey')
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
    ...
    def monthly_return_distribution(self, name = "distribution-of-monthly-returns.png",width = 3.5*2, height = 2.5*2):
        ...
        fig = ax.get_figure()
        plt.xticks(rotation = 0,ha = 'center')
        plt.axvline(x = -tmp_min, color = 'grey', linewidth = 0.75)
        vline = plt.axvline(x = tmp_mean-tmp_min,color = "red", ls = "dashed", label = "mean", linewidth = 0.75)    
        plt.legend([vline],["mean"],loc='upper left', frameon=False)
        plt.setp(ax.spines.values(), color='grey')
        plt.setp([ax.get_xticklines(), ax.get_yticklines()], color='grey')
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)

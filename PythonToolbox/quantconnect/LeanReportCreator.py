...
import re
import math
from quantconnect.GenerateHTML import GenerateHTMLReport
...
class LeanReportCreator(object):
    ...
    def cumulative_return(self, name = "cumulative-return.png", width = 11.5, height = 2.5):
        if self.is_drawable:
            ...
            fig.savefig(self.outdir + "/" + name, dpi = 40, bbox_inches='tight')
            plt.cla()
            plt.clf()
            plt.close('all')
        return True
    ...
    def crisis_events(self, width = 3.5*2, height = 2.5*2):
        if self.is_drawable:
            ...
            for i in range(len(start_date)):    
                ...
                ax.grid()
                fig.set_size_inches(width, height) 
                fig.savefig(self.outdir + "/crisis-" +re.sub(r' ','-',titles[i].lower())+".png", dpi = 40, bbox_inches='tight')
                plt.cla()
                plt.clf()
                plt.close('all')
        return True
...
#            df_pie["Weight"] = [round(x/sum(df_pie["Value"])*100,1) for x in df_pie["Value"]]
            ...
            plt.axis('equal')
            fig.set_size_inches(width, height) 
            fig.savefig(self.outdir + "/asset-allocation-all.png", dpi = 40, bbox_inches='tight')
            plt.cla()
            plt.clf()
            plt.close('all')
...
#                    df_pie["Weight"] = [round(x/sum(df_pie["Value"])*100,1) for x in df_pie["Value"]]
                    ...
                    plt.axis('equal')                   
                    fig.set_size_inches(width, height) 
                    fig.savefig(self.outdir + "/asset-allocation-"+SecurityTypeName[SecurityType].lower()+".png", dpi = 40, bbox_inches='tight')
                    plt.cla()
                    plt.clf()
                    plt.close('all')                    
        return True
    ...
    def genearte_report(self):
        ...
        self.output_json()
        GenerateHTMLReport(self.outdir)

...
import re
import math
from quantconnect import Base64
...
class LeanReportCreator(object):
    ...
    def __init__(self, **kwargs):
        ...
        # Define the images resolution
        self.dpi = 200 if 'dpi' not in kwargs else kwargs['dpi']

        # Create output directory
        if 'outdir' not in kwargs:
            kwargs['outdir'] = kwargs['backtestId'] if 'backtestId' in kwargs else 'outdir'
        ...
        if "Strategy Equity" in data["Charts"]:
            series = data["Charts"]["Strategy Equity"]["Series"]["Equity"]["Values"]
            df_strategy = pd.DataFrame(series).set_index('x')
            df_strategy = df_strategy[df_strategy > 0]
            df_strategy = df_strategy[~df_strategy.index.duplicated(keep='first')]

        df_benchmark = pd.DataFrame()
        if "Benchmark" in data["Charts"]:
            series = data["Charts"]["Benchmark"]["Series"]["Benchmark"]["Values"] 
            df_benchmark = pd.DataFrame(series).set_index('x')
        ...
        self.startingCash = df["Strategy"][0]
        ...
        if "Orders" in data:
            ...
            df_orders = df_orders.transpose().set_index('Time').sort_index()
            df_orders.index = pd.to_datetime(df_orders.index)
            #df_orders['Symbol'] = [x['Symbol']['Value'] for x in assets.itertuples()]
            df = df.join(-df_orders["Value"].rename("Cash"), how = "outer")
            df['Cash'][0] = self.startingCash
            df['Cash'].fillna(0, inplace = True)
            df['Cash'] = np.cumsum(df['Cash'])
            df['Value'] = df["Strategy"] - df["Cash"]

        self.df = df
        self.orders = df_orders

        self.result = self.create_result_dict(data)
            ...
            #res = {"Key Characteristics": [("Diversified", Diversified),
            #                               ("Markets", Markets)]}
    ...
    def get_data(self, kwargs):
        ...
        if 'json' in kwargs:
            ...
            if 'projectId' not in kwargs:
                raise KeyError('projectId key not found')
            if 'backtestId' not in kwargs:
                raise KeyError('backtestId key not found')
            fname = f"{kwargs['backtestId']}.json"
        ...
        api = Api(userId = kwargs['userId'], token = kwargs['token'])
        data = api.read_backtest(projectId = kwargs['projectId'], backtestId = kwargs['backtestId'])
        if data['success']:
    ...
    def create_result_dict(self, data):

        output = {"Key Statistics":{"CAGR": "0%",
                                    "Drawdown": "0%",
                                    "Sharpe Ratio": "0",
                                    "Information Ratio": "0",
                                    "Trades Per Day": "0"},
            "Key Characteristics": {"Significant Period": False,
                                    "Significant Trading": False,
                                    "Diversified": False,
                                    "Risk Control": False,
                                    "Markets": []}}

        if "TotalPerformance" in data:
            data = data["TotalPerformance"]

            output["Key Characteristics"]["Significant Period"] = (self.df.index[-1] - self.df.index[0]).days/365 > 5
            output["Key Characteristics"]["Significant Trading"] = len(self.orders) >= 100
            output["Key Characteristics"]["Risk Control"] = data["PortfolioStatistics"]["Beta"] < 0.5

            CAGR = data["PortfolioStatistics"]["CompoundingAnnualReturn"] * 100
            output["Key Statistics"]["CAGR"] = f"{round(CAGR, 2)}%"

            Drawdown = data["PortfolioStatistics"]["Drawdown"] * 100
            output["Key Statistics"]["Drawdown"] = f"{round(Drawdown, 2)}%"

            SharpeRatio = data["PortfolioStatistics"]["SharpeRatio"]
            output["Key Statistics"]["Sharpe Ratio"] = f"{round(SharpeRatio, 3)}"

            InformationRatio = data["PortfolioStatistics"]["InformationRatio"]
            output["Key Statistics"]["Information Ratio"] = f"{round(InformationRatio, 3)}"

            TradesPerDay = len(self.orders) / max( (self.df.index[-1] - self.df.index[0]).days , 1)
            output["Key Statistics"]["Trades Per Day"] = f"{round(TradesPerDay, 6)}"

        return output


    def asset_allocation(self,width = 3.5*2, height = 2.5*2):
        if self.df.empty or self.orders.empty: return
        df = self.df[['Strategy']]
        assets = self.orders[['Symbol','Value','SecurityType']]

        start_date = df.index[0]
        end_date = df.index[-1]
        duration = (end_date-start_date).total_seconds()
        if start_date in assets.index:
        ...
        cash["Symbol"] = "CASH"
        cash["SecurityType"] = 0
        cash.loc[start_date] = ["CASH", self.startingCash, 0]

        assets = assets.append(cash).sort_index()



        #SecurityTypeName = {'Cash': (0,0), 'Equity': (1,0), 'Option': (2,0), 'Commodity': 0, 'Forex': 0, 'Future': 0, 'Cfd': 0, 'Crypto': 0}
        #for SecurityType in range(0, len(SecurityTypeName)):
        #    tmp = assets.where(assets["SecurityType"] == SecurityType)['Value']
        #    tmp = tmp.groupby(tmp.index).sum().cumsum()
        #    list_timestamp = list(df.index)
        #    list_timestamp.append(end_date)
        #    timeWeightedValue = sum([(list_timestamp[i+1] - list_timestamp[i]).total_seconds()/duration*tmp[i] for i in range(len(tmp))])
        ...
        if self.is_drawable:
            ...
            ddf_values["Value"] = [x["Value"] for x in self.orders.values()]
            ...
            if timeBegin in df_cash_tmp.index:
                df_cash_tmp.loc[timeBegin-timedelta(seconds = 1)] = ["CASH", self.startingCash, 0]
                timeBegin = timeBegin-timedelta(seconds = 1)
            else:
                df_cash_tmp.loc[timeBegin] = ["CASH", self.startingCash, 0]
...
#            df_pie["Weight"] = [round(x/sum(df_pie["Value"])*100,1) for x in df_pie["Value"]]
            ...
            fig.savefig(self.outdir + "/asset-allocation-all.png", dpi = self.dpi, bbox_inches='tight')
            plt.cla()
            plt.clf()
            plt.close('all')
...
#                    df_pie["Weight"] = [round(x/sum(df_pie["Value"])*100,1) for x in df_pie["Value"]]
                    ...
                    plt.axis('equal')                   
                    fig.set_size_inches(width, height) 
                    fig.savefig(self.outdir + "/asset-allocation-"+SecurityTypeName[SecurityType].lower()+".png", dpi = self.dpi, bbox_inches='tight')
                    plt.cla()
                    plt.clf()
                    plt.close('all')
        return True
    ...
    def cumulative_return(self, name = "cumulative-return.png", width = 11.5, height = 2.5):
        ...
        df = self.df[['Strategy','Benchmark']]
        df['Strategy'] = (df['Strategy']/self.startingCash-1)*100
        df['Benchmark'] = (df['Benchmark']/df['Benchmark'][0]-1)*100
        fname = f"{self.outdir}/{name}"
        ...
        try:
            ...
            plt.clf()
            plt.close('all')
            # If file is created, save
            self.output.update({'cumulative-return': Base64(fname)})
        except:
    ...
    def daily_returns(self, name = "daily-returns.png", width = 11.5, height = 2.5):
        ...
        if self.df.empty: return
        df = self.df[['Strategy']]
        df = df.groupby([df.index.date]).apply(lambda x: x.tail(1))
        df.index = df.index.droplevel(1)
        ret = np.array([self.startingCash] + df["Strategy"].tolist())
        df["Strategy"] = (ret[1:] / ret[:-1] - 1) * 100
        df.index = pd.to_datetime(df.index)
        if len(df) > 1:
            for i in range(len(df)-1):
                tmp_delta = df.index[i+1] - df.index[i]
                for j in range(1, tmp_delta.days):
                    df.loc[df.index[i] + timedelta(j)] = 0
                    df.loc[df.index[i] + timedelta(0.99)] = df.loc[df.index[i]]
                    df.loc[df.index[i+1] + timedelta(0.99)] = df.loc[df.index[i+1]]  
        ...
        try:
            plt.figure()
            ax = df.plot(color = "white",  alpha=0)
            ax.fill_between(df.index.values,0,df['Strategy'], where = 0<df['Strategy'], color = "#F5AE29",step = "pre")
            ax.fill_between(df.index.values,0,df['Strategy'], where = 0>df['Strategy'], color = "grey",step = "pre")
            fig = ax.get_figure()
            plt.xticks(rotation = 0,ha = 'center')
            plt.xlabel("")
            ...
            plt.close('all')
            # If file is created, save
            self.output.update({'daily-returns': Base64(fname)})
        except:
            print(f"Could not create {fname}")
    ...
    def drawdown(self,name = "drawdowns.png",width = 11.5, height = 2.5):
        if self.is_drawable:
            ...
            ax.grid()
            fig.set_size_inches(width, height)
            fig.savefig(self.outdir + "/" + name, dpi = self.dpi, bbox_inches='tight')
            plt.cla()
            plt.clf()
            plt.close('all')
        return True
    ...
    def crisis_events(self, width = 3.5*2, height = 2.5*2):
        if self.is_drawable:
            ...
            for i in range(len(start_date)):    
                ...
                ax.grid()
                fig.set_size_inches(width, height) 
                fig.savefig(self.outdir + "/crisis-" +re.sub(r' ','-',titles[i].lower())+".png", dpi = self.dpi, bbox_inches='tight')
                plt.cla()
                plt.clf()
                plt.close('all')
        return True
    ...
    def rolling_beta(self, name = "rolling-portfolio-beta-to-equity.png",width = 11.5, height = 2.5):
        if self.is_drawable:
            ...
            if len(set(self.df.index.date)) > days_L:
                ...
                ret_strategy = np.array([self.startingCash] + df_this["Strategy"].tolist())
                ret_strategy = ret_strategy[1:]/ret_strategy[:-1] - 1
                df_this["Strategy"] = ret_strategy*100
                ret_benchmark = np.array([self.startingCash] + df_this["Benchmark"].tolist())
    ...
    def genearte_report(self):
        ...
        #self.net_holdings()
        #self.leverage()
        GenerateHTMLReport(self.result)

if __name__ == "__main__":
    arguments = docopt(__doc__)
    report = Report(arguments)
    report.genearte_report()
    sys.exit(0)
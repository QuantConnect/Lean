    ...
    public class UpdateOrderRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private readonly CircularQueue<OrderType> _orderTypesQueue = new CircularQueue<OrderType>(
            Enum.GetValues(typeof(OrderType))
                .OfType<OrderType>()
                .Where(x =>
                    x != OrderType.OptionExercise
                    && x != OrderType.LimitIfTouched
                    && x != OrderType.ComboMarket
                    && x != OrderType.ComboLimit
                    && x != OrderType.ComboLegLimit
                )
        );
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (Time.Month != LastMonth)
            {
                ...
                var stopPrice = isLong
                    ? (1 + StopPercentage) * slice.Bars[symbol].High
                    : (1 - StopPercentage) * slice.Bars[symbol].Low;
                ...
                {
                    limitPrice = !isLong
                        ? (1 + LimitPercentage) * slice.Bars[symbol].High
                        : (1 - LimitPercentage) * slice.Bars[symbol].Low;
                }
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            ...
            if (order.Status == OrderStatus.Canceled && order.CanceledTime != orderEvent.UtcTime)
            {
                throw new RegressionTestException(
                    "Expected canceled order CanceledTime to equal canceled order event time."
                );
            }
            ...
            {
                throw new RegressionTestException(
                    "Expected filled order LastFillTime to equal fill order event time."
                );
            }

            // check the ticket to see if the update was successfully processed
            if (
                ticket.UpdateRequests.Any(ur => ur.Response?.IsSuccess == true)
                && order.CreatedTime != UtcTime
                && order.LastUpdateTime == null
            )
            {
                throw new RegressionTestException(
                    "Expected updated order LastUpdateTime to equal submitted update order event time"
                );
            }

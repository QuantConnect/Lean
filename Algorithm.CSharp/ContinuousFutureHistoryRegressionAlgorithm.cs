...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class ContinuousFutureHistoryRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice data)
        {
            ...
            {
                throw new TestException($"Unexpected mapped continuous contract future {_continuousContract.Mapped}");
            }
            ...
            if (IsWarmingUp)
            {
                ...
                {
                    throw new TestException($"ContinuousContract did not get any data during warmup!");
                }
                ...
                {
                    throw new TestException($"Unexpected current mapped contract expiration {backMonthExpiration}" +
                        $" @ {Time} it should be AFTER front month expiration {frontMonthExpiration}");
                }
            }
            if (data.Keys.Count != 1)
            {
                throw new TestException($"We are getting data for more than one symbols! {string.Join(",", data.Keys.Select(symbol => symbol))}");
            }
        ...
        public override void OnEndOfAlgorithm()
        {
            if (!_warmedUp)
            {
                throw new TestException("Algorithm didn't warm up!");
            }
        }

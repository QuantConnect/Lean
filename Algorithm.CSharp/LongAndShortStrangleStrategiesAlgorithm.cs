...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class LongAndShortStrangleStrategiesAlgorithm : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            ...
            {
                var callContracts = group
                    .Where(x => x.Right == OptionRight.Call)
                    .OrderByDescending(x => x.Strike)
                    .ToList();
                var putContracts = group
                    .Where(x => x.Right == OptionRight.Put)
                    .OrderBy(x => x.Strike)
                    .ToList();

                if (
                    callContracts.Count > 0
                    && putContracts.Count > 0
                    && callContracts[0].Strike > putContracts[0].Strike
                )
                {
            ...
            {
                _strangle = OptionStrategies.Strangle(
                    _optionSymbol,
                    callContract.Strike,
                    putContract.Strike,
                    callContract.Expiry
                );
                _shortStrangle = OptionStrategies.ShortStrangle(
                    _optionSymbol,
                    callContract.Strike,
                    putContract.Strike,
                    callContract.Expiry

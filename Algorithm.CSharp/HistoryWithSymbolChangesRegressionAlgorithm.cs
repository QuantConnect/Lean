...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class HistoryWithSymbolChangesRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            {
                throw new TestException($"History slices - expected: {expectedSliceCount}, actual: {history.Count}");
            }

            var totalBars = history.Count(slice => slice.Bars.Count > 0 && slice.Bars.ContainsKey(symbol));
            if (totalBars != expectedSliceCount)
            {
                throw new TestException($"History bars - expected: {expectedSliceCount}, actual: {totalBars}");
            }

            var firstBar = history.First().Bars.GetValue(symbol);
            if (firstBar.EndTime != new DateTime(1998, 3, 3) || firstBar.Close != 24.88039125m)
            {
                throw new TestException("First History bar - unexpected data received");
            }
        }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class DuplicatedIndexOptionSubscriptionRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        public override void Initialize()
        {
            ...
            {
                throw new TestException($"Expected a single subscription to exist ({spx})");
            }
            ...
            {
                throw new TestException("Expected subscriptions for the added index option contract");
            }

            if (SubscriptionManager.Subscriptions.Count(x => x.Symbol == spx) != 1)
            {
                throw new TestException("Expected a single subscription for the underlying index security");
            }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionStrategyFilteringUniverseBoxSpreadRegressionAlgorithm
        : OptionStrategyFilteringUniverseBaseAlgorithm
    {
        ...
        protected override void TestFiltering(OptionChain chain)
        {
            ...
            var otmCall = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Call && x.Strike == higherStrike && x.Expiry == expiry
            );
            var itmCall = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Call && x.Strike == lowerStrike && x.Expiry == expiry
            );
            var itmPut = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Put && x.Strike == higherStrike && x.Expiry == expiry
            );
            var otmPut = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Put && x.Strike == lowerStrike && x.Expiry == expiry
            );

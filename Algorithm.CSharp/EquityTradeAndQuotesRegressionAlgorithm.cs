...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class EquityTradeAndQuotesRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            {
                throw new TestException($"History not all slices have trades and quotes.");
            }
        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            foreach (var addedSecurity in changes.AddedSecurities)
            {
                ...
                if (addedSecurity.Symbol == _symbol)
                {
                    ...
                    {
                        throw new TestException($"Subscriptions were not correctly added for high resolution.");
                    }
                }
                else
                {
                    ...
                    {
                        throw new TestException($"Subscriptions were not correctly added for low resolution.");
                    }
                }
            }
        }
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled)
            {
                ...
                {
                    throw new TestException($"Fill price is not the expected for OrderId {orderEvent.OrderId} at Algorithm Time {Time:s}." +
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException($"Fail at trade bars count expected >= 1955, actual: {_tradeCounter}.");
            }
            // We expect 390 * 5 = 1950 quote bars. 
            if (_quoteCounter != 1950)
            {
                throw new TestException($"Fail at trade bars count expected: 1950, actual: {_quoteCounter}.");
            }

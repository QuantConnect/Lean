...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class PortfolioRebalanceOnCustomFuncRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            SetPortfolioConstruction(
                new EqualWeightingPortfolioConstructionModel(time =>
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        foreach (
                            var security in Securities.Values.Where(security => security.Invested)
                        )
                        {
                            var reservedBuyingPowerForCurrentPosition =
                                security
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            ...
            {
                if (
                    UtcTime - _lastRebalanceTime > TimeSpan.Zero
                    || UtcTime.DayOfWeek != DayOfWeek.Monday
                )
                {
                    throw new RegressionTestException(
                        $"{UtcTime} {orderEvent.Symbol} {UtcTime - _lastRebalanceTime}"
                    );
                }
            }
        }

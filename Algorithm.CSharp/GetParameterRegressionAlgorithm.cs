...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class GetParameterRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private void CheckParameter<T, P>(T expected, P actual, string call)
        {
            ...
            {
                throw new TestException($"{call} should have returned null but returned {actual} ({actual.GetType()})");
            }

            if (expected != null && actual == null)
            {
                throw new TestException($"{call} should have returned {expected} ({expected.GetType()}) but returned null");
            }

            if (expected != null && actual != null && (expected.GetType() != actual.GetType() || !expected.Equals(actual)))
            {
                throw new TestException($"{call} should have returned {expected} ({expected.GetType()}) but returned {actual} ({actual.GetType()})");
            }
        }

    ...
    public abstract class OptionsMarginCallEventsAlgorithmBase : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled && !_firstOrderEventReceived)
            {
                ...
                {
                    throw new TestException($"Expected order quantity to be {OriginalQuantity} but was {orderEvent.Quantity}");
                }
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException($"OnMarginCall was called {_onMarginCallCount} times, expected 1");
            }

            if (_onMarginCallWarningCount == 0)
            {
                throw new TestException("OnMarginCallWarning was not called");
            }

            var orders = Transactions.GetOrders().ToList();
            if (orders.Count != ExpectedOrdersCount)
            {
                throw new TestException($"Expected {ExpectedOrdersCount} orders, found {orders.Count}");
            }

            if (orders.Any(order => !order.Status.IsFill()))
            {
                throw new TestException("All orders should be filled");
            }
            ...
            {
                throw new TestException($@"Strategy position group quantity should have been decreased from the original quantity {OriginalQuantity

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class ConsolidatorAnIdentityIndicatorRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        private readonly Dictionary<DateTime, decimal> _expectedValues = new Dictionary<
            DateTime,
            decimal
        >
        {
                ...
                (TradeBar bar) =>
                {
                    ...
                    if (_expectedValues[Time] != bar.Value)
                    {
                        throw new RegressionTestException(
                            $"{Time} - Consolidate unexpected current value: {bar.Value}"
                        );
                    }
                }
            );
        ...
        private void _identity_Updated(object sender, IndicatorDataPoint updated)
        {
            ...
            {
                throw new RegressionTestException(
                    $"{Time} - _identity_Updated unexpected current value: {_identity.Current.Value}"
                );
            }
        }
        ...
        private void Min_Updated(object sender, IndicatorDataPoint updated)
        {
            ...
            {
                throw new RegressionTestException(
                    $"{Time} - Min_Updated unexpected current value: {_identity.Current.Value}"
                );
            }
        }

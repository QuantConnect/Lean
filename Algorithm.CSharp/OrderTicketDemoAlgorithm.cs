...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class OrderTicketDemoAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            ...
            {
                throw new TestException("OrderEvent quantity is Not expected to be 0, it should hold the current order Quantity");
            }
            if (orderEvent.Quantity != order.Quantity)
            {
                throw new TestException("OrderEvent quantity should hold the current order Quantity");
            }
            if (order is LimitOrder && orderEvent.LimitPrice == 0
                || order is StopLimitOrder && orderEvent.LimitPrice == 0)
            {
                throw new TestException("OrderEvent LimitPrice is Not expected to be 0 for LimitOrder and StopLimitOrder");
            }
            if (order is StopMarketOrder && orderEvent.StopPrice == 0)
            {
                throw new TestException("OrderEvent StopPrice is Not expected to be 0 for StopMarketOrder");
            }

            // We can access the order ticket from the order event
            if (orderEvent.Ticket == null)
            {
                throw new TestException("OrderEvent Ticket was not set");
            }
            if (orderEvent.OrderId != orderEvent.Ticket.OrderId)
            {
                throw new TestException("OrderEvent.OrderId and orderEvent.Ticket.OrderId do not match");
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException($"There were expected 9 filled orders and 12 order tickets");
            }
            if (openOrders.Count != 0 || openOrderTickets.Any())
            {
                throw new TestException($"No open orders or tickets were expected");
            }
            if (remainingOpenOrders != 0m)
            {
                throw new TestException($"No remaining quantity to be filled from open orders was expected");
            }
            ...
            {
                throw new TestException($"There were expected 12 orders and 12 order tickets");
            }

    ...
    public class UnsettledCashWhenQuoteCurrencyIsNotAccountCurrencyAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled && orderEvent.Direction == OrderDirection.Sell)
            {
                ...
                {
                    throw new TestException($"Unsettled cash entry for {orderEvent.FillPriceCurrency} not found");
                }
                ...
                {
                    throw new TestException($"Expected unsettled cash to be {expectedUnsettledCash} but was {actualUnsettledCash}");
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            foreach (var kvp in Portfolio.CashBook)
            {
                ...
                {
                    throw new TestException($@"Unsettled cash conversion rate for {symbol} is {unsettledCash.ConversionRate} but should be {cash.ConversionRate}");
                }
                ...
                if (unsettledCash.Symbol == accountCurrency)
                {
                    ...
                    {
                        throw new TestException($@"Conversion rate for {unsettledCash.Symbol} (the account currency) in the UnsettledCashBook should be 1 but was {unsettledCash.ConversionRate}.");
                    }
                    ...
                    {
                        throw new TestException($@"Currency conversion for {unsettledCash.Symbol} (the account currency) in the UnsettledCashBook should be an identity conversion of type {nameof(ConstantCurrencyConversion)}");
                    }
                }
                else
                {
                    ...
                    {
                        throw new TestException($@"Currency conversion for {unsettledCash.Symbol} in the UnsettledCashBook should be of type {nameof(SecurityCurrencyConversion)}");
                    }
                    ...
                    {
                        throw new TestException($@"Currency conversion for {unsettledCash.Symbol} in UnsettledCashBook is not correct. Source and destination currency should have been {accountCurrency} and {unsettledCash.Symbol} or vice versa but were {sourceCurrency} and {destinationCurrency}.");
                    }
                }
            }
        }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class UnsettledCashWhenQuoteCurrencyIsNotAccountCurrencyAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            ...
            )
            {
                ...
                if (
                    !Portfolio.UnsettledCashBook.TryGetValue(
                        orderEvent.FillPriceCurrency,
                        out var unsettledCash
                    )
                )
                {
                    throw new RegressionTestException(
                        $"Unsettled cash entry for {orderEvent.FillPriceCurrency} not found"
                    );
                }

                var expectedUnsettledCash = Math.Abs(
                ...
                {
                    throw new RegressionTestException(
                        $"Expected unsettled cash to be {expectedUnsettledCash} but was {actualUnsettledCash}"
                    );
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            foreach (var kvp in Portfolio.CashBook)
            {
                ...
                {
                    throw new RegressionTestException(
                        $@"Unsettled cash conversion rate for {symbol} is {unsettledCash.ConversionRate} but should be {cash.ConversionRate}"
                    );
                }
                ...
                if (unsettledCash.Symbol == accountCurrency)
                {
                    ...
                    {
                        throw new RegressionTestException(
                            $@"Conversion rate for {unsettledCash.Symbol} (the account currency) in the UnsettledCashBook should be 1 but was {unsettledCash.ConversionRate}."
                        );
                    }

                    if (
                        unsettledCash.CurrencyConversion.GetType()
                            != typeof(ConstantCurrencyConversion)
                    ...
                    {
                        throw new RegressionTestException(
                            $@"Currency conversion for {unsettledCash.Symbol} (the account currency) in the UnsettledCashBook should be an identity conversion of type {nameof(ConstantCurrencyConversion)}"
                        );
                    }
                }
                else
                {
                    if (
                        unsettledCash.CurrencyConversion.GetType()
                        != typeof(SecurityCurrencyConversion)
                    )
                    {
                        throw new RegressionTestException(
                            $@"Currency conversion for {unsettledCash.Symbol} in the UnsettledCashBook should be of type {nameof(SecurityCurrencyConversion)}"
                        );
                    }
                    ...
                    if (
                        !(
                            (
                                ...
                                && destinationCurrency == unsettledCash.Symbol
                            )
                            || (
                                sourceCurrency == unsettledCash.Symbol
                    ...
                    {
                        throw new RegressionTestException(
                            $@"Currency conversion for {unsettledCash.Symbol} in UnsettledCashBook is not correct. Source and destination currency should have been {accountCurrency} and {unsettledCash.Symbol} or vice versa but were {sourceCurrency} and {destinationCurrency}."
                        );
                    }
                }
            }

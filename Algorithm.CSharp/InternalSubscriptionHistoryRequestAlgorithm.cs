...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class InternalSubscriptionHistoryRequestAlgorithm
        : QCAlgorithm,
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                if (
                    !history.Any()
                    || !history.All(slice =>
                        slice.Bars.All(pair => pair.Value.Period == TimeSpan.FromHours(1))
                    )
                )
                {
                    throw new RegressionTestException(
                        "Unexpected history result for internal subscription"
                    );
                }
                ...
                if (
                    !history.Any()
                    || !history.All(slice =>
                        slice.Bars.All(pair => pair.Value.Period == TimeSpan.FromDays(1))
                    )
                )
                {
                    throw new RegressionTestException(
                        "Unexpected history result for user subscription"
                    );
                }
            }
        }

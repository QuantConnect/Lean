...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityBullPutLadderRegressionAlgorithm
        : OptionEquityBaseStrategyRegressionAlgorithm
    {
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                {
                    var putContracts = chain.Where(contract => contract.Right == OptionRight.Put);
                    ...
                    var midStrikePut = putContracts.First(contract =>
                        contract.Strike > lowStrikePut.Strike
                        && contract.Expiry == lowStrikePut.Expiry
                    );
                    var highStrikePut = putContracts.First(contract =>
                        contract.Strike > midStrikePut.Strike
                        && contract.Expiry == lowStrikePut.Expiry
                    );
                    ...
                    var expectedMarginUsage =
                        (highStrikePut.Strike - midStrikePut.Strike)
                        * Securities[highStrikePut.Symbol].SymbolProperties.ContractMultiplier
                        * 5;

    ...
    public class SetHoldingsFutureRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
                ...
                foreach (var chain in slice.FutureChains)
                {
                    ...
                    if (contract != null)
                    {
                        ...
                        try
                        {
                            ...
                            throw new TestException("We expect invalid target for futures to throw an exception");
                        }
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled && Portfolio.Invested)
            {
                ...
                {
                    throw new TestException($"We expect TotalMarginUsed to be {marginUsed}, but was {Portfolio.TotalMarginUsed}");
                }
                ...
                {
                    throw new TestException("We expect to be trading using the biggest position we can, there seems to be room for another contract");
                }
            }
        }

    ...
    public class SetHoldingsFutureRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled && Portfolio.Invested)
            {
                ...
                if (Portfolio.TotalAbsoluteHoldingsCost / Portfolio.TotalPortfolioValue < 10)
                {
                    throw new Exception("Expected to be trading using the futures margin leverage");
                }

                var model = Securities[_contractSymbol].BuyingPowerModel as FutureMarginModel;
                var marginUsed = model.MaintenanceMarginRequirement * Securities[_contractSymbol].Holdings.AbsoluteQuantity;
                ...
                var initialMarginRequired = model.InitialMarginRequirement * Securities[_contractSymbol].Holdings.AbsoluteQuantity;

                if (Portfolio.TotalPortfolioValue - initialMarginRequired > model.InitialMarginRequirement * Securities[_contractSymbol].SymbolProperties.LotSize)
                {

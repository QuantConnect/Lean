...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class LongAndShortPutCalendarSpreadStrategiesAlgorithm
        : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            ...
            foreach (var group in contractsByStrike)
            {
                ...
                _putCalendarSpread = OptionStrategies.PutCalendarSpread(
                    _optionSymbol,
                    strike,
                    nearExpiration,
                    farExpiration
                );
                _shortPutCalendarSpread = OptionStrategies.ShortPutCalendarSpread(
                    _optionSymbol,
                    strike,
                    nearExpiration,
                    farExpiration
        ...
        protected override void AssertStrategyPositionGroup(IPositionGroup positionGroup)
        {
            ...
            var nearExpirationPosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Put && x.Symbol.ID.Date == nearExpiration
            );
            ...
            var farExpirationPosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Put && x.Symbol.ID.Date == farExpiration
            );

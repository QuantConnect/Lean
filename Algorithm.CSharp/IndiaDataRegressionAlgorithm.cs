...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    {
        private Symbol _mappingSymbol,
            _splitAndDividendSymbol;
        ...
        public override void OnDividends(Dividends dividends)
        {
            if (dividends.ContainsKey(_splitAndDividendSymbol))
            {
                ...
                if (
                    Time.Date == new DateTime(2010, 06, 15)
                    && (
                        dividend.Price != 0.5m
                        || dividend.ReferencePrice != 88.8m
                        || dividend.Distribution != 0.5m
                    )
                )
                {
        ...
        public override void OnSplits(Splits splits)
        {
            if (splits.ContainsKey(_splitAndDividendSymbol))
            {
                ...
                else if (split.Type == SplitType.SplitOccurred)
                {
                    ...
                    if (
                        split.Price != 421m
                        || split.ReferencePrice != 421m
                        || split.SplitFactor != 0.2m
                    )
                    {
        ...
        public override void OnSymbolChangedEvents(SymbolChangedEvents symbolsChanged)
        {
            ...
            {
                var mappingEvent = symbolsChanged
                    .Single(x => x.Key.SecurityType == SecurityType.Equity)
                    .Value;
                Log(
                ...
                {
                    if (mappingEvent.NewSymbol == "3MINDIA" && mappingEvent.OldSymbol == "BIRLA3M")
                    {

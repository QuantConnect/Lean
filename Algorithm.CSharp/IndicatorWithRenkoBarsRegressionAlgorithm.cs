...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class IndicatorWithRenkoBarsRegressionAlgorithm
        : QCAlgorithm,
            ...
            foreach (var indicator in _indicators)
            {
                if (!indicator.IsReady)
                {
                    throw new RegressionTestException(
                        $"{indicator.Name} indicator should be ready"
                    );
                }
                else if (indicator.Current.Value == 0)
                {
                    throw new RegressionTestException(
                        $"The current value of {indicator.Name} indicator should be different than zero"
                    );
                }
            }
        }

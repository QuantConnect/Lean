...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class ScheduledEventsOrderRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                Schedule.On(dateRule, TimeRules.At(9, 25), (name, time) =>
                {
                    ...
                    if (aEventCount != id)
                    {
                        throw new TestException($"Scheduled event triggered out of order: {Time} expected id {id} but was {aEventCount}");
                    }
                    aEventCount++;
                    // goes from 0 to 1
                    aEventCount %= 2;
                ...
                Schedule.On(dateRule, TimeRules.BeforeMarketClose(_spy, 5), (name, time) =>
                {
                    ...
                    if (bEventCount != id)
                    {
                        throw new TestException($"Scheduled event triggered out of order: {Time} expected id {id} but was {bEventCount}");
                    }
                    bEventCount++;
                    // goes from 0 to 1
                    bEventCount %= 2;
                ...
                Schedule.On(dateRule, TimeRules.At(16, 5), (name, time) =>
                {
                    ...
                    if (cEventCount != id)
                    {
                        throw new TestException($"Scheduled event triggered out of order: {Time} expected id {id} but was {cEventCount}");
                    }
                    cEventCount++;
                    // goes from 0 to 1
                    cEventCount %= 2;
        ...
        private void AssertScheduledEventTime()
        {
            if (_lastTime > Time)
            {
                throw new TestException($"Scheduled event time shouldn't go backwards, last time {_lastTime}, current {Time}");
            }
            _lastTime = Time;
        ...
        private void AfterMarketOpen()
        {
            ...
            {
                throw new TestException($"AfterMarketOpen unexpected event time: {Time}");
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException($"OnEndOfAlgorithm expected scheduled events but was {_scheduledEventCount}");
            }
            if (_afterMarketOpenEventCount != 1)
            {
                throw new TestException($"OnEndOfAlgorithm expected after MarketOpenEvent count {_afterMarketOpenEventCount}");
            }
        }

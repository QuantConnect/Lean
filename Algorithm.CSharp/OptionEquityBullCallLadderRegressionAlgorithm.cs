...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityBullCallLadderRegressionAlgorithm
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                )
                {
                    ...
                    var midStrikeCall = callContracts.First(contract =>
                        contract.Strike > lowStrikeCall.Strike
                        && contract.Expiry == lowStrikeCall.Expiry
                    );
                    var highStrikeCall = callContracts.First(contract =>
                        contract.Strike > midStrikeCall.Strike
                        && contract.Expiry == lowStrikeCall.Expiry
                    );
                    ...
                    var priceLadderDifference = GetPriceSpreadDifference(
                        lowStrikeCall.Symbol,
                        midStrikeCall.Symbol,
                        highStrikeCall.Symbol
                    );
                    if (
                        initialMargin
                        != (

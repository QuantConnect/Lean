...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class LongAndShortCallCalendarSpreadStrategiesAlgorithm
        : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            ...
            foreach (var group in contractsByStrike)
            {
                ...
                _callCalendarSpread = OptionStrategies.CallCalendarSpread(
                    _optionSymbol,
                    strike,
                    nearExpiration,
                    farExpiration
                );
                _shortCallCalendarSpread = OptionStrategies.ShortCallCalendarSpread(
                    _optionSymbol,
                    strike,
                    nearExpiration,
                    farExpiration
        ...
        protected override void AssertStrategyPositionGroup(IPositionGroup positionGroup)
        {
            if (positionGroup.Positions.Count() != 2)
            {
                throw new RegressionTestException(
                    $"Expected position group to have 2 positions. Actual: {positionGroup.Positions.Count()}"
                );
            }

            var nearExpiration = _callCalendarSpread.OptionLegs.Min(leg => leg.Expiration);
            var nearExpirationPosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Call && x.Symbol.ID.Date == nearExpiration
            );

            if (nearExpirationPosition.Quantity != -2)
            {
                throw new RegressionTestException(
                    $"Expected near expiration position quantity to be -2. Actual: {nearExpirationPosition.Quantity}"
                );
            }

            var farExpiration = _callCalendarSpread.OptionLegs.Max(leg => leg.Expiration);
            var farExpirationPosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Call && x.Symbol.ID.Date == farExpiration
            );

            if (farExpirationPosition.Quantity != 2)
            {
                throw new RegressionTestException(
                    $"Expected far expiration position quantity to be 2. Actual: {farExpirationPosition.Quantity}"
                );
            }
        }

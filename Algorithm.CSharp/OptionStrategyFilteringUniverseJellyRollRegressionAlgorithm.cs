...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionStrategyFilteringUniverseJellyRollRegressionAlgorithm
        : OptionStrategyFilteringUniverseBaseAlgorithm
    {
        ...
        protected override void TestFiltering(OptionChain chain)
        {
            ...
            var nearCallExpiryContract = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Call && x.Strike == strike && x.Expiry == nearExpiry
            );
            var farCallExpiryContract = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Call && x.Strike == strike && x.Expiry == farExpiry
            );
            var nearPutExpiryContract = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Put && x.Strike == strike && x.Expiry == nearExpiry
            );
            var farPutExpiryContract = chain.SingleOrDefault(x =>
                x.Right == OptionRight.Put && x.Strike == strike && x.Expiry == farExpiry
            );
            if (
                nearCallExpiryContract == null
                || farCallExpiryContract == null
                || nearPutExpiryContract == null
                || farPutExpiryContract == null
            )
            {

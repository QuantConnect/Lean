                ...
                foreach (var chain in slice.OptionChains)
                {
                    foreach (var contract in chain.Value)
                    {
                            ...
                            contract.Symbol.ID.Date == new DateTime(2016, 01, 15))
                        {
                            ...
                            if (history.Count == 0)
                            {
                                throw new TestException("Regression test failed: open interest history request is empty");
                            }
                            ...
                            {
                                throw new TestException("Regression test failed: current open interest isn't in the security cache");
                            }

                            if (slice.Time.Date == new DateTime(2014, 06, 05) && (contract.OpenInterest != 50 || security.OpenInterest != 50))
                            {
                                throw new TestException("Regression test failed: current open interest was not correctly loaded and is not equal to 50");
                            }
                            if (slice.Time.Date == new DateTime(2014, 06, 06) && (contract.OpenInterest != 70 || security.OpenInterest != 70))
                            {
                                throw new TestException("Regression test failed: current open interest was not correctly loaded and is not equal to 70");
                            }

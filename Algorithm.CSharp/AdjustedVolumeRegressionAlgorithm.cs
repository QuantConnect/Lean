...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class AdjustedVolumeRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private readonly IEnumerator<decimal> _expectedAdjustedVolume = new List<decimal>
        {
            6164842,
            3044047,
            3680347,
        ...
        private readonly IEnumerator<decimal> _expectedAdjustedAskSize = new List<decimal>
        {
            215600,
            5600,
            25200,
        ...
        private readonly IEnumerator<decimal> _expectedAdjustedBidSize = new List<decimal>
        {
            2800,
            11200,
            2800,
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (slice.Bars.ContainsKey(_aapl))
            {
                ...
                if (
                    ...
                    && _expectedAdjustedVolume.Current != aaplData.Volume
                )
                {
                    // Our values don't match lets try and give a reason why
                    var dayFactor = _factorFile.GetPriceScale(
                        aaplData.Time,
                    ...
                    if (_expectedAdjustedVolume.Current == probableAdjustedVolume)
                    {
                        throw new ArgumentException(
                            $"Volume was incorrect; but manually adjusted value is correct."
                    ...
                    {
                        throw new ArgumentException(
                            $"Volume was incorrect; even when adjusted manually by"
            ...
            if (slice.QuoteBars.ContainsKey(_aapl))
            {
                ...
                if (
                    _expectedAdjustedAskSize.MoveNext()
                    && _expectedAdjustedAskSize.Current != aaplQuoteData.LastAskSize
                )
                {
                    // Our values don't match lets try and give a reason why
                    var dayFactor = _factorFile.GetPriceScale(
                    ...
                    {
                        throw new ArgumentException(
                            $"Ask size was incorrect; but manually adjusted value is correct."
                    ...
                    {
                        throw new ArgumentException(
                            $"Ask size was incorrect; even when adjusted manually by"
                ...
                if (
                    _expectedAdjustedBidSize.MoveNext()
                    && _expectedAdjustedBidSize.Current != aaplQuoteData.LastBidSize
                )
                {
                    // Our values don't match lets try and give a reason why
                    var dayFactor = _factorFile.GetPriceScale(
                    ...
                    {
                        throw new ArgumentException(
                            $"Bid size was incorrect; but manually adjusted value is correct."
                                + $" Adjustment by multiplying size by {1 / dayFactor} is not occurring."
                        );
                    }
                    else
                    {
                        throw new ArgumentException(
                            $"Bid size was incorrect; even when adjusted manually by"
                                + $" multiplying size by {1 / dayFactor}. Data may have changed."
                        );
                    }
                }

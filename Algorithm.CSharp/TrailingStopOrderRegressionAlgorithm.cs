    ...
    public class TrailingStopOrderRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            else if (_buyOrderTicket.Status != OrderStatus.Filled)
            {
                ...
                {
                    throw new TestException($"StopPrice {stopPrice} should be within {BuyTrailingAmount} of the previous low price {low} at all times.");
                }
            }
            ...
            else if (_sellOrderTicket.Status != OrderStatus.Filled)
            {
                ...
                {
                    throw new TestException($"StopPrice {stopPrice} should be within {SellTrailingAmount} of the previous high price {high} at all times.");
                }
            }
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled)
            {
                if (orderEvent.Direction == OrderDirection.Buy)
                {
                    ...
                    {
                        throw new TestException($@"Buy trailing stop order should have filled with price greater than or equal to the stop price {
                ...
                else
                {
                    ...
                    if (orderEvent.FillPrice > stopPrice)
                    {
                        throw new TestException($@"Sell trailing stop order should have filled with price less than or equal to the stop price {
                            stopPrice}. Fill price: {orderEvent.FillPrice}");
                    }
                }
            }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class LongAndShortStraddleStrategiesAlgorithm : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            var contracts = chain
                ...
                .FirstOrDefault(group =>
                    group.Any(x => x.Right == OptionRight.Call)
                    && group.Any(x => x.Right == OptionRight.Put)
                );
            ...
            if (contracts != null)
            {
                ...
                _straddle = OptionStrategies.Straddle(
                    _optionSymbol,
                    contract.Strike,
                    contract.Expiry
                );
                _shortStraddle = OptionStrategies.ShortStraddle(
                    _optionSymbol,
                    contract.Strike,
                    contract.Expiry
                );
        ...
        protected override void AssertStrategyPositionGroup(IPositionGroup positionGroup)
        {
            ...
            var callPosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Call
            );
            var putPosition = positionGroup.Positions.Single(x =>
            ...
            {
                throw new RegressionTestException(
                    $@"Expected call position quantity to be {expectedCallPositionQuantity}. Actual: {callPosition.Quantity}"
                );
            }
            ...
            {
                throw new RegressionTestException(
                    $@"Expected put position quantity to be {expectedPutPositionQuantity}. Actual: {putPosition.Quantity}"
                );
            }
        }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    {
        private readonly Symbol _appl = QuantConnect.Symbol.Create(
            "AAPL",
            SecurityType.Equity,
            Market.USA
        );
        ...
        private readonly Symbol _qqq = QuantConnect.Symbol.Create(
            "QQQ",
            SecurityType.Equity,
            Market.USA
        );
        private readonly Symbol _fb = QuantConnect.Symbol.Create(
            "FB",
            SecurityType.Equity,
            Market.USA
        );
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (_step == 1)
            {
                if (!slice.ContainsKey(_qqq) || !slice.ContainsKey(_appl))
                {
            ...
            {
                if (!slice.ContainsKey(_fb) || !slice.ContainsKey(_spy))
                {
                    throw new RegressionTestException($"Unexpected symbols found, step: {_step}");
                }
                if (slice.Count != 2)
        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            ...
            foreach (var removed in changes.RemovedSecurities)
            {
                ...
                if (
                    removed.Symbol == _appl && _step != 1 && _step != 2

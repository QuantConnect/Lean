...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class ContinuousFutureImmediateUniverseSelectionRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            _es.SetFilter(universe =>
            {
                if (_esSelectionTimeUtc == DateTime.MinValue)
                {
                    ...
                    {
                        throw new TestException($"Expected ES universe selection to happen on algorithm start ({_startDateUtc}), " +
            ...
            _milk.SetFilter(universe =>
            {
                if (_milkSelectionTimeUtc == DateTime.MinValue)
                {
                    ...
                    {
                        throw new TestException($"Expected DC universe selection to happen on algorithm start ({_startDateUtc}), " +
        ...
        public override void OnData(Slice data)
        {
            ...
            if (_es.Mapped == null)
            {
                throw new TestException("ES mapped contract is null");
            }
            ...
            {
                throw new TestException("DC mapped contract is null");
            }
        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            if (!_securitiesChangedEventReceived)
            {
                ...
                if (Time != StartDate)
                {
                    throw new TestException($"Expected OnSecuritiesChanged to be called on algorithm start ({StartDate}), " +
                        $"but happened on {Time}");
                }

                if (_esSelectionTimeUtc == DateTime.MinValue)
                {
                    throw new TestException("ES universe selection time was not set");
                }

                if (_milkSelectionTimeUtc == DateTime.MinValue)
                {
                    throw new TestException("DC universe selection time was not set");
                }

                if (changes.AddedSecurities.Count == 0 || changes.RemovedSecurities.Count != 0)
                {
                    throw new TestException($"Unexpected securities changes. Expected multiple securities added and none removed " +
                ...
                {
                    throw new TestException($"Expected to find a multiple futures for ES");
                }

                if (!changes.AddedSecurities.Any(x => !x.Symbol.IsCanonical() && x.Symbol.Canonical == _milk.Symbol))
                {
                    throw new TestException($"Expected to find a multiple futures for DC");
                }
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            if (!_dataReceived)
            {
                throw new TestException("No data was received so no checks were done");
            }

            if (!_securitiesChangedEventReceived)
            {
                throw new TestException("OnSecuritiesChanged was not called");
            }
        }

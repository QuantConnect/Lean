...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class HistoryAuxiliaryDataRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            {
                throw new TestException($"Unexpected multi symbol dividend count: {multiSymbolRequest.Count}");
            }
            ...
            {
                throw new TestException($"Unexpected continuous future mapping event count: {continuousFutureOpenInterestMapping.Count}");
            }
            ...
            {
                throw new TestException($"Unexpected continuous future mapping event count: {continuousFutureLastTradingDayMapping.Count}");
            }
            ...
            {
                throw new TestException($"Unexpected continuous future mapping times");
            }

            var dividends = History<Dividend>(aapl, 360).ToList();
            if (dividends.Count != 6)
            {
                throw new TestException($"Unexpected dividend count: {dividends.Count}");
            }
            foreach (var dividend in dividends)
            {
                if (dividend.Distribution == 0)
                {
                    throw new TestException($"Unexpected Distribution: {dividend.Distribution}");
                }
            }
            ...
            if (splits.Count != 2)
            {
                throw new TestException($"Unexpected split count: {splits.Count}");
            }
            foreach (var split in splits)
            {
                if (split.SplitFactor == 0)
                {
                    throw new TestException($"Unexpected SplitFactor: {split.SplitFactor}");
                }
            }
            ...
            {
                throw new TestException($"Unexpected margin interest count: {marginInterests.Count}");
            }
            foreach (var marginInterest in marginInterests)
            {
                if (marginInterest.InterestRate == 0)
                {
                    throw new TestException($"Unexpected InterestRate: {marginInterest.InterestRate}");
                }
            }
            ...
            {
                throw new TestException($"Unexpected delistings count: {delistings.Count}");
            }
            if (delistings[0].Type != DelistingType.Warning)
            {
                throw new TestException($"Unexpected delisting: {delistings[0]}");
            }
            if (delistings[1].Type != DelistingType.Delisted)
            {
                throw new TestException($"Unexpected delisting: {delistings[1]}");
            }
            ...
            {
                throw new TestException($"Unexpected SymbolChangedEvents count: {symbolChangedEvents.Count}");
            }
            ...
            {
                throw new TestException($"Unexpected SymbolChangedEvents: {symbolChangedEvents[0]}");
            }
            ...
            {
                throw new TestException($"Unexpected SymbolChangedEvents: {symbolChangedEvents[1]}");
            }
        }

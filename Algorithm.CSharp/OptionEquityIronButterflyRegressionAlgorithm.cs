...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityIronButterflyRegressionAlgorithm
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                )
                {
                    ...
                    var atTheMoneyCall = contracts.Single(contract =>
                        contract.Right == OptionRight.Call
                        && contract.Expiry == outOfTheMoneyPut.Expiry
                        && contract.Strike == atTheMoneyPut.Strike
                    );

                    var outOfTheMoneyCall = contracts.Single(contract =>
                        contract.Right == OptionRight.Call
                        && contract.Expiry == outOfTheMoneyPut.Expiry
                    ...
                    var expectedMarginUsage =
                        (atTheMoneyPut.Strike - outOfTheMoneyPut.Strike)
                        * Securities[atTheMoneyPut.Symbol].SymbolProperties.ContractMultiplier
                        * 10;
                    ...
                    var priceSpreadDifference = GetPriceSpreadDifference(
                        outOfTheMoneyPut.Symbol,
                        atTheMoneyPut.Symbol,
                        atTheMoneyCall.Symbol,
                        outOfTheMoneyCall.Symbol
                    );
                    if (
                        initialMargin
                        != (

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class AllShortableSymbolsCoarseSelectionRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        private static readonly Symbol _aapl = QuantConnect.Symbol.Create(
            "AAPL",
            SecurityType.Equity,
            Market.USA
        );
        private static readonly Symbol _bac = QuantConnect.Symbol.Create(
            "BAC",
            SecurityType.Equity,
            Market.USA
        );
        private static readonly Symbol _gme = QuantConnect.Symbol.Create(
            "GME",
            SecurityType.Equity,
            Market.USA
        );
        private static readonly Symbol _goog = QuantConnect.Symbol.Create(
            "GOOG",
            SecurityType.Equity,
            Market.USA
        );
        private static readonly Symbol _qqq = QuantConnect.Symbol.Create(
            "QQQ",
            SecurityType.Equity,
            Market.USA
        );
        private static readonly Symbol _spy = QuantConnect.Symbol.Create(
            "SPY",
            SecurityType.Equity,
            Market.USA
        );
        ...
        private static readonly Dictionary<DateTime, bool> _coarseSelected = new Dictionary<
            DateTime,
            bool
        >
        {
        ...
        private static readonly Dictionary<DateTime, Symbol[]> _expectedSymbols = new Dictionary<
            DateTime,
            Symbol[]
        >
        {
            { _20140325, new[] { _bac, _qqq, _spy } },
            { _20140326, new[] { _spy } },
            { _20140327, new[] { _aapl, _bac, _gme, _qqq, _spy, } },
            { _20140328, new[] { _goog } },
        ...
        public override void OnData(Slice slice)
        {
            ...
            foreach (
                var (symbol, security) in ActiveSecurities
            ...
            )
            {
                ...
                if (shortableQuantity == null)
                {
                    throw new RegressionTestException(
                        $"Expected {symbol} to be shortable on {Time:yyyy-MM-dd}"
                    );
                }
        ...
        {
            var shortableSymbols = (_security.ShortableProvider as dynamic).AllShortableSymbols(
                Time
            );
            ...
            if (Time.Date == _20140327)
            {
                ...
                {
                    throw new RegressionTestException(
                        "Expected unmapped GME in shortable symbols list on 2014-03-27"
                    );
                }
                if (!coarse.Select(x => x.Symbol.Value).Contains("GME"))
                {
                    throw new RegressionTestException(
                        "Expected mapped GME in coarse symbols on 2014-03-27"
                    );
                }
            ...
            {
                throw new RegressionTestException(
                    $"Expected Symbols selected on {Time.Date:yyyy-MM-dd} to match expected Symbols, but the following Symbols were missing: {string.Join(", ", missing.Select(s => s.ToString()))}"
                );
            }
        ...
        public override void OnEndOfAlgorithm()
        {
            if (!_coarseSelected.Values.All(x => x))
            {
                throw new AggregateException(
                    $"Expected coarse selection on all dates, but didn't run on: {string.Join(", ", _coarseSelected.Where(kvp => !kvp.Value).Select(kvp => kvp.Key.ToStringInvariant("yyyy-MM-dd")))}"
                );
            }
        }
        ...
        {
            public AllShortableSymbolsRegressionAlgorithmBrokerageModel()
                : base() { }
        ...
        private class RegressionTestShortableProvider : LocalDiskShortableProvider
        {
            ...
            {
                var shortableDataDirectory = Path.Combine(
                    Globals.DataFolder,
                ...
                while (i <= 7)
                {
                    var shortableListFile = Path.Combine(
                        shortableDataDirectory,
                        "dates",
                        $"{localTime.AddDays(-i):yyyyMMdd}.csv"
                    );

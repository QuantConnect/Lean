...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class ScaledRawHistoryAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException("No split or dividend was found in the algorithm.");
            }
            ...
            {
                throw new TestException($@"Expected history results to not be empty and have the same count. Raw: {rawHistory.Count
            ...
            for (var i = 0; i < rawHistory.Count; i++)
            {
                ...
                if (rawBar.Time < _lastSplitOrDividendDate)
                {
                    ...
                    {
                        throw new TestException($@"Expected history results to be different at {rawBar.Time
                ...
                {
                    throw new TestException($@"Expected history results to be the same at {rawBar.Time} after the last split or dividend date {_lastSplitOrDividendDate}");
                }
            }
        }

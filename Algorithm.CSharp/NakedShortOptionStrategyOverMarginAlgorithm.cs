...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class NakedShortOptionStrategyOverMarginAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (!Portfolio.Invested)
            {
                if (slice.OptionChains.TryGetValue(_optionSymbol, out var chain))
                {
                    ...
                    if (atmStraddle != null)
                    {
                        _optionStrategy = OptionStrategies.Straddle(
                            _optionSymbol,
                            atmStraddle.Strike,
                            atmStraddle.Expiry
                        );
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            ...
            if (
                orderEvent.Quantity == _quantityOverMargin
                && orderEvent.Status != OrderStatus.Invalid
            )
            {
                throw new RegressionTestException(
                    $"Orders with quantity {_quantityOverMargin} should be invalid"
                );
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new RegressionTestException(
                    $"Expected {expectedFilledOrdersCount} filled orders, found {filledOrdersCount}"
                );
            }
            ...
            {
                throw new RegressionTestException(
                    $"Expected position quantity to be {expectedQuantity} but was {positionGroup.Quantity}"
                );
            }
        }

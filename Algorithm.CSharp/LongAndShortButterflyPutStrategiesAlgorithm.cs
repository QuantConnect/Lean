...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class LongAndShortButterflyPutStrategiesAlgorithm
        : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            ...
            foreach (var group in contractsByExpiry)
            {
                ...
                var atmStrike = contracts
                    .MinBy(x => Math.Abs(x.Strike - chain.Underlying.Value))
                    .Strike;
                ...
                {
                    // Ready to trade
                    _butterflyPut = OptionStrategies.ButterflyPut(
                        _optionSymbol,
                        itmStrike,
                        atmStrike,
                        otmStrike,
                    ...
                    _shortButterflyPut = OptionStrategies.ShortButterflyPut(
                        _optionSymbol,
                        itmStrike,
                        atmStrike,
                        otmStrike,
        ...
        protected override void AssertStrategyPositionGroup(IPositionGroup positionGroup)
        {
            if (positionGroup.Positions.Count() != 3)
            {
                throw new RegressionTestException(
                    $"Expected position group to have 3 positions. Actual: {positionGroup.Positions.Count()}"
                );
            }

            var higherStrike = _butterflyPut.OptionLegs.Max(leg => leg.Strike);
            var higherStrikePosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Put
                && x.Symbol.ID.StrikePrice == higherStrike
            );

            if (higherStrikePosition.Quantity != 2)
            {
                throw new RegressionTestException(
                    $"Expected higher strike position quantity to be 2. Actual: {higherStrikePosition.Quantity}"
                );
            }

            var lowerStrike = _butterflyPut.OptionLegs.Min(leg => leg.Strike);
            var lowerStrikePosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Put && x.Symbol.ID.StrikePrice == lowerStrike
            );

            if (lowerStrikePosition.Quantity != 2)
            {
                throw new RegressionTestException(
                    $"Expected lower strike position quantity to be 2. Actual: {lowerStrikePosition.Quantity}"
                );
            }

            var middleStrike = _butterflyPut
                .OptionLegs.Single(leg => leg.Strike < higherStrike && leg.Strike > lowerStrike)
                .Strike;
            var middleStrikePosition = positionGroup.Positions.Single(x =>
                x.Symbol.ID.OptionRight == OptionRight.Put
                && x.Symbol.ID.StrikePrice == middleStrike
            );

            if (middleStrikePosition.Quantity != -4)
            {
                throw new RegressionTestException(
                    $"Expected middle strike position quantity to be -4. Actual: {middleStrikePosition.Quantity}"
                );
            }
        }

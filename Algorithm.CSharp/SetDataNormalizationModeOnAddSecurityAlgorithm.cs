...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class SetDataNormalizationModeOnAddSecurityAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        private readonly DataNormalizationMode _esNormalizationMode =
            DataNormalizationMode.BackwardsRatio;
        private readonly DataNormalizationMode _btcustdNormalizationMode =
        ...
        public override void Initialize()
        {
            ...
            var esFutures = AddSecurity(
                SecurityType.Future,
                "ES",
                Resolution.Minute,
                dataNormalizationMode: _esNormalizationMode
            );
            CheckEquityDataNormalizationMode(esFutures, _esNormalizationMode);

            var btsustdCryto = AddSecurity(
                SecurityType.Crypto,
                "BTCUSDT",
                Resolution.Minute,
                dataNormalizationMode: _btcustdNormalizationMode
            );
        ...
        public override void OnData(Slice slice)
        {
            foreach (var kvp in _priceRanges)
            {
                ...
                if (
                    security.HasData
                    && (security.Price < minExpectedPrice || security.Price > maxExpectedPrice)
                )
                {
                    throw new RegressionTestException(
                        $"{security.Symbol}: Price {security.Price} is  out of expected range [{minExpectedPrice}, {maxExpectedPrice}]"
                    );
                }
            }
        }
        ...
        )
        {
            var subscriptions = SubscriptionManager.Subscriptions.Where(x =>
                x.Symbol == security.Symbol
            );
            if (subscriptions.Any(x => x.DataNormalizationMode != expectedNormalizationMode))
            {
                throw new RegressionTestException(
                    $"Expected {security.Symbol} to have data normalization mode {expectedNormalizationMode} but was {subscriptions.First().DataNormalizationMode}"
                );
            }
        }

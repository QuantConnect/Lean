...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class AddFutureOptionContractFromFutureChainRegressionAlgorithm
        : QCAlgorithm,
        ...
        public override void OnData(Slice slice)
        {
            if (!_addedOptions)
            {
                ...
                foreach (var futuresContracts in slice.FutureChains.Values)
                {
                    ...
                    {
                        var option_contract_symbols = OptionChainProvider
                            .GetOptionContractList(contract.Symbol, Time)
                            .ToList();
                        ...
                        foreach (
                            var option_contract_symbol in option_contract_symbols
                                .OrderBy(x => x.ID.Date)
                                .ThenBy(x => x.ID.StrikePrice)
                                .ThenBy(x => x.ID.OptionRight)
                                .Take(5)
                        )
                        {

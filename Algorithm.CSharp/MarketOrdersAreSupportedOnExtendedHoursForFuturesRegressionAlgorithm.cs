...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class MarketOrdersAreSupportedOnExtendedHoursForFuturesRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            if (Time.TimeOfDay.Hours > 18 && !Portfolio.Invested)
            {
                ...
                if (
                    futureContractMarketOrder.Status == OrderStatus.Invalid
                    || continuousContractMarketOrder.Status == OrderStatus.Invalid
                )
                {
                    throw new RegressionTestException(
                        $"Market orders should be allowed for futures outside of regular market hours"
                    );
                }
            }
        }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityShortButterflyPutRegressionAlgorithm
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                )
                {
                    ...
                    var middlePut = contracts.First(contract =>
                        contract.Expiry == lowerPut.Expiry
                        && contract.Strike == lowerPut.Strike + distanceBetweenStrikes
                    );
                    var highestPut = contracts.First(contract =>
                        contract.Expiry == lowerPut.Expiry
                        && contract.Strike == middlePut.Strike + distanceBetweenStrikes
                    );
                    ...
                    var expectedMarginUsage = Math.Max(
                        (middlePut.Strike - lowerPut.Strike)
                            * Securities[middlePut.Symbol].SymbolProperties.ContractMultiplier
                            * 10,
                        0
                    );

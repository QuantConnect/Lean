...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class ETFConstituentUniverseCompositeDelistingRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private IEnumerable<Symbol> FilterETFs(IEnumerable<ETFConstituentUniverse> constituents)
        {
            ...
            {
                throw new TestException($"Performing constituent universe selection on {UtcTime:yyyy-MM-dd HH:mm:ss.fff} after composite ETF has been delisted");
            }
        ...
        public override void OnData(Slice data)
        {
            ...
            {
                throw new TestException($"Received unexpected slice in OnData(...) after universe was deselected");
            }
        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            ...
            {
                throw new TestException("New securities added after ETF constituents were delisted");
            }
        ...
        public override void OnEndOfAlgorithm()
        {
            if (!_universeAdded)
            {
                throw new TestException("ETF constituent universe was never added to the algorithm");
            }
            if (!_universeRemoved)
            {
                throw new TestException("ETF constituent universe was not removed from the algorithm after delisting");
            }
            if (ActiveSecurities.Count > 2)
            {
                throw new TestException($"Expected less than 2 securities after algorithm ended, found {Securities.Count}");
            }
        }

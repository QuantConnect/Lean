...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class HistoryWithDifferentContinuousContractDepthOffsetsRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException("History results are empty or bar counts did not match");
            }
            ...
            for (int i = 0; i < historyResults.Count; i++)
            {
                ...
                foreach (var slice in historyResults[i])
                {
                    ...
                    if (underlyings.Add(underlying) && underlyings.Count > 1)
                    {
                        ...
                        if (contractDepthOffsets[i] == 0)   // Front month
                        {
                            ...
                            {
                                throw new TestException($"Unexpected current mapped contract expiration {currentExpiration}" +
                ...
                {
                    throw new TestException($"History results for contractDepthOffset={contractDepthOffsets[i]} did not contain any mappings");
                }
            ...
            for (int i = 0; i < underlyingsPerHistory.Count; i++)
            {
                for (int j = i + 1; j < underlyingsPerHistory.Count; j++)
                {
                    ...
                    {
                        throw new TestException($"History results for contractDepthOffset={contractDepthOffsets[i]} and {contractDepthOffsets[j]} contain the same underlying");
                    }
                }
            }
            ...
            for (int j = 0; j < historyResults[0].Count; j++)
            {
                ...
                {
                    throw new TestException($"History results close prices should have been different for each contract depth offset at each time");
                }
            }
        }

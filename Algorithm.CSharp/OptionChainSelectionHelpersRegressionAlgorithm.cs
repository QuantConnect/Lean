...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class OptionChainSelectionHelpersRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            if (contract.Expiry != new DateTime(2015, 12, 31) || contract.Strike != 747.5m || contract.DaysToExpiry != 7)
            {
                throw new RegressionTestException($"Unexpected contract selected: {contract.Symbol.Value}, {contract.DaysToExpiry} days to expiry");
            }

            // Pick is a synonym of Select
            if (!contract.Symbol.Equals(chain.Pick(right: OptionRight.Call, targetDte: 10).Symbol))
            {
                throw new RegressionTestException("Pick() and Select() must select the same contract");
            }
            ...
            if (chain.Expiries[0] != Time.Date || chain.Expiries.Last() != chain.Expiries.Max())
            {
            ...
            var strikes = atExpiry.StrikePrices;
            if (strikes.ClosestTo(spot) != 747.5m || strikes.FirstAbove(spot) != 750m || strikes.FirstBelow(spot) != 747.5m)
            {
                throw new RegressionTestException(
                    $"StrikePrices helpers mismatch: {strikes.ClosestTo(spot)}/{strikes.FirstAbove(spot)}/{strikes.FirstBelow(spot)}");
            }

            // Strike distance in price units, like the universe strategy filters: 5 above the spot is the 752.50 call
            var otmCall = chain.Select(right: OptionRight.Call, targetDte: 7, strikeFromAtm: 5);
            if (otmCall == null || otmCall.Strike != 752.5m)
            {
                throw new RegressionTestException($"Select(strikeFromAtm) expected the 752.50 call but got {otmCall?.Symbol.Value}");
            }

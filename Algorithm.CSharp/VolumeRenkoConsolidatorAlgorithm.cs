...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class VolumeRenkoConsolidatorAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        private Symbol _spy,
            _ibm;
        private VolumeRenkoConsolidator _tradebarVolumeConsolidator,
            _tickVolumeConsolidator;
        ...
        public override void Initialize()
        {
            ...
            _tradebarVolumeConsolidator.DataConsolidated += (sender, bar) =>
            {
                ...
                Debug(
                    $"SPY {bar.Time} to {bar.EndTime} :: O:{bar.Open} H:{bar.High} L:{bar.Low} C:{bar.Close} V:{bar.Volume}"
                );
                if (bar.Volume != 1000000)
                {
                    throw new RegressionTestException(
                        "Volume of consolidated bar does not match set value!"
                    );
                }
            };
            ...
            {
                Debug(
                    $"IBM {bar.Time} to {bar.EndTime} :: O:{bar.Open} H:{bar.High} L:{bar.Low} C:{bar.Close} V:{bar.Volume}"
                );
                if (bar.Volume != 1000000)
                {
                    throw new RegressionTestException(

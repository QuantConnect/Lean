        ...
        public override void OnData(Slice data)
        {
            ...
            if (current != null)
            {
                if (Time == new DateTime(2014, 06, 09, 4, 1, 0) && !Portfolio.Invested)
                {
                    ...
                    {
                        throw new TestException($"Was expecting a first fill forward bar {Time}");
                    }
                ...
                {
                    throw new TestException($"FillForwarded data point at {Time} was scaled. Actual: {current.Close}; Expected: {_lastRealBar.Close}");
                }
            }
        }

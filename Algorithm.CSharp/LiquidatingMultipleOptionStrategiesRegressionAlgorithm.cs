...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class LiquidatingMultipleOptionStrategiesRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (!Portfolio.Invested)
            {
                ...
                _bullCallSpread = OptionStrategies.BullCallSpread(
                    _symbol,
                    calls[0].Strike,
                    calls[1].Strike,
                    calls[0].Expiry
                ...
                _bearPutSpread = OptionStrategies.BearPutSpread(
                    _symbol,
                    puts[0].Strike,
                    puts[1].Strike,
                    puts[0].Expiry
            ...
            else
            {
                ...
                if (positionGroups.Count != 2)
                {
                    throw new RegressionTestException(
                        $"Expected 2 position groups, one for each spread, but found {positionGroups.Count}"
                    );
                }

                var positionGroupMatchesSpreadStrategy = (
                ...
                ) =>
                {
                    ...
                    {
                        var legSymbol = QuantConnect.Symbol.CreateOption(
                            strategy.Underlying,
                            strategy.CanonicalOption?.ID?.Symbol,
                            strategy.Underlying.ID.Market,
                            _symbol.ID.OptionStyle,
                            leg.Right,
                            leg.Strike,
                            leg.Expiration
                        );
                        return positionGroup.Positions.Any(position =>
                            position.Symbol == legSymbol
                        );
                    });
                };
                if (
                    !positionGroups.All(group =>
                        positionGroupMatchesSpreadStrategy(group, _bullCallSpread)
                        || positionGroupMatchesSpreadStrategy(group, _bearPutSpread)
                    )
                )
                {
                    throw new RegressionTestException(
                        "Expected both spreads to have a matching position group in the portfolio."
                    );
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new RegressionTestException(
                    "The spreads should have been liquidated by the end of the algorithm"
                );
            }
        }

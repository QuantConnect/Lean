    ...
    public class ContinuousFutureRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice data)
        {
            ...
            if (!_continuousContract.Exchange.DateTimeIsOpen(Time.AddMinutes(-1)))
            {
                ...
                {
                    throw new TestException($"We are getting data during closed market!");
                }
            }
            ...
            foreach (var changedEvent in data.SymbolChangedEvents.Values)
            {
                if (changedEvent.Symbol == _continuousContract.Symbol)
                {
                    ...
                    {
                        throw new TestException($"Continuous contract current symbol did not change! {_continuousContract.Mapped}");
                    }
            ...
            if (_lastMonth.Month != Time.Month && currentlyMappedSecurity.HasData)
            {
                ...
                if(Time.Month == 1 && Time.Year == 2013)
                {
                    ...
                    if (!response.Any())
                    {
                        throw new TestException("Unexpected empty history response");
                    }
                }
            }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException($"Unexpected symbol changed events: {_mappings.Count}, was expecting {expectedMappingCounts}");
            }
            ...
            {
                throw new TestException($"We should have a single non tradable future contract security! found: {securities.Count}");
            }
        }

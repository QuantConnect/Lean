    ...
    public class CustomDataMultiFileObjectStoreRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            if (slice.ContainsKey(_customSymbol))
            {
                ...
                {
                    throw new TestException("Custom data price was not expected to be zero");
                }
                var customData2 = CurrentSlice.Get<ExampleCustomData>(_customSymbol);
                if (customData2.Price == 0)
                {
                    throw new TestException("Custom data 2 price was not expected to be zero");
                }
                ...
                if (customData.Price == 0)
                {
                    throw new TestException("Custom data price was not expected to be zero, index access");
                }
                customData2 = CurrentSlice.Get<ExampleCustomData>()[_customSymbol];
                if (customData2.Price == 0)
                {
                    throw new TestException("Custom data 2 price was not expected to be zero, index access");
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException("Custom data was not fetched");
            }

            var customSecurity = Securities[_customSymbol];
            if (customSecurity == null || customSecurity.Price == 0)
            {
                throw new TestException("Expected the custom security to be added to the algorithm securities and to have a price that is not zero");
            }
            ...
            {
                throw new TestException("History request returned different data than expected");
            }

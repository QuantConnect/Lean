...
namespace QuantConnect.Algorithm.CSharp
{
            ...
            IRegressionAlgorithmDefinition
    {
            ...
            if (slice.ContainsKey(_customSymbol))
            {
                ...
                if (customData.Price == 0)
                {
                    throw new RegressionTestException(
                        "Custom data price was not expected to be zero"
                    );
                }
                ...
                {
                    throw new RegressionTestException(
                        "Custom data 2 price was not expected to be zero"
                    );
                }
                ...
                {
                    throw new RegressionTestException(
                        "Custom data price was not expected to be zero, index access"
                    );
                }
                ...
                {
                    throw new RegressionTestException(
                        "Custom data 2 price was not expected to be zero, index access"
                    );
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new RegressionTestException(
                    "Expected the custom security to be added to the algorithm securities and to have a price that is not zero"
                );
            }

            // Make sure history requests work as expected
            var history = History<ExampleCustomData>(
                    _customSymbol,
                    StartDate,
                    EndDate,
                    Resolution.Hour

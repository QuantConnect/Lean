...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class StopLimitOrderRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            {
                _buyOrderTicket = StopLimitOrder(
                    _symbol,
                    100,
                    stopPrice: security.High * 1.10m,
                    limitPrice: security.High * 1.11m
                );
            }
            else if (
                _buyOrderTicket.Status == OrderStatus.Filled
                && _sellOrderTicket == null
                && TrendIsDown
            )
            {
                _sellOrderTicket = StopLimitOrder(
                    _symbol,
                    -100,
                    stopPrice: security.Low * 0.99m,
                    limitPrice: security.Low * 0.98m
                );
            }
        }
            ...
            if (orderEvent.Status == OrderStatus.Filled)
            {
                ...
                if (!((StopLimitOrder)order).StopTriggered)
                {
                    throw new RegressionTestException(
                        "StopLimitOrder StopTriggered should haven been set if the order filled."
                    );
                }
                ...
                if (orderEvent.Direction == OrderDirection.Buy)
                {
                    ...
                    {
                        throw new RegressionTestException(
                            $@"Buy stop limit order should have filled with price less than or equal to the limit price {
                ...
                else
                {
                    ...
                    {
                        throw new RegressionTestException(
                            $@"Sell stop limit order should have filled with price greater than or equal to the limit price {
                            limitPrice}. Fill price: {orderEvent.FillPrice}"
                        );
                    }
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new RegressionTestException(
                    "Expected two orders (buy and sell) to have been filled at the end of the algorithm."
                );
            }

            if (
                _buyOrderTicket.Status != OrderStatus.Filled
                || _sellOrderTicket.Status != OrderStatus.Filled
            )
            {
                throw new RegressionTestException(
                    "Expected the two orders (buy and sell) to have been filled at the end of the algorithm."
                );
            }
        }

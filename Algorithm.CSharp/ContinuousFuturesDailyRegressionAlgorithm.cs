...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class ContinuousFuturesDailyRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            foreach (var changedEvent in slice.SymbolChangedEvents.Values)
            {
                if (changedEvent.Symbol == _continuousContract.Symbol)
                {
                    ...
                    Log(
                        $"{Time} - SymbolChanged event: {changedEvent}. New expiration {_continuousContract.Mapped.ID.Date}"
                    );
                }
            }
            ...
            if (_symbolChangedEvent != null)
            {
                if (_previousFactor == priceFactor)
                {
                    throw new RegressionTestException(
                        $"Price factor did not change after symbol changed! {Time} {priceFactor}"
                    );
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new RegressionTestException(
                    "Unexpected a symbol changed event but got none!"
                );
            }
        }

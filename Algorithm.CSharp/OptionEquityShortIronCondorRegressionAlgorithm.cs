...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityShortIronCondorRegressionAlgorithm
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                )
                {
                    ...
                    var oufOfTheMoneyPut = contracts.First(contract =>
                        contract.Right == OptionRight.Put
                    );

                    var lessOufOfTheMoneyPut = contracts.First(contract =>
                        contract.Right == OptionRight.Put
                        && contract.Expiry == oufOfTheMoneyPut.Expiry
                        && contract.Strike > oufOfTheMoneyPut.Strike
                    );

                    var oufOfTheMoneyCall = contracts.First(contract =>
                        contract.Right == OptionRight.Call
                        && contract.Expiry == oufOfTheMoneyPut.Expiry
                        && contract.Strike > chain.Underlying.Price
                    );

                    var moreOufOfTheMoneyCall = contracts.First(contract =>
                        contract.Right == OptionRight.Call
                        && contract.Expiry == oufOfTheMoneyPut.Expiry
                    ...
                    var priceSpreadDifference = GetPriceSpreadDifference(
                        ...
                        lessOufOfTheMoneyPut.Symbol,
                        oufOfTheMoneyCall.Symbol,
                        moreOufOfTheMoneyCall.Symbol
                    );
                    if (
                        initialMargin
                        != (

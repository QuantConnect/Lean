    ...
    public class FuturesFrameworkRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (continuous.Price == Securities[(continuous as Future).Mapped].Price)
            {
                ...
                throw new TestException($"Unexpected continuous future price {continuous.Price}");
            }
        }
        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            ...
            foreach (var subscriptions in SubscriptionManager.Subscriptions.Where(x => canonicals.Contains(x.Symbol.Canonical)).GroupBy(x => x.Symbol.Canonical))
            {
                ...
                {
                    throw new TestException($"Unexpected canonical subscription count {subscriptions.Count(x => x.Symbol.IsCanonical())}");
                }

                // trade and quote for non canonicals
                if (subscriptions.Count(x => !x.Symbol.IsCanonical()) != nonCanonicals.Count * 2)
                {
                    throw new TestException($"Unexpected non canonical subscription count {subscriptions.Count(x => !x.Symbol.IsCanonical())}");
                }
            }
            ...
            {
                throw new TestException($"Unexpected internal subscription count {internalSubscriptions.Count}");
            }
            ...
            {
                throw new TestException($"Unexpected universe subscription count {universeSubscriptions}");
            }
        ...
        public override void OnEndOfAlgorithm()
        {
            foreach (var canonical in _addedCanonical)
            {
                if (!canonical.Value)
                {
                    throw new TestException($"Canonical {canonical} was not added!");
                }
            }
            foreach (var canonical in _removedCanonical)
            {
                ...
                {
                    throw new TestException($"Canonical {canonical} was not removed!");
                }
            }
            foreach (var canonical in _canonicalData)
            {
                ...
                {
                    throw new TestException($"Canonical {canonical} emitted no data!");
                }
            }

            if (SubscriptionManager.Subscriptions.Any(x => x.Symbol.ID.Symbol == "ES"))
            {
                throw new TestException($"There should be no ES subscription!");
            }
        }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class HistoryWithDifferentDataMappingModeRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException("History results bar count did not match");
            }
            ...
            for (int i = 0; i < historyResults.Count; i++)
            {
                ...
                if (mappingsCount == 0)
                {
                    throw new TestException($"History results for {dataMappingModes[i]} data mapping mode did not contain any mappings");
                }
            }

            if (mappingDates.Count < dataMappingModes.Count)
            {
                throw new TestException($"History results should have had different mapping dates for each data mapping mode");
            }
            ...
            for (int j = 0; j < historyResults[0].Count; j++)
            {
                ...
                {
                    throw new TestException($"History results close prices should have been different for each data mapping mode at each time");
                }
            }
        }

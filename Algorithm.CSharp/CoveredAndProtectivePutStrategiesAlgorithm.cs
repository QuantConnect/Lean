...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class CoveredAndProtectivePutStrategiesAlgorithm
        : OptionStrategyFactoryMethodsBaseAlgorithm
    {
        ...
        protected override void TradeStrategy(OptionChain chain)
        {
            ...
            if (contract != null)
            {
                _coveredPut = OptionStrategies.CoveredPut(
                    _optionSymbol,
                    contract.Strike,
                    contract.Expiry
                );
                _protectivePut = OptionStrategies.ProtectivePut(
                    _optionSymbol,
                    contract.Strike,
                    contract.Expiry
                );
        ...
        protected override void AssertStrategyPositionGroup(IPositionGroup positionGroup)
        {
            ...
            {
                throw new RegressionTestException(
                    $"Expected option position to be a put. Actual: {optionPosition.Symbol.ID.OptionRight}"
                );
            }

            var underlyingPosition = positionGroup.Positions.Single(x =>
                x.Symbol.SecurityType == SecurityType.Equity
            );
            var expectedOptionPositionQuantity = -2;
            var expectedUnderlyingPositionQuantity =
                -2 * Securities[_optionSymbol].SymbolProperties.ContractMultiplier;
            ...
            {
                throw new RegressionTestException(
                    $@"Expected underlying position quantity to be {expectedUnderlyingPositionQuantity}. Actual: {underlyingPosition.Quantity}"
                );
            }
        }

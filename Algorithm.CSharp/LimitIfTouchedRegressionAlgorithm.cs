...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class LimitIfTouchedRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private readonly Queue<string> _expectedEvents = new Queue<string>(
            new[]
            {
        ...
        public override void OnData(Slice slice)
        {
            ...
            if (Transactions.GetOpenOrders().Count == 0)
            {
                ...
                var orderRequest = new SubmitOrderRequest(
                    ...
                    _negative * 10,
                    0,
                    slice["SPY"].Price - (decimal)_negative,
                    slice["SPY"].Price - (decimal)0.25 * _negative,
                    UtcTime,
            ...
            if (_request != null)
            {
                ...
                _request.UpdateTriggerPrice(
                    _request.Get(OrderField.TriggerPrice).RoundToSignificantDigits(5)
                );
            }
        }
            ...
            if (orderEvent.Status == OrderStatus.Filled)
            {
                ...
                if (orderEvent.ToString() != expected)
                {
                    throw new RegressionTestException(
                        $"orderEvent {orderEvent.Id} differed from {expected}. Actual {orderEvent}"
                    );
                }
            }
        }

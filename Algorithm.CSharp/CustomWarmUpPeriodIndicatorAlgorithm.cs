...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class CustomWarmUpPeriodIndicatorAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            {
                throw new TestException("_customWarmUp indicator was expected to be ready");
            }
            if (_customWarmUp.Samples != 60)
            {
                throw new TestException("_customWarmUp indicator was expected to have processed 60 datapoints already");
            }
            ...
            {
                throw new TestException("_customNotWarmUp indicator wasn't expected to be warmed up");
            }
            ...
            {
                throw new TestException("_customNotInherit indicator was expected to be ready");
            }
            if (_customNotInherit.Samples != 60)
            {
                throw new TestException("_customNotInherit indicator was expected to have processed 60 datapoints already");
            }
            ...
            {
                throw new TestException("_duplicateSMA indicator was expected to be ready");
            }
            if (_duplicateSMA.Samples != 60)
            {
                throw new TestException("_duplicateSMA indicator was expected to have processed 60 datapoints already");
            }
        }
        ...
        public override void OnData(Slice data)
        {
            ...
            if (Time.Second == 0)
            {
                ...
                {
                    throw new TestException("_customNotWarmUp indicator was expected to be ready when the number of samples were bigger that its WarmUpPeriod parameter");
                }

                // Check their values are the same when both are ready
                if (diff > 1e-10m && _customNotWarmUp.IsReady && _customWarmUp.IsReady) 
                {
                    throw new TestException($"The values of the indicators are not the same. The difference is {diff}");
                }
            }
        }

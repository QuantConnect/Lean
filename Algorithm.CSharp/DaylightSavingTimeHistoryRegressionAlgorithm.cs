...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class DaylightSavingTimeHistoryRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            for (int i = 0; i < _symbols.Length; i++)
            {
                ...
                if (symbol.SecurityType == SecurityType.Equity)
                {
                    try
                    {
                        ...
                        throw new RegressionTestException(
                            "We were expecting an argument exception to be thrown. Equity does not have daily QuoteBars!"
                        );
                    }
                    ...
                    history = History<TradeBar>(symbol, 10, Resolution.Daily)
                        .Select(bar => bar as BaseData);
                }
                else
                {
                ...
                var duplications = history.GroupBy(k => k.Time).Where(g => g.Count() > 1);
                if (duplications.Any())
                {
                    var time = duplications.First().Key;
                    throw new RegressionTestException(
                        $"Duplicated bars were issued for time {time}"
                    );
                }
            }
        }

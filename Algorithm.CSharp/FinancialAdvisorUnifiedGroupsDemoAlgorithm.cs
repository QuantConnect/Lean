...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class FinancialAdvisorUnifiedGroupsDemoAlgorithm : QCAlgorithm
    {
                ...
                lock (_orderStateLock)
                {
                    ...
                    if (_groupOrderSubmitted ||
                        ...
                        !submissionSnapshot.Groups.TryGetValue(
                            GroupName,
                            out var submissionGroup) ||
                        submissionGroup.AccountIds.Any(accountId =>
                            !submissionSnapshot.Accounts.ContainsKey(
                                accountId)))
                    {
                        return;
                    }
            ...
            out string errorMessage)
        {
            ...
            if (!IsSnapshotFresh(snapshot) ||
                ...
                GetSnapshotFinancialDataAsOfUtc(snapshot) <
            ...
            BrokerageAccountSnapshot snapshot)
        {
            ...
            return GetSnapshotFinancialDataAsOfUtc(snapshot) >=
                UtcTime - MaximumSnapshotAge;
        }

        private static DateTime GetSnapshotFinancialDataAsOfUtc(
            BrokerageAccountSnapshot snapshot) =>
            snapshot.CollectionStartedUtc == default
                ? snapshot.LastSuccessfulUpdateUtc
                : snapshot.CollectionStartedUtc;

        private static bool IsSupportedGroupAllocationMethod(
            string allocationMethod) =>
            allocationMethod?.Trim() is { } method &&
            (method.Equals("Equal", StringComparison.OrdinalIgnoreCase) ||
             method.Equals("NetLiq", StringComparison.OrdinalIgnoreCase) ||
             method.Equals("AvailableEquity", StringComparison.OrdinalIgnoreCase) ||
             method.Equals("Ratio", StringComparison.OrdinalIgnoreCase) ||
             method.Equals("Percent", StringComparison.OrdinalIgnoreCase));

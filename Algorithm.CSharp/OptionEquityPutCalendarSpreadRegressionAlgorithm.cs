...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class OptionEquityPutCalendarSpreadRegressionAlgorithm
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                )
                {
                    ...
                    var longPut = contracts.First(contract =>
                        contract.Expiry > shortPut.Expiry && contract.Strike == shortPut.Strike
                    );
                    ...
                    AssertOptionStrategyIsPresent(
                        OptionStrategyDefinitions.PutCalendarSpread.Name,
                        10
                    );

                    var expectedMarginUsage = Math.Max(
                        (shortPut.Strike - longPut.Strike)
                            * Securities[longPut.Symbol].SymbolProperties.ContractMultiplier
                            * 10,
                        0
                    );

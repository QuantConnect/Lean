    ...
    public class RevertComboOrderPositionsAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnOrderEvent(OrderEvent orderEvent)
        {
            if (orderEvent.Status == OrderStatus.Filled)
            {
                ...
                {
                    throw new TestException($"Order event quantity {orderEvent.Quantity} does not match expected quantity {expectedQuantity}");
                }
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                throw new TestException("Portfolio should not be invested at the end of the algorithm.");
            }

            if (_entryOrderTickets.Count == 0 || _entryOrderTickets.Any(ticket => ticket.Status != OrderStatus.Filled))
            {
                throw new TestException("Entry order was not filled");
            }

            if (_exitOrderTickets.Count == 0 || _exitOrderTickets.Any(ticket => ticket.Status != OrderStatus.Filled))
            {
                throw new TestException("Exit order was not filled");
            }
            ...
            for (var i = 0; i < _orderLegs.Count; i++)
            {
                ...
                {
                    throw new TestException($@"Entry order ticket quantity and filled quantity do not match expected quantity for leg {i
                ...
                {
                    throw new TestException($@"Exit order ticket quantity and filled quantity do not match expected quantity for leg {i

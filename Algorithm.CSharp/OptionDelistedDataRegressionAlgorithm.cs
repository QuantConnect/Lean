    ...
    public class OptionDelistedDataRegressionAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice data)
        {
            ...
            if (Time.Minute == 0)
            {
                foreach (var d in data)
                {
                    ...
                    {
                        throw new TestException("We shouldn't be recieving data from an already delisted symbol");
                    }
                }
            }
        }
        ...
        public override void OnEndOfAlgorithm()
        {
            if (_executed)
            {
                ...
                {
                    throw new TestException("Expecting exactly 20 delisted events");
                }
                if (_toBeDelisted.Count != 20)
                {
                    throw new TestException("Expecting exactly 20 to be delisted warning events");
                }
            }

...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    /// </summary>
    public class FutureOptionShortPutITMExpiryRegressionAlgorithm
        : QCAlgorithm,
            IRegressionAlgorithmDefinition
    {
        ...
        )
        {
            if (orderEvent.Message.Contains("Assignment"))
            {
                ...
                {
                    throw new RegressionTestException(
                        "Option was not assigned at expected strike price (3400)"
                    );
                }
                if (orderEvent.Direction != OrderDirection.Buy || future.Holdings.Quantity != 1)
                {
                    throw new RegressionTestException(
                        $"Expected Qty: 1 futures holdings for assigned future {future.Symbol}, found {future.Holdings.Quantity}"
                    );
                }
            }
            if (
                !orderEvent.Message.Contains("Assignment")
                && orderEvent.Direction == OrderDirection.Sell
                && future.Holdings.Quantity != 0
            )
            {
                // We buy back the underlying at expiration, so we expect a neutral position then
                throw new RegressionTestException(
                    $"Expected no holdings when liquidating future contract {future.Symbol}"
                );
            }
        }

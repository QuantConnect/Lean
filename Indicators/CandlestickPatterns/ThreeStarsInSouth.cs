    ...
    public class ThreeStarsInSouth : CandlestickPattern
    {
        ...
        public ThreeStarsInSouth(string name)
            : base(
                ...
                Math.Max(
                    Math.Max(
                        CandleSettings.Get(CandleSettingType.ShadowVeryShort).AveragePeriod,
            ...
            )
        {
            ...
            _shadowLongAveragePeriod = CandleSettings
                ...
                .AveragePeriod;
            _shadowVeryShortAveragePeriod = CandleSettings
                .Get(CandleSettingType.ShadowVeryShort)
                .AveragePeriod;
        ...
        )
        {
            if (!IsReady)
            {
                ...
                {
                    _shadowLongPeriodTotal += GetCandleRange(
                ...
                {
                    _shadowVeryShortPeriodTotal[1] += GetCandleRange(
                        CandleSettingType.ShadowVeryShort,
                        window[1]
                    );
                    _shadowVeryShortPeriodTotal[0] += GetCandleRange(
                        CandleSettingType.ShadowVeryShort,
                        input
                    );
                }
            ...
            if (
                ...
                GetLowerShadow(window[1])
                    > GetCandleAverage(
                        CandleSettingType.ShadowVeryShort,
                        _shadowVeryShortPeriodTotal[1],
                        window[1]
                    )
                &&
                // 3rd: small marubozu
            ...
            _shadowLongPeriodTotal +=
                ...
                - GetCandleRange(
                    CandleSettingType.ShadowLong,
                    window[2 + _shadowLongAveragePeriod]
                );

            for (var i = 1; i >= 0; i--)
            {
                _shadowVeryShortPeriodTotal[i] +=
                    GetCandleRange(CandleSettingType.ShadowVeryShort, window[i])
                    - GetCandleRange(
                        CandleSettingType.ShadowVeryShort,
                        window[i + _shadowVeryShortAveragePeriod]
                    );
            }

/*
 * QUANTCONNECT.COM - Democratizing Finance, Empowering Individuals.
 * Lean Algorithmic Trading Engine v2.0. Copyright 2014 QuantConnect Corporation.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
*/

using System;
using System.Collections.Generic;
using Accord.Math;
using Accord.Math.Optimization;

namespace QuantConnect.Algorithm.Framework.Portfolio.Optimization
{
    public class MeanVariancePortfolio : IPortfolioOptimization
    {
        protected double _lower;
        protected double _upper;
        protected double _targetReturn;
        protected double[,] _cov;
        protected List<LinearConstraint> _constraints;

        protected int Size => _cov == null ? 0 : _cov.GetLength(0);

        public MeanVariancePortfolio(double lower, double upper, double targetReturn = 0.0)
        {
            _lower = lower;
            _upper = upper;
            _targetReturn = targetReturn;
            _cov = null;
            _constraints = new List<LinearConstraint>();
        }

        public void SetCovariance(double[,] cov)
        {
            _cov = cov;
        }

        protected double[] Optimize()
        {
            // sum(x) = 1
            _constraints.Add(new LinearConstraint(Size)
            {
                CombinedAs = Vector.Create(Size, 1.0),
                ShouldBe = Accord.Math.Optimization.ConstraintType.EqualTo,
                Value = 1.0
            });

            // lw <= x <= up
            for (int i = 0; i < Size; i++)
            {
                _constraints.Add(new LinearConstraint(1)
                {
                    VariablesAtIndices = new int[] { i },
                    ShouldBe = Accord.Math.Optimization.ConstraintType.GreaterThanOrEqualTo,
                    Value = _lower
                });
                _constraints.Add(new LinearConstraint(1)
                {
                    VariablesAtIndices = new int[] { i },
                    ShouldBe = Accord.Math.Optimization.ConstraintType.LesserThanOrEqualTo,
                    Value = _upper
                });
            }

            var optfunc = new QuadraticObjectiveFunction(_cov, Vector.Create(Size, 0.0));
            var solver = new GoldfarbIdnani(optfunc, _constraints);
            _constraints.Clear();

            bool success = solver.Minimize(Vector.Create(Size, 1.0 / Size));
            return success ? solver.Solution : Vector.Create(Size, Double.NaN);
        }

        public double[] Optimize(double[] expectedReturns)
        {
            // mu^T x = R  or mu^T x >= 0
            _constraints.Add(new LinearConstraint(Size)
            {
                CombinedAs = expectedReturns,
                ShouldBe = _targetReturn > 0 ? Accord.Math.Optimization.ConstraintType.EqualTo : Accord.Math.Optimization.ConstraintType.GreaterThanOrEqualTo,
                Value = _targetReturn
            });

            return Optimize();
        }

    }

    public class MaxSharpeRatioPortfolio : MeanVariancePortfolio
    {
        private double _riskFreeRate;

        public MaxSharpeRatioPortfolio(double lower, double upper, double riskFreeRate = 0.0) : base(lower, upper, 0.0)
        {
            _riskFreeRate = riskFreeRate;
        }

        public double[] Optimize(double[] expectedReturns)
        {
            // (r -r_f)^T x = R or (r -r_f)^T x >= 0
            _constraints.Add(new LinearConstraint(Size)
            {
                CombinedAs = expectedReturns.Subtract(_riskFreeRate),
                ShouldBe = _targetReturn > 0 ? Accord.Math.Optimization.ConstraintType.EqualTo : Accord.Math.Optimization.ConstraintType.GreaterThanOrEqualTo,
                Value = _targetReturn
            });

            return Optimize();
        }
    }
}

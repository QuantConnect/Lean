...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        /// </summary>
        private readonly string _symbolEmptyErrorMessage =
        ...
        private bool TryGetWarmupHistoryStartTime(out DateTime result)
        {
            ...
            if (_warmupBarCount.HasValue)
            {
                ...
                if (symbols.Count != 0)
                {
                    var startTimeUtc = CreateBarCountHistoryRequests(
                            symbols,
                            _warmupBarCount.Value,
                            Settings.WarmupResolution
                        )
                        .DefaultIfEmpty()
        ...
        [DocumentationAttribute(HistoricalData)]
        public IEnumerable<Slice> History(
            TimeSpan span,
        ...
        /// <returns>An enumerable of slice containing data over the most recent span for all configured securities</returns>
        [DocumentationAttribute(HistoricalData)]
        public IEnumerable<BaseDataCollection> History(
            Universe universe,
            int periods,
            Resolution? resolution = null,
            bool? fillForward = null,
        ...
        )
        {
            ...
            CheckPeriodBasedHistoryRequestResolution(
                symbols,
                resolution,
                universe.Configuration.Type
            );
        ...
        /// <returns>An enumerable of slice containing the requested historical data</returns>
        [DocumentationAttribute(HistoricalData)]
        public IEnumerable<DataDictionary<T>> History<T>(
            TimeSpan span,
            Resolution? resolution = null,
            bool? fillForward = null,
            bool? extendedMarketHours = null,
        ...
        /// <returns>An enumerable of slice containing the requested historical data</returns>
        [DocumentationAttribute(HistoricalData)]
        public IEnumerable<T> History<T>(
            Symbol symbol,
            TimeSpan span,
            Resolution? resolution = null,
            bool? fillForward = null,
        ...
        /// <returns>An enumerable of slice containing the requested historical data</returns>
        [DocumentationAttribute(HistoricalData)]
        public IEnumerable<TradeBar> History(
            Symbol symbol,
            int periods,
            Resolution? resolution = null,
            bool? fillForward = null,
        ...
        public IEnumerable<BaseData> GetLastKnownPrices(Symbol symbol)
        {
                ...
                var historyRequests = CreateBarCountHistoryRequests(new[] { symbol }, period)
                    .Select(request =>
                    {
                        ...
                        request.Resolution = (Resolution)
                            Math.Max((int)Resolution.Minute, (int)request.Resolution);
            ...
            if (!requestData(5))
            {
                if (resolution.HasValue)
                {
                    ...
                    var periods =
                        resolution.Value == Resolution.Daily
                            ? 3
        ...
        /// </remarks>
        private IEnumerable<T> GetDataTypedHistory<T>(
            ...
            where T : IBaseData
        {
            ...
            {
                result = GetPythonCustomDataTypeHistory(slices, historyRequests, symbol)
                    .OfType<T>();
            }
        ...
        /// This method will check for Python custom data types in order to call the right Slice.Get dynamic method
        /// </remarks>
        protected IEnumerable<DataDictionary<T>> GetDataTypedHistory<T>(
            IEnumerable<HistoryRequest> requests
        )
            where T : IBaseData
        {
            ...
            {
                result = GetPythonCustomDataTypeHistory(slices, historyRequests)
                    .OfType<DataDictionary<T>>();
            }
            else
            {
                ...
                if (
                    typeof(T) == typeof(BaseDataCollection)
                    && historyRequests[0].DataType != typeof(BaseDataCollection)
                )
                {
        ...
        private IEnumerable<Slice> History(
            IEnumerable<HistoryRequest> requests,
        ...
        private IEnumerable<HistoryRequest> GetFilterestRequests(
            IEnumerable<HistoryRequest> requests
        )
        {
        ...
        /// </summary>
        protected IEnumerable<HistoryRequest> CreateDateRangeHistoryRequests(
            IEnumerable<Symbol> symbols,
        ...
        )
        {
            ...
            return CreateDateRangeHistoryRequests(
                symbols,
                GetCustomDataTypeFromSymbols((Symbol[])symbols) ?? typeof(BaseData),
                startAlgoTz,
                endAlgoTz,
                resolution,
                fillForward,
        ...
        )
        {
            return symbols
                ...
                .SelectMany(x =>
                {
                    ...
                    {
                        var request = _historyRequestFactory.CreateHistoryRequest(
                            config,
                            startAlgoTz,
                            endAlgoTz,
                            GetExchangeHours(x, requestedType),
                            resolution,
                            fillForward,
                            extendedMarketHours,
                            dataMappingMode,
        ...
        /// </summary>
        private IEnumerable<HistoryRequest> CreateBarCountHistoryRequests(
            IEnumerable<Symbol> symbols,
            int periods,
            Resolution? resolution = null,
            bool? fillForward = null,
        ...
        )
        {
            return symbols
                ...
                .SelectMany(symbol =>
                {
                    ...
                    var configs = GetMatchingSubscriptions(symbol, requestedType, resolution)
                        .ToList();
                    if (configs.Count == 0)
                    {
                    ...
                    var start = _historyRequestFactory.GetStartTimeAlgoTz(
                        ...
                        res,
                        exchange,
                        configs.First().DataTimeZone,
                        extendedMarketHours
                    );
                    var end = Time;

                    return configs.Select(config =>
                        _historyRequestFactory.CreateHistoryRequest(
                            config,
                            start,
                            end,
                            exchange,
        ...
        private IEnumerable<SubscriptionDataConfig> GetMatchingSubscriptions(
            Symbol symbol,
            Type type,
            Resolution? resolution = null
        )
        {
            ...
            var matchingSubscriptions = subscriptions.Where(s =>
                SubscriptionDataConfigTypeFilter(type, s.Type)
            );
        ...
        private bool SubscriptionDataConfigTypeFilter(Type targetType, Type configType)
        {
            ...
            return targetType.IsAssignableFrom(configType)
                && (!targetIsGenericType || configType != typeof(OpenInterest));
        }
        ...
        private Resolution GetResolution(Symbol symbol, Resolution? resolution, Type type)
        {
            ...
            foreach (
                var config in SubscriptionManager
        ...
        /// </summary>
        private void CheckPeriodBasedHistoryRequestResolution(
            IEnumerable<Symbol> symbols,
        ...
        )
        {
            if (
                symbols.Any(symbol =>
                    GetResolution(symbol, resolution, requestedType) == Resolution.Tick
                )
            )
            {
                throw new InvalidOperationException(
        ...
        /// </remarks>
        private static IEnumerable<dynamic> GetPythonCustomDataTypeHistory(
            IEnumerable<Slice> slices,
            List<HistoryRequest> requests,
            Symbol symbol = null
        )
        {

...
namespace QuantConnect.Algorithm
{
    ...
    public partial class QCAlgorithm : MarshalByRefObject, IAlgorithm
    {
        ...
        {
            return History(Securities.Keys, span);
        }

        /// <summary>
        /// Gets the history for the specified symbols over the requested span. This function
        ...
        /// <param name="span">The span over which to request data. This is a calendar span, so take into consideration weekends and such</param>
        /// <returns>An enumerable of slice containing data over the most recent span for the specified configured securities</returns>
        public IEnumerable<Slice> History(IEnumerable<Symbol> symbols, TimeSpan span)
        {
            var start = UtcTime - span;
            var end = UtcTime;
            var requests = symbols.Select(x => new HistoryRequest(Securities[x], start, end));
        ...
        /// </summary>
        /// <param name="symbol">The symbol to request historical data for</param>
        /// <param name="barCount">The number of trade bars to receive at the resolution.</param>
        ...
        public IEnumerable<Slice> History(Symbol symbol, int barCount, Resolution resolution)
        {
            ...
            return History(symbol, barCount, resolution, config.FillDataForward, config.ExtendedMarketHours);
        }
        ...
        /// <returns>An enumerable of slice containing the specified number of bars at the specified resolution for the configured security</returns>
        public IEnumerable<Slice> History(Symbol symbol, int barCount, Resolution resolution, bool fillForward, bool extendedMarket = false)
        {
            if (resolution == Resolution.Tick) throw new ArgumentException("History functions that accept a bar count require that the resolution does not equal Resolution.Tick");
            var security = Securities[symbol];
            var start = QuantConnect.Time.GetStartTimeForTradeBars(security.Exchange.Hours, Time, resolution.ToTimeSpan(), barCount, extendedMarket);
        ...
        public IEnumerable<Slice> History(Symbol symbol, SecurityType securityType, int barCount, Resolution resolution, bool fillForward, bool extendedMarket = false)
        {
            if (resolution == Resolution.Tick) throw new ArgumentException("History functions that accept a bar count require that the resolution does not equal Resolution.Tick");
            Security security;
            ...
            var start = QuantConnect.Time.GetStartTimeForTradeBars(exchangeHours, Time, resolution.ToTimeSpan(), barCount, extendedMarket);
        ...
        /// </summary>
        /// <param name="symbols">The symbols to request historical data for</param>
        /// <param name="barCount">The number of trade bars to receive at the resolution for each symbol.</param>
        /// <param name="resolution">The requested data resolution. This must not equal <see cref="Resolution.Tick"/></param>
        ...
        public IEnumerable<Slice> History(IEnumerable<Symbol> symbols, int barCount, Resolution resolution)
        {
            if (resolution == Resolution.Tick) throw new ArgumentException("History functions that accept a bar count require that the resolution does not equal Resolution.Tick");
            return History(symbols, barCount, resolution, true);
        }
        ...
        /// <param name="extendedMarket">True to receive pre and post market data, false for only normal market hours data</param>
        /// <returns>An enumerable of slice containing the specified number of bars at the specified resolution for the configured securities</returns>
        public IEnumerable<Slice> History(IEnumerable<Symbol> symbols, int barCount, Resolution resolution, bool fillForward, bool extendedMarket = false)
        {
            if (resolution == Resolution.Tick) throw new ArgumentException("History functions that accept a bar count require that the resolution does not equal Resolution.Tick");
            var requests = symbols.Select(symbol =>
            {
                ...
                var start = QuantConnect.Time.GetStartTimeForTradeBars(security.Exchange.Hours, security.LocalTime, resolution.ToTimeSpan(), barCount, extendedMarket).ConvertToUtc(securityTimeZone);

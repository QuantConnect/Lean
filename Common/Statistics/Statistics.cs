...
namespace QuantConnect.Statistics
{
    ...
    public class Statistics
    {
        ...
        public static decimal CompoundingAnnualPerformance(
        ...
        public static double AnnualStandardDeviation(
        ...
        /// <remarks>Invokes the variance extension in the MathNet Statistics class</remarks>
        /// <returns>Annual variance value</returns>
        public static double AnnualDownsideVariance(
            List<double> performance,
            double tradingDaysPerYear,
        ...
        )
        {
            return AnnualVariance(
                performance.Where(ret => ret < minimumAcceptableReturn).ToList(),
                tradingDaysPerYear
            );
        }
        ...
        public static double AnnualDownsideStandardDeviation(
            List<double> performance,
            double tradingDaysPerYear,
            double minimumAcceptableReturn = 0
        )
        {
            return Math.Sqrt(
                AnnualDownsideVariance(performance, tradingDaysPerYear, minimumAcceptableReturn)
            );
        }
        ...
        )
        {
            return standardDeviation == 0
                ? 0
                : (averagePerformance - riskFreeRate) / standardDeviation;
        }
        ...
        {
            return SharpeRatio(
                AnnualPerformance(algoPerformance, tradingDaysPerYear),
        ...
        )
        {
            ...
            var estimateStandardDeviation = Math.Pow(
                (1 - operandA + operandB) / (listPerformance.Count - 1),
                0.5
            );
            ...
            var value = estimateStandardDeviation.IsNaNOrZero()
                ? 0
                : (observedSharpeRatio - benchmarkSharpeRatio) / estimateStandardDeviation;

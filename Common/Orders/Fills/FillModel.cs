...
namespace QuantConnect.Orders.Fills
{
    ...
    public class FillModel : IFillModel
    {
        ...
        public virtual Fill Fill(FillModelParameters parameters)
        {
            ...
            switch (parameters.Order.Type)
            {
                ...
                case OrderType.Limit:
                    orderEvents.Add(
                        PythonWrapper != null
                            ? PythonWrapper.LimitFill(
                                parameters.Security,
                                parameters.Order as LimitOrder
                            )
                ...
                case OrderType.LimitIfTouched:
                    orderEvents.Add(
                        PythonWrapper != null
                            ? PythonWrapper.LimitIfTouchedFill(
                                parameters.Security,
                                parameters.Order as LimitIfTouchedOrder
                            )
                            : LimitIfTouchedFill(
                                parameters.Security,
                                parameters.Order as LimitIfTouchedOrder
                ...
                case OrderType.TrailingStop:
                    orderEvents.Add(
                        PythonWrapper != null
                            ? PythonWrapper.TrailingStopFill(
                                parameters.Security,
                                parameters.Order as TrailingStopOrder
                            )
                            : TrailingStopFill(
                                parameters.Security,
                                parameters.Order as TrailingStopOrder
                ...
                case OrderType.MarketOnOpen:
                    orderEvents.Add(
                        PythonWrapper != null
                            ? PythonWrapper.MarketOnOpenFill(
                                parameters.Security,
                ...
                case OrderType.MarketOnClose:
                    orderEvents.Add(
                        PythonWrapper != null
                            ? PythonWrapper.MarketOnCloseFill(
                                parameters.Security,
        ...
        public virtual List<OrderEvent> ComboLimitFill(Order order, FillModelParameters parameters)
        {
            ...
            var fillParameters = new List<ComboLimitOrderLegParameters>(
                parameters.SecuritiesForOrders.Count
            );
            foreach (var kvp in parameters.SecuritiesForOrders.OrderBy(x => x.Key.Id))
            {
                var targetOrder = kvp.Key;
            ...
            var currentPrice = fillParameters.Aggregate(
                0m,
                (accumulatedPrice, p) => accumulatedPrice + p.Price
            );
                ...
                case OrderDirection.Buy:
                    ...
                    if (currentPrice < limitPrice)
                    {
                        for (var i = 0; i < fillParameters.Count; i++)
                        {
                            ...
                            var utcTime = targetParameters.Security.LocalTime.ConvertToUtc(
                                targetParameters.Security.Exchange.TimeZone
                            );
                            var fill = new OrderEvent(
                                targetParameters.Order,
                                utcTime,
                                OrderFee.Zero
                            );
        ...
        /// <param name="parameters">Fill parameters for the order</param>
        /// <returns>Order fill information detailing the average price and quantity filled for each leg. If any of the fills fails, none of the orders will be filled and the returned list will be empty</returns>
        public virtual List<OrderEvent> ComboLegLimitFill(
            Order order,
        ...
        )
        {
            ...
            foreach (var kvp in parameters.SecuritiesForOrders.OrderBy(x => x.Key.Id))
            {
                ...
                var fill = InternalLimitFill(
                    security,
                    targetOrder,
                    (targetOrder as ComboLegLimitOrder).LimitPrice,
        ...
        public virtual OrderEvent TrailingStopFill(Security asset, TrailingStopOrder order)
        {
            ...
            if (
                fill.Status != OrderStatus.Filled
                && TrailingStopOrder.TryUpdateStopPrice(
        ...
        /// Default limit order fill model in the base security class.
        /// </summary>
        private OrderEvent InternalLimitFill(
            Security asset,
            Order order,
            decimal limitPrice,
            decimal quantity
        ...
        public virtual OrderEvent MarketOnOpenFill(Security asset, MarketOnOpenOrder order)
        {
            if (asset.Exchange.Hours.IsMarketAlwaysOpen)
            {
                throw new InvalidOperationException(
                    Messages.FillModel.MarketNeverCloses(asset, OrderType.MarketOnOpen)
                );
            }
            ...
            if (
                asset.Exchange.DateTimeIsOpen(localOrderTime)
                && localOrderTime.Date == asset.LocalTime.Date
            )
            {
                return fill;
            }
        ...
        public virtual OrderEvent MarketOnCloseFill(Security asset, MarketOnCloseOrder order)
        {
            ...
            {
                throw new InvalidOperationException(
                    Messages.FillModel.MarketNeverCloses(asset, OrderType.MarketOnClose)
                );
            }
        ...
        private decimal GetAskPrice(Security asset, out DateTime endTime)
        {
            ...
            if (isTickSubscribed)
            {
                ...
                var quote = ticks.LastOrDefault(x =>
                    x.TickType == TickType.Quote && x.AskPrice > 0
                );
                if (quote != null)
                {
            ...
            throw new InvalidOperationException(
                Messages.FillModel.NoMarketDataToGetAskPriceForFilling(asset)
            );
        }
        ...
        private decimal GetBidPrice(Security asset, out DateTime endTime)
        {
            ...
            if (isTickSubscribed)
            {
                ...
                var quote = ticks.LastOrDefault(x =>
                    x.TickType == TickType.Quote && x.BidPrice > 0
                );
                if (quote != null)
                {
                    endTime = quote.EndTime;
            ...
            throw new InvalidOperationException(
                Messages.FillModel.NoMarketDataToGetBidPriceForFilling(asset)
            );
        }
        ...
        protected virtual HashSet<Type> GetSubscribedTypes(Security asset)
        {
            ...
            {
                throw new InvalidOperationException(
                    Messages.FillModel.NoDataSubscriptionFoundForFilling(asset)
                );
            }
        ...
        private bool IsExchangeOpen(Security asset)
        {
            // even though data from internal configurations are not sent to the algorithm.OnData they still drive security cache and data
            // this is specially relevant for the continuous contract underlying mapped contracts which are internal configurations
            var configs = Parameters.ConfigProvider.GetSubscriptionDataConfigs(
                asset.Symbol,
        ...
        /// </summary>
        protected virtual Prices GetPricesCheckingPythonWrapper(
            Security asset,
        ...
        protected virtual bool IsExchangeOpen(Security asset, bool isExtendedMarketHours)
        {
            if (!asset.Exchange.Hours.IsOpen(asset.LocalTime, isExtendedMarketHours))
            {
                ...
                var resolution = (
                    currentBar.EndTime - currentBar.Time
                ).ToHigherResolutionEquivalent(false);
                var isOnCurrentBar =
                ...
                return isOnCurrentBar
                    && asset.Exchange.IsOpenDuringBar(
                        currentBar.Time,
                        currentBar.EndTime,
        ...
        private class ComboLimitOrderLegParameters
        {
            ...
            public decimal Price
            {
                get
                {
                    ...
                    var price =
                        Order.GroupOrderManager.Direction == OrderDirection.Buy
                            ? Prices.Low
                            : Prices.High;

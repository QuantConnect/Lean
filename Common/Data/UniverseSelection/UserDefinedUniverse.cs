...
namespace QuantConnect.Data.UniverseSelection
{
    ...
    public class UserDefinedUniverse : Universe, INotifyCollectionChanged, ITimeTriggeredUniverse
    {
        ...
        private readonly HashSet<SubscriptionDataConfig> _subscriptionDataConfigs =
        ...
        private readonly HashSet<SubscriptionDataConfig> _pendingRemovedConfigs =
            new HashSet<SubscriptionDataConfig>();
            ...
            : base(configuration)
        {
            ...
            _selector = time =>
            {
                lock (_lock)
                {
                    return _subscriptionDataConfigs
                        .Select(x => x.Symbol)
                        .Union(_symbols)
                        .ToHashSet();
                }
            ...
            : base(configuration)
        {
            ...
            _selector = time =>
            {
                var selectSymbolsResult = selector(
                    time.ConvertFromUtc(Configuration.ExchangeTimeZone)
                );
                ...
                if (ReferenceEquals(selectSymbolsResult, Unchanged))
                    return Unchanged;
                return selectSymbolsResult.Select(sym =>
                    Symbol.Create(sym, Configuration.SecurityType, Configuration.Market)
                );
            };
        }
        ...
        )
        {
            ...
            var first = true;
            foreach (
                var dateTime in LinqExtensions.Range(
                    localStartTime,
                    localEndTime,
                    dt => dt + Interval
                )
        ...
        )
        {
            ...
            lock (_lock)
            {
                ...
                if (!result.Any())
                {
                    ...
                    {
                        result = base.GetSubscriptionRequests(
                                security,
                                currentTimeUtc,
                                maximumEndTimeUtc,
                                subscriptionService

...
namespace QuantConnect.Securities.Future
{
    ...
    public class FutureMarginModel : SecurityMarginModel
    {
        ...
        private static Dictionary<string, MarginRequirementsEntry[]> _marginRequirementsCache =
            new();
        ...
        /// </summary>
        public virtual decimal InitialOvernightMarginRequirement =>
            GetCurrentMarginRequirements(_security)?.InitialOvernight ?? 0m;

        /// <summary>
        /// Maintenance Overnight margin requirement for the contract effective from the date of change
        /// </summary>
        public virtual decimal MaintenanceOvernightMarginRequirement =>
            GetCurrentMarginRequirements(_security)?.MaintenanceOvernight ?? 0m;

        /// <summary>
        /// Initial Intraday margin for the contract effective from the date of change
        /// </summary>
        public virtual decimal InitialIntradayMarginRequirement =>
            GetCurrentMarginRequirements(_security)?.InitialIntraday ?? 0m;

        /// <summary>
        /// Maintenance Intraday margin requirement for the contract effective from the date of change
        /// </summary>
        public virtual decimal MaintenanceIntradayMarginRequirement =>
            GetCurrentMarginRequirements(_security)?.MaintenanceIntraday ?? 0m;
        ...
        )
        {
            ...
            var fees = parameters
                ...
                .Value;
            var feesInAccountCurrency = parameters
                .CurrencyConverter.ConvertToAccountCurrency(fees)
                .Amount;

            var orderMargin = this.GetInitialMarginRequirement(
                parameters.Security,
        ...
        )
        {
            ...
            )
            {
                return new InitialMargin(
                    marginReq.InitialIntraday * quantity * security.QuoteCurrency.ConversionRate
                );
            }

            // margin is per contract
            return new InitialMargin(
                marginReq.InitialOvernight * quantity * security.QuoteCurrency.ConversionRate
            );
        }
        ...
        private MarginRequirementsEntry GetCurrentMarginRequirements(Security security)
        {
            ...
            while (
                _marginCurrentIndex + 1 < marginRequirementsHistory.Length
        ...
        {
            if (
                !_marginRequirementsCache.TryGetValue(
                    symbol.ID.Symbol,
                    out var marginRequirementsEntries
                )
            )
            {
                lock (_locker)
                {
                    if (
                        !_marginRequirementsCache.TryGetValue(
                            symbol.ID.Symbol,
                            out marginRequirementsEntries
                        )
                    )
                    {
                        Dictionary<string, MarginRequirementsEntry[]> marginRequirementsCache =
        ...
        private static MarginRequirementsEntry[] FromCsvFile(Symbol symbol)
        {
            ...
            var marginRequirementsEntries = _dataProvider
                .ReadLines(file)
            ...
            if (marginRequirementsEntries.Length == 0)
            {
                ...
                {
                    new MarginRequirementsEntry { Date = DateTime.MinValue }
                };
            }
            return marginRequirementsEntries;
        }

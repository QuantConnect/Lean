...
namespace QuantConnect.Securities
{
    ...
    public class SecurityPortfolioManager
        : ExtendedDictionary<SecurityHolding>,
            IDictionary<Symbol, SecurityHolding>,
            ISecurityProvider
    {
            ...
            CashBook.Updated += (sender, args) =>
            {
                if (args.UpdateType == CashBookUpdateType.Added)
                {
                    ...
                    cash.CurrencyConversionUpdated += (sender, args) =>
                    {
                        ...
                        UnsettledCashBook[((Cash)sender).Symbol].CurrencyConversion =
        ...
        public void Add(Symbol symbol, SecurityHolding holding)
        {
            throw new NotImplementedException(
                Messages.SecurityPortfolioManager.DictionaryAddNotImplemented
            );
        }
        ...
        /// <exception cref="NotImplementedException">Portfolio object is an adaptor for Security Manager. This method is not applicable for PortfolioManager class.</exception>
        /// <remarks>This method is not implemented and using it will throw an exception</remarks>
        public void Add(KeyValuePair<Symbol, SecurityHolding> pair)
        {
            throw new NotImplementedException(
                Messages.SecurityPortfolioManager.DictionaryAddNotImplemented
            );
        ...
        {
            throw new NotImplementedException(
                Messages.SecurityPortfolioManager.DictionaryClearNotImplemented
            );
        }
        ...
        {
            throw new NotImplementedException(
                Messages.SecurityPortfolioManager.DictionaryRemoveNotImplemented
            );
        }
        ...
        public void CopyTo(KeyValuePair<Symbol, SecurityHolding>[] array, int index)
        {
            ...
            foreach (var asset in Securities.Values)
            {
                if (i >= index)
                {
                    array[i] = new KeyValuePair<Symbol, SecurityHolding>(
                        asset.Symbol,
                        asset.Holdings
                    );
                }
        ...
        /// </returns>
        protected override IEnumerable<SecurityHolding> GetValues =>
        ...
        IEnumerator<KeyValuePair<Symbol, SecurityHolding>> IEnumerable<
            KeyValuePair<Symbol, SecurityHolding>
        >.GetEnumerator()
        {
            return Securities
                .Select(x => new KeyValuePair<Symbol, SecurityHolding>(x.Key, x.Value.Holdings))
                .GetEnumerator();
        }
        ...
        public decimal TotalUnleveredAbsoluteHoldingsCost
        {
            get
            {
                return Securities.Values.Sum(security =>
                    security.Holdings.UnleveredAbsoluteHoldingsCost
                );
            }
        }
        ...
        {
            get { return Securities.Values.Sum(security => security.Holdings.UnrealizedProfit); }
        }
            ...
            get
            {
                lock (_totalPortfolioValueLock)
                {
                    if (!_isTotalPortfolioValueValid)
                    {
                        ...
                        foreach (
                            var security in Securities.Values.Where((x) => x.Holdings.Invested)
                        )
                        {
                            ...
                            {
                                totalHoldingsValueWithoutForexCryptoFutureCfd += position
                                    .Holdings
                                    .HoldingsValue;
                            }
                            ...
                            {
                                totalFuturesAndCfdHoldingsValue += position
                                    .Holdings
                                    .UnrealizedProfit;
                            }
                        ...
                        _totalPortfolioValue =
                            CashBook.TotalValueInAccountCurrency
        ...
        public decimal TotalPortfolioValueLessFreeBuffer
        {
            get
            {
                ...
                else
                {
                    ...
                    _freePortfolioValue =
                        TotalPortfolioValue * _algorithmSettings.FreePortfolioValuePercentage;
                }
        ...
        {
            get { return Securities.Total.Sum(security => security.Holdings.TotalFees); }
        }
        ...
        {
            get { return Securities.Total.Sum(security => security.Holdings.Profit); }
        }
        ...
        {
            get { return Securities.Total.Sum(security => security.Holdings.NetProfit); }
        }
        ...
        {
            get { return Securities.Total.Sum(security => security.Holdings.TotalSaleVolume); }
        }
        ...
        public decimal TotalMarginUsed
        {
            get
            {
                ...
                {
                    sum += group.BuyingPowerModel.GetReservedBuyingPowerForPositionGroup(
                        this,
                        group
                    );
                }
                ...
                if (accountCurrency != CashBook.AccountCurrency)
                {
                    Log.Trace(
                        "SecurityPortfolioManager.SetAccountCurrency(): "
                            + Messages.SecurityPortfolioManager.AccountCurrencyAlreadySet(
                                CashBook,
                                accountCurrency
                            )
                    );
        ...
        public decimal GetMarginRemaining(
            Symbol symbol,
        ...
        )
        {
            ...
            {
                positionGroupOrderDirection =
                    direction == OrderDirection.Buy ? OrderDirection.Sell : OrderDirection.Buy;
            }

            var parameters = new PositionGroupBuyingPowerParameters(
                this,
                positionGroup,
                positionGroupOrderDirection
            );
        ...
        {
            Log.Trace(
                Messages.SecurityPortfolioManager.TotalMarginInformation(
                    TotalMarginUsed,
                    MarginRemaining
                )
            );
            ...
            {
                var direction =
                    orderSubmitRequest.Quantity > 0 ? OrderDirection.Buy : OrderDirection.Sell;
                var security = Securities[orderSubmitRequest.Symbol];

                var positionGroup = Positions.GetOrCreateDefaultGroup(security);
                var marginUsed =
                    positionGroup.BuyingPowerModel.GetReservedBuyingPowerForPositionGroup(
                        this,
                        positionGroup
                    );
                ...
                Log.Trace(
                    Messages.SecurityPortfolioManager.OrderRequestMarginInformation(
                        marginUsed,
                        marginRemaining.Value
                    )
                );
            }
        }
        ...
        )
        {
            ...
            {
                return group.BuyingPowerModel.HasSufficientBuyingPowerForOrder(
                    new HasSufficientPositionGroupBuyingPowerForOrderParameters(this, group, orders)
                );
            }

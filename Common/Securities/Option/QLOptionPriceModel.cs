    ...
    public class QLOptionPriceModel : IOptionPriceModel
    {
        ...
        public OptionPriceModelResult Evaluate(Security security, Slice slice, OptionContract contract)
        {
            ...
            try
            {
                ...
                // Update the Black Vol Term Structure with the Implied Volatility
                ...
                var delta = black.delta(spot);
                var gamma = black.gamma(spot);
                var lambda = black.elasticity(spot);

                var blackVolatilityMaturity = underlyingVol.link.dayCounter()
                    .yearFraction(underlyingVol.link.referenceDate(), maturityDate);
                var vega = black.vega(blackVolatilityMaturity) / 100;                   // per cent
                var theta = black.thetaPerDay(spot, blackVolatilityMaturity);           // daily value decay

                var riskFreeRateMaturity = riskFreeRate.link.dayCounter()
                    .yearFraction(riskFreeRate.link.referenceDate(), maturityDate);
                var rho = black.rho(riskFreeRateMaturity) / 100;                        // per cent
                ...
                decimal tryGetGreekOrReevaluate(Func<double> greek, Func<double> reevalFunc)
                {
                    try
                    {
                ...
                Tuple<decimal, decimal> evalDeltaGamma()
                {
                    try
                    {
                        return Tuple.Create(option.delta().SafeDecimalCast(), option.gamma().SafeDecimalCast());
                    }
                    catch (Exception)
                    {
                        ...
                        return Tuple.Create(delta.SafeDecimalCast(), gamma.SafeDecimalCast());
                    }
                };
                ...
                return new OptionPriceModelResult((decimal)npv,
                            ...
                            () => new Greeks(evalDeltaGamma,
                                            () => tryGetGreekOrReevaluate(() => option.vega(), () => vega),
                                            () => tryGetGreekOrReevaluate(() => option.theta(), () => theta),
                                            () => tryGetGreekOrReevaluate(() => option.rho(), () => rho),
                                            () => tryGetGreekOrReevaluate(() => option.elasticity(), () => lambda)));
            }
            catch (Exception err)
            {

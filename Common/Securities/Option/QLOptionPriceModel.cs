    ...
    public class QLOptionPriceModel : IOptionPriceModel
    {
        ...
        public OptionPriceModelResult Evaluate(Security security, Slice slice, OptionContract contract)
        {
            ...
            try
            {
                ...
                decimal tryGetGreekOrReevaluate(Func<double> greek, Func<double> black)
                {
                    try
                    {
                ...
                return new OptionPriceModelResult((decimal)npv,
                            ...
                            () => new Greeks(() => tryGetGreekOrReevaluate(() => option.delta(), () => black.delta(spot)),
                                            () => tryGetGreekOrReevaluate(() => option.gamma(), () => black.gamma(spot)),
                                            () => tryGetGreekOrReevaluate(() => option.vega(), () => black.vega(blackVolatilityMaturity) / 100),
                                            () => tryGetGreekOrReevaluate(() => option.theta(), () => black.thetaPerDay(spot, blackVolatilityMaturity)),
                                            () => tryGetGreekOrReevaluate(() => option.rho(), () => black.rho(riskFreeRateMaturity) / 100),
                                            () => tryGetGreekOrReevaluate(() => option.elasticity(), () => black.elasticity(spot))));
            }
            catch (Exception err)
            {

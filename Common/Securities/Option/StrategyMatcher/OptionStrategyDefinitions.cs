...
namespace QuantConnect.Securities.Option.StrategyMatcher
{
    ...
    public static class OptionStrategyDefinitions
    {
        ...
        private static readonly Lazy<ImmutableList<OptionStrategyDefinition>> All = new Lazy<
            ImmutableList<OptionStrategyDefinition>
        >(
            () =>
            ...
            get
            {
                ...
                return strategies
                    .SelectMany(optionStrategy =>
                    {
                        ...
                        var inverted = new OptionStrategyDefinition(
                            optionStrategy.Name,
                            optionStrategy.UnderlyingLots * -1,
                            optionStrategy.Legs.Select(leg => new OptionStrategyLegDefinition(
                                leg.Right,
                                leg.Quantity * -1,
                                leg
                            ))
                        ...
                        if (
                            strategies.Any(strategy =>
                                strategy.UnderlyingLots == inverted.UnderlyingLots
                                && strategy.Legs.Count == inverted.Legs.Count
                                && strategy.Legs.All(leg =>
                                    inverted.Legs.Any(invertedLeg =>
                                        invertedLeg.Right == leg.Right
                                        && leg.Quantity == invertedLeg.Quantity
                                        && leg.All(predicate =>
                                            invertedLeg.Any(invertedPredicate =>
                                                invertedPredicate.ToString() == predicate.ToString()
                                            )
                                        )
                                    )
                                )
        ...
        public static OptionStrategyDefinition CoveredCall { get; } =
            OptionStrategyDefinition.Create(
                "Covered Call",
                1,
        ...
        public static OptionStrategyDefinition CoveredPut { get; } =
            OptionStrategyDefinition.Create("Covered Put", -1, OptionStrategyDefinition.PutLeg(-1));
        ...
        public static OptionStrategyDefinition ProtectiveCollar { get; } =
            OptionStrategyDefinition.Create(
                ...
                1,
                OptionStrategyDefinition.CallLeg(-1),
                OptionStrategyDefinition.PutLeg(
                    1,
        ...
        public static OptionStrategyDefinition NakedCall { get; } =
            OptionStrategyDefinition.Create("Naked Call", OptionStrategyDefinition.CallLeg(-1));

        /// <summary>
        /// Sell 1 put contract without holding the underlying
        /// </summary>
        public static OptionStrategyDefinition NakedPut { get; } =
            OptionStrategyDefinition.Create("Naked Put", OptionStrategyDefinition.PutLeg(-1));
        ...
        public static OptionStrategyDefinition BearCallSpread { get; } =
            OptionStrategyDefinition.Create(
                "Bear Call Spread",
                OptionStrategyDefinition.CallLeg(-1),
                OptionStrategyDefinition.CallLeg(
                    +1,
                    (legs, p) => p.Strike > legs[0].Strike,
                    (legs, p) => p.Expiration == legs[0].Expiration
                )
        ...
        public static OptionStrategyDefinition IronButterfly { get; } =
            OptionStrategyDefinition.Create(
                ...
                OptionStrategyDefinition.CallLeg(
                    -1,
                    (legs, c) => c.Strike == legs[0].Strike,
                    (legs, c) => c.Expiration == legs[0].Expiration
                ),
                OptionStrategyDefinition.CallLeg(
                    +1,
                    (legs, c) => c.Strike == legs[0].Strike * 2 - legs[1].Strike,
                    (legs, c) => c.Expiration == legs[0].Expiration
                )
            );

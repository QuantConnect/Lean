    ...
    public class OptionStrategyPositionGroupBuyingPowerModel : PositionGroupBuyingPowerModel
    {
        ...
        )
        {
            if (_optionStrategy == null)
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.ProtectivePut.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ProtectiveCall.Name
            )
            {
                // Minimum (((10% * Call/Put Strike Price) + Call/Put Out of the Money Amount), Short Stock/Long Maintenance Requirement)
                var optionPosition = parameters.PositionGroup.Positions.FirstOrDefault(position =>
                    position.Symbol.SecurityType.IsOption()
                );
                var underlyingPosition = parameters.PositionGroup.Positions.FirstOrDefault(
                    position => !position.Symbol.SecurityType.IsOption()
                );
                ...
                var outOfTheMoneyAmount =
                    optionSecurity.OutOfTheMoneyAmount(underlyingSecurity.Price)
                    * optionSecurity.ContractUnitOfTrade
                    * absOptionQuantity;

                var underlyingMarginRequired = Math.Abs(
                    underlyingSecurity.BuyingPowerModel.GetMaintenanceMargin(
                ...
                var result = Math.Min(
                    0.1m
                        ...
                        * absOptionQuantity
                        + outOfTheMoneyAmount,
                    underlyingMarginRequired
                );
                var inAccountCurrency = parameters.Portfolio.CashBook.ConvertToAccountCurrency(
                    result,
                    optionSecurity.QuoteCurrency.Symbol
                );
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.CoveredCall.Name)
            {
                ...
                var inTheMoneyAmount =
                    intrinsicValue
                    * optionSecurity.ContractUnitOfTrade
                    * Math.Abs(optionPosition.Quantity);

                var underlyingValue = underlyingSecurity
                    .Holdings.GetQuantityValue(underlyingPosition.Quantity)
                    .InAccountCurrency;
                var optionValue = optionSecurity
                    .Holdings.GetQuantityValue(optionPosition.Quantity)
                    .InAccountCurrency;

                // mark price, strike price
                var underlyingPriceToEvaluate = Math.Min(
                    underlyingSecurity.Price,
                    optionSecurity.ScaledStrikePrice
                );
                var underlyingHypotheticalValue = underlyingSecurity
                    .Holdings.GetQuantityValue(
                        underlyingPosition.Quantity,
                        underlyingPriceToEvaluate
                    )
                    .InAccountCurrency;

                var hypotheticalMarginRequired =
                    underlyingSecurity.BuyingPowerModel.GetMaintenanceMargin(
                        new MaintenanceMarginParameters(
                            underlyingSecurity,
                            underlyingPosition.Quantity,
                            0,
                            underlyingHypotheticalValue
                        )
                    );
                ...
                var secondOperand = Math.Min(
                    underlyingValue,
                    Math.Max(optionValue, marginRequired)
                );
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.CoveredPut.Name)
            {
                ...
                var initialMarginRequirement =
                    underlyingSecurity.BuyingPowerModel.GetInitialMarginRequirement(
                        underlyingSecurity,
                        underlyingPosition.Quantity
                    );
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.ProtectiveCollar.Name)
            {
                ...
                var putPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Symbol.SecurityType.IsOption()
                    && position.Symbol.ID.OptionRight == OptionRight.Put
                );
                var callPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Symbol.SecurityType.IsOption()
                    && position.Symbol.ID.OptionRight == OptionRight.Call
                );
                var underlyingPosition = parameters.PositionGroup.Positions.FirstOrDefault(
                    position => !position.Symbol.SecurityType.IsOption()
                );
                ...
                var putMarginRequirement =
                    0.1m * putSecurity.StrikePrice
                    + putSecurity.OutOfTheMoneyAmount(underlyingSecurity.Price);
                var callMarginRequirement = 0.25m * callSecurity.StrikePrice;

                // call and put has the exact same number of contracts
                var contractUnits =
                    Math.Abs(putPosition.Quantity) * putSecurity.ContractUnitOfTrade;
                var result = Math.Min(putMarginRequirement, callMarginRequirement) * contractUnits;
                var inAccountCurrency = parameters.Portfolio.CashBook.ConvertToAccountCurrency(
                    result,
                    underlyingSecurity.QuoteCurrency.Symbol
                );
            ...
            {
                return GetConversionMaintenanceMargin(
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.ReverseConversion.Name)
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.NakedCall.Name
            ...
            )
            {
                ...
                var margin = security.BuyingPowerModel.GetMaintenanceMargin(
                    MaintenanceMarginParameters.ForQuantityAtCurrentPrice(security, option.Quantity)
                );

                return new MaintenanceMargin(margin);
            }
            else if (
                _optionStrategy.Name == OptionStrategyDefinitions.BearCallSpread.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.BullCallSpread.Name
            )
            {
                var result = GetLongCallShortCallStrikeDifferenceMargin(
                    parameters.PositionGroup.Positions,
                    parameters.Portfolio,
                    parameters.PositionGroup.Quantity
                );
            ...
            )
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.ShortCallCalendarSpread.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ShortPutCalendarSpread.Name
            )
            {
                var shortCall = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity < 0
                );
                var shortCallSecurity = (Option)parameters.Portfolio.Securities[shortCall.Symbol];
                var result = shortCallSecurity.BuyingPowerModel.GetMaintenanceMargin(
                    MaintenanceMarginParameters.ForQuantityAtCurrentPrice(
                        shortCallSecurity,
                        shortCall.Quantity
                    )
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.BearPutSpread.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.BullPutSpread.Name
            )
            {
                var result = GetShortPutLongPutStrikeDifferenceMargin(
                    parameters.PositionGroup.Positions,
                    parameters.Portfolio,
                    parameters.PositionGroup.Quantity
                );
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.Straddle.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.Strangle.Name
            )
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.ShortStraddle.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ShortStrangle.Name
            )
            {
                var result = GetShortStraddleStrangleMargin(
                    parameters.PositionGroup,
                    parameters.Portfolio,
                    (option, quantity) =>
                        Math.Abs(
                            option.BuyingPowerModel.GetMaintenanceMargin(
                                MaintenanceMarginParameters.ForQuantityAtCurrentPrice(
                                    option,
                                    quantity
                                )
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.ButterflyCall.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ButterflyPut.Name
            )
            {
                return new MaintenanceMargin(0);
            }
            else if (
                _optionStrategy.Name == OptionStrategyDefinitions.ShortButterflyPut.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ShortButterflyCall.Name
            )
            {
                var result = GetMiddleAndLowStrikeDifference(
                    parameters.PositionGroup,
                    parameters.Portfolio
                );
                return new MaintenanceMargin(result);
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.IronCondor.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.IronButterfly.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ShortIronCondor.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.ShortIronButterfly.Name
            )
            {
                var result = GetShortPutLongPutStrikeDifferenceMargin(
                    parameters.PositionGroup.Positions,
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.ShortBoxSpread.Name)
            {
                ...
                var longCallPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity > 0 && position.Symbol.ID.OptionRight == OptionRight.Call
                );
                var shortCallPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity < 0 && position.Symbol.ID.OptionRight == OptionRight.Call
                );
                var longPutPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity > 0 && position.Symbol.ID.OptionRight == OptionRight.Put
                );
                var shortPutPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity < 0 && position.Symbol.ID.OptionRight == OptionRight.Put
                );
                var longCallSecurity = (Option)
                    parameters.Portfolio.Securities[longCallPosition.Symbol];
                var shortCallSecurity = (Option)
                    parameters.Portfolio.Securities[shortCallPosition.Symbol];
                var longPutSecurity = (Option)
                    parameters.Portfolio.Securities[longPutPosition.Symbol];
                var shortPutSecurity = (Option)
                    parameters.Portfolio.Securities[shortPutPosition.Symbol];
                ...
                var orderCosts =
                    shortCallSecurity.AskPrice
                    - longCallSecurity.BidPrice
                    + shortPutSecurity.AskPrice
                    - longPutSecurity.BidPrice;
                var multiplier =
                    Math.Abs(longCallPosition.Quantity) * longCallSecurity.ContractUnitOfTrade;
                var closeCost = commissionFees + orderCosts * multiplier;

                var strikeDifference =
                    longCallPosition.Symbol.ID.StrikePrice
                    - shortCallPosition.Symbol.ID.StrikePrice;

                var result = Math.Max(1.02m * closeCost, strikeDifference * multiplier);
                var inAccountCurrency = parameters.Portfolio.CashBook.ConvertToAccountCurrency(
                    result,
                    longCallSecurity.QuoteCurrency.Symbol
                );

                return new MaintenanceMargin(inAccountCurrency);
            }
            else if (
                _optionStrategy.Name == OptionStrategyDefinitions.JellyRoll.Name
            ...
            )
            {
                ...
                var furtherExpiry = parameters.PositionGroup.Positions.Max(position =>
                    position.Symbol.ID.Date
                );
                var shortCalendarSpreadShortLeg = parameters.PositionGroup.Positions.Single(
                    position => position.Quantity < 0 && position.Symbol.ID.Date == furtherExpiry
                );
                var shortCalendarSpreadShortLegSecurity = (Option)
                    parameters.Portfolio.Securities[shortCalendarSpreadShortLeg.Symbol];
                var result = Math.Abs(
                    shortCalendarSpreadShortLegSecurity.BuyingPowerModel.GetMaintenanceMargin(
                        MaintenanceMarginParameters.ForQuantityAtCurrentPrice(
                            shortCalendarSpreadShortLegSecurity,
                            shortCalendarSpreadShortLeg.Quantity
                        )
                    )
                );
        ...
        )
        {
            ...
            )
            {
                ...
                result = parameters.Portfolio.CashBook.ConvertToAccountCurrency(
                    result,
                    underlyingSecurity.QuoteCurrency.Symbol
                );
            }
            ...
            else if (_optionStrategy.Name == OptionStrategyDefinitions.CoveredPut.Name)
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.ProtectiveCollar.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.Conversion.Name
            )
            {
                result = GetCollarConversionInitialMargin(
                    parameters.PositionGroup,
                    parameters.Portfolio,
                    OptionRight.Call
                );
            ...
            )
            {
                ...
                if (optionMargin != null)
                {
                    return new OptionInitialMargin(
                        Math.Abs(optionMargin.ValueWithoutPremium),
                        optionMargin.Premium
                    );
                }
            ...
            {
                result = GetLongCallShortCallStrikeDifferenceMargin(
                    parameters.PositionGroup.Positions,
                    parameters.Portfolio,
                    parameters.PositionGroup.Quantity
                );
            ...
            {
                var shortOptionPosition = parameters.PositionGroup.Positions.Single(position =>
                    position.Quantity < 0
                );
                var shortOption = (Option)
                    parameters.Portfolio.Securities[shortOptionPosition.Symbol];
                result = Math.Abs(
                    shortOption.BuyingPowerModel.GetInitialMarginRequirement(
                        shortOption,
                        shortOptionPosition.Quantity
                    )
                );
            }
            else if (
            ...
            {
                result = GetShortPutLongPutStrikeDifferenceMargin(
                    parameters.PositionGroup.Positions,
                    parameters.Portfolio,
                    parameters.PositionGroup.Quantity
                );
            ...
            {
                result = GetShortStraddleStrangleMargin(
                    parameters.PositionGroup,
                    parameters.Portfolio,
                    (option, quantity) =>
                        Math.Abs(
                            option.BuyingPowerModel.GetInitialMarginRequirement(option, quantity)
                        )
                );
            }
            else if (
            ...
            {
                result = GetMiddleAndLowStrikeDifference(
                    parameters.PositionGroup,
                    parameters.Portfolio
                );
            }
            ...
            )
            {
                ...
                _optionStrategy.Name == OptionStrategyDefinitions.BearCallLadder.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.BearPutLadder.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.BullCallLadder.Name
                || _optionStrategy.Name == OptionStrategyDefinitions.BullPutLadder.Name
            )
            {
                result = GetMaintenanceMargin(
                    new PositionGroupMaintenanceMarginParameters(
            ...
            var premium = 0m;
            foreach (
                var position in parameters.PositionGroup.Positions.Where(position =>
                    position.Symbol.SecurityType.IsOption()
                )
            )
            {
        ...
        )
        {
            ...
            var fees = security.FeeModel.GetOrderFee(
                new OrderFeeParameters(security, parameters.Order)
            );
            var feesInAccountCurrency = parameters.Portfolio.CashBook.ConvertToAccountCurrency(
                fees.Value
            );

            var initialMarginRequired = GetInitialMarginRequirement(
        ...
        /// </summary>
        protected override decimal GetContemplatedGroupsInitialMargin(
        ...
        )
        {
            ...
            var ordersGroups = portfolio.Positions.ResolvePositionGroups(
            ...
            foreach (var orderGroup in ordersGroups)
            {
                ...
                if (optionInitialMargin != null)
                {
                    ...
                    contemplatedMargin +=
                        optionInitialMargin.Value - optionInitialMargin.ValueWithoutPremium;
                }
            }
        ...
        /// </summary>
        private static decimal GetShortPutLongPutStrikeDifferenceMargin(
            IEnumerable<IPosition> positions,
            SecurityPortfolioManager portfolio,
            decimal quantity
        )
        {
            var longOption = positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Put && position.Quantity > 0
            );
            var shortOption = positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Put && position.Quantity < 0
            );
            ...
            var strikeDifference =
                shortOption.Symbol.ID.StrikePrice - longOption.Symbol.ID.StrikePrice;

            var result = Math.Max(
                strikeDifference * optionSecurity.ContractUnitOfTrade * Math.Abs(quantity),
                0
            );

            // convert into account currency
            return portfolio.CashBook.ConvertToAccountCurrency(
                result,
                optionSecurity.QuoteCurrency.Symbol
            );
        }
        ...
        /// </summary>
        private static decimal GetLongCallShortCallStrikeDifferenceMargin(
            IEnumerable<IPosition> positions,
            SecurityPortfolioManager portfolio,
            decimal quantity
        )
        {
            var longOption = positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Call && position.Quantity > 0
            );
            var shortOption = positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Call && position.Quantity < 0
            );
            ...
            var strikeDifference =
                longOption.Symbol.ID.StrikePrice - shortOption.Symbol.ID.StrikePrice;
        ...
        /// </summary>
        private static decimal GetMiddleAndLowStrikeDifference(
            IPositionGroup positionGroup,
            SecurityPortfolioManager portfolio
        )
        {
            var options = positionGroup
                .Positions.OrderBy(position => position.Symbol.ID.StrikePrice)
                .ToList();
            ...
            var strikeDifference = Math.Max(
                (middleCallStrike - lowestCallStrike)
                    * optionSecurity.ContractUnitOfTrade
                    * Math.Abs(positionGroup.Quantity),
                0
            );
        ...
        /// </summary>
        private static decimal GetShortStraddleStrangleMargin(
            IPositionGroup positionGroup,
            SecurityPortfolioManager portfolio,
            Func<Option, decimal, decimal> getOptionMargin
        )
        {
            var callOption = positionGroup.Positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Call
            );
            ...
            var putOption = positionGroup.Positions.Single(position =>
                position.Symbol.ID.OptionRight == OptionRight.Put
            );
            ...
            if (putMargin > callMargin)
            {
                result =
                    putMargin
                    + callSecurity.Price
                        * callSecurity.ContractUnitOfTrade
                        * Math.Abs(callOption.Quantity);
            }
            else
            {
                result =
                    callMargin
                    + putSecurity.Price
                        * putSecurity.ContractUnitOfTrade
                        * Math.Abs(putOption.Quantity);
            }
        ...
        /// </summary>
        private static decimal GetConversionMaintenanceMargin(
            IPositionGroup positionGroup,
            SecurityPortfolioManager portfolio,
        ...
        )
        {
            ...
            var optionPosition = positionGroup.Positions.Single(position =>
                position.Symbol.SecurityType.IsOption()
                && position.Symbol.ID.OptionRight == optionRight
            );
            var underlyingPosition = positionGroup.Positions.FirstOrDefault(position =>
                !position.Symbol.SecurityType.IsOption()
            );
            ...
            var marginRequirement =
                0.1m * optionSecurity.StrikePrice
                + optionSecurity.GetIntrinsicValue(underlyingSecurity.Price);
            var result =
                marginRequirement
                * Math.Abs(optionPosition.Quantity)
                * optionSecurity.ContractUnitOfTrade;
            var inAccountCurrency = portfolio.CashBook.ConvertToAccountCurrency(
                result,
                underlyingSecurity.QuoteCurrency.Symbol
            );
        ...
        /// </summary>
        private static decimal GetCollarConversionInitialMargin(
            IPositionGroup positionGroup,
            SecurityPortfolioManager portfolio,
            OptionRight optionRight
        )
        {
            ...
            var underlyingPosition = positionGroup.Positions.Single(position =>
                !position.Symbol.SecurityType.IsOption()
            );
            var optionSecurity = (Option)portfolio.Securities[optionPosition.Symbol];
            var underlyingSecurity = portfolio.Securities[underlyingPosition.Symbol];
        ...
        /// </summary>
        private static decimal GetCallLadderMargin(
            PositionGroupMaintenanceMarginParameters parameters,
            bool bearCallLadder
        )
        {
            ...
            if ((quantity >= 0 && bearCallLadder) || (quantity < 0 && !bearCallLadder))
            {
                ...
                var callSpread = parameters
                    .PositionGroup.Positions.OrderBy(position => position.Symbol.ID.StrikePrice)
                    .Take(2)
                    .ToList();
                return GetLongCallShortCallStrikeDifferenceMargin(
                    callSpread,
                    parameters.Portfolio,
                    Math.Abs(quantity)
                );
            ...
            else
            {
                ...
                var shortNakedCall = parameters
                    .PositionGroup.Positions.OrderByDescending(position =>
                        position.Symbol.ID.StrikePrice
                    )
                    .First();
        ...
        /// </summary>
        private static decimal GetPutLadderMargin(
            PositionGroupMaintenanceMarginParameters parameters,
            bool bullPutLadder
        )
        {
            ...
            if ((quantity >= 0 && bullPutLadder) || (quantity < 0 && !bullPutLadder))
            {
                ...
                var putSpread = parameters
                    .PositionGroup.Positions.OrderByDescending(position =>
                        position.Symbol.ID.StrikePrice
                    )
                    .Take(2)
                    .ToList();
                return GetShortPutLongPutStrikeDifferenceMargin(
                    putSpread,
                    parameters.Portfolio,
                    Math.Abs(quantity)
                );

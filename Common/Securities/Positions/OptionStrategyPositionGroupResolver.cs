...
namespace QuantConnect.Securities.Positions
{
    ...
    public class OptionStrategyPositionGroupResolver : IPositionGroupResolver
    {
        ...
        public OptionStrategyPositionGroupResolver(SecurityManager securities)
            : this(
                securities,
                OptionStrategyMatcherOptions.ForDefinitions(
                    OptionStrategyDefinitions.AllDefinitions
                )
            ) { }
        ...
        public OptionStrategyPositionGroupResolver(
        ...
        )
        {
            ...
            if (currentPositions.Count > 0)
            {
                ...
                var positionsToConsiderInNewGroup = impactedGroups.SelectMany(positionGroup =>
            ...
            @group = GetPositionGroups(positions)
                .Select(positionGroup =>
                {
                    ...
                    if (
                        newPositions.Any(position =>
                            positionGroup.TryGetPosition(position.Symbol, out position)
                        )
                    )
                    {
                        return positionGroup;
        ...
        )
        {
            ...
            var symbolsSet = positions
                .Where(position =>
                    position.Symbol.SecurityType.HasOptions()
                    || position.Symbol.SecurityType.IsOption()
                )
                .SelectMany(position =>
                {
                    return position.Symbol.HasUnderlying
            ...
            return groups.Where(group =>
                group.Quantity != 0
                && group.Positions.Any(position1 =>
                    symbolsSet.Contains(position1.Symbol)
                    || position1.Symbol.HasUnderlying
                        && position1.Symbol.SecurityType.IsOption()
                        && symbolsSet.Contains(position1.Symbol.Underlying)
                )
            );
        }
        ...
        {
            foreach (
                var positionsByUnderlying in positions
                    .Where(position =>
                        position.Symbol.SecurityType.HasOptions()
                        || position.Symbol.SecurityType.IsOption()
                    )
                    .GroupBy(position =>
                        position.Symbol.HasUnderlying ? position.Symbol.Underlying : position.Symbol
                    )
                    .Select(x => x.ToList())
            )
            {
                var optionPosition = positionsByUnderlying.FirstOrDefault(position =>
                    position.Symbol.SecurityType.IsOption()
                );
                ...
                var contractMultiplier =
                    (
                        _securities[optionPosition.Symbol].SymbolProperties
                        as OptionSymbolProperties
                    )?.ContractUnitOfTrade ?? 100;

                var optionPositionCollection = OptionPositionCollection.FromPositions(
                ...
                if (optionPositionCollection.Count == 0 && positionsByUnderlying.Count > 0)
                {
                    ...
                    yield return PositionGroup.Empty(
                        new OptionStrategyPositionGroupBuyingPowerModel(null)
                    );
                    yield break;
                }
                ...
                {
                    var groupQuantity = Math.Abs(
                        matchedStrategy
                            .OptionLegs.Cast<Leg>()
                            .Concat(matchedStrategy.UnderlyingLegs)
                            .Select(leg => leg.Quantity)
                            .GreatestCommonDivisor()
                    );
                    var positionsToGroup = matchedStrategy
                        .OptionLegs.Select(optionLeg =>
                            (IPosition)
                                new Position(
                                    optionLeg.Symbol,
                                    optionLeg.Quantity,
                        ...
                        .Concat(
                            matchedStrategy.UnderlyingLegs.Select(underlyingLeg => new Position(
                                underlyingLeg.Symbol,
                                ...
                                (
                                    Math.Abs(underlyingLeg.Quantity)
                                    * contractMultiplier
                                    / groupQuantity
                                )
                            ))
                    ...
                    yield return new PositionGroup(
                        new PositionGroupKey(
                            new OptionStrategyPositionGroupBuyingPowerModel(matchedStrategy),
                            positionsToGroup.Values
                        ),
                        groupQuantity,
                        positionsToGroup
                    );

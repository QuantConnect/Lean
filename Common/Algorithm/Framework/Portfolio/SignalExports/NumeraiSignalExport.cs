...
namespace QuantConnect.Algorithm.Framework.Portfolio.SignalExports
{
    ...
    public class NumeraiSignalExport : BaseSignalExport
    {
        ...
        /// </summary>
        private readonly Dictionary<string, string> _numeraiMarketFormat =
            new() // There can be stocks from other markets
            {
                { Market.USA, "US" },
                { Market.SGX, "SP" }
            };
        ...
        protected bool ConvertTargetsToNumerai(
        ...
        private bool SendPositions(string positions)
        {
            ...
            var arguments = new { filename = _fileName, modelId = _modelId };
            var argumentsMessage = JsonConvert.SerializeObject(arguments);

            using var variables = new StringContent(
                argumentsMessage,
                Encoding.UTF8,
                "application/json"
            );
            ...
            {
                _algorithm.Error(
                    $"Numerai API returned the following errors: {string.Join(",", parsedResponseContent["errors"])}"
                );
                return false;
            }

            var putUrl = new Uri(
                (string)parsedResponseContent["data"]["submissionUploadSignalsAuth"]["url"]
            );
            var submissionFileName = (string)
                parsedResponseContent["data"]["submissionUploadSignalsAuth"]["filename"];
            ...
            var createArguments = new { filename = submissionFileName, modelId = _modelId };
            var createArgumentsMessage = JsonConvert.SerializeObject(createArguments);

            using var submissionQuery = new StringContent(
                createQuery,
                Encoding.UTF8,
                "application/json"
            );
            using var submissionVariables = new StringContent(
                createArgumentsMessage,
                Encoding.UTF8,
                "application/json"
            );
            ...
            {
                _algorithm.Error(
                    $"Numerai API returned the following errors: {string.Join(",", parsedSubmissionResponseContent["errors"])}"
                );
                return false;
            }

...
namespace QuantConnect.Queues
{
    ...
    public class JobQueue : IJobQueueHandler
    {
        ...
        protected Language Language
        {
            get
            {
                if (_language == null)
                {
                    ...
                    {
                        _language = (Language)
                            Enum.Parse(typeof(Language), algorithmLanguage, ignoreCase: true);
                    }
                }
        ...
        private string AlgorithmLocation { get; } =
            Config.Get("algorithm-location", "QuantConnect.Algorithm.CSharp.dll");
        ...
        public static IBrokerageFactory GetFactoryFromDataQueueHandler(string dataQueueHandler)
        {
            ...
            var dataQueueHandlerType = Composer
                .Instance.GetExportedTypes<IBrokerage>()
                .FirstOrDefault(x =>
                    x.FullName != null
                    && x.FullName.EndsWith(
                        dataQueueHandler,
        ...
        public AlgorithmNodePacket NextJob(out string algorithmPath)
        {
            ...
            var controls = new Controls()
            {
                ...
                MaximumDataPointsPerChartSeries = Config.GetInt(
            ...
            if (Globals.LiveMode)
            {
                ...
                var liveJob = new LiveNodePacket
                {
                    ...
                    DataChannelProvider = Config.Get(
                        "data-channel-provider",
                        DefaultDataChannelProvider
                    ),
                    Channel = Channel,
                ...
                try
                {
                    ...
                    var brokerageFactory = Composer.Instance.Single<IBrokerageFactory>(factory =>
                        factory.BrokerageType.MatchesTypeName(liveJob.Brokerage)
                    );
                ...
                var brokerageBasedHistoryProvider = liveJob
                    .HistoryProvider.DeserializeList()
                    .Select(x =>
                    {
                ...
                foreach (
                    var dataHandlerName in dataHandlers
                        .DeserializeList()
                        .Concat(brokerageBasedHistoryProvider)
                        .Distinct()
                )
                {
                    var brokerageFactoryForDataHandler = GetFactoryFromDataQueueHandler(
                        dataHandlerName
                    );
            ...
            var backtestJob = new BacktestNodePacket(
                ...
                "",
                new byte[] { },
                Config.Get("backtest-name", "local")
            )
            {

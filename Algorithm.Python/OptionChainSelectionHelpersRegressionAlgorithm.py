...
class OptionChainSelectionHelpersRegressionAlgorithm(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # to the underlying price (at the money is the default when no strike criteria is given)
        ...
        # 2015-12-24: GOOG at 748.54, closest expiry to 10 days out is 2015-12-31 (7 days), ATM strike is 747.50
        if contract.expiry != datetime(2015, 12, 31) or contract.strike != 747.5 or contract.days_to_expiry != 7:
            raise AssertionError(f"Unexpected contract selected: {contract.symbol.value}, {contract.days_to_expiry} days to expiry")

        # pick() is a synonym of select()
        if contract.symbol != chain.pick(right=OptionRight.CALL, target_dte=10).symbol:
            raise AssertionError("pick() and select() must select the same contract")
        ...
        # The sorted expiries start at the chain date, contracts expiring today are still in the chain
        ...
        otm_call = chain.select(right=OptionRight.CALL, target_dte=7, strike_from_atm=5)
        if otm_call is None or otm_call.strike != 752.5:
            raise AssertionError(f"select(strike_from_atm) expected the 752.50 call but got {otm_call}")

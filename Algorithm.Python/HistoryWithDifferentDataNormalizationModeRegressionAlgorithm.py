...
class HistoryWithDifferentDataMappingModeRegressionAlgorithm(QCAlgorithm):
    def Initialize(self):
        ...
        self.esFutureSymbol = self.AddFuture(Futures.Indices.SP500EMini, Resolution.Daily, extendedMarketHours=True).Symbol
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.CheckHistoryResultsForDataNormalizationModes(self.esFutureSymbol, self.StartDate, self.EndDate, Resolution.Daily,
            futureDataNormalizationModes, True)

    def CheckHistoryResultsForDataNormalizationModes(self, symbol, start, end, resolution, dataNormalizationModes, extendedMarket=False):
        historyResults = [self.History([symbol], start, end, resolution, dataNormalizationMode=x, extendedMarket=extendedMarket) for x in dataNormalizationModes]

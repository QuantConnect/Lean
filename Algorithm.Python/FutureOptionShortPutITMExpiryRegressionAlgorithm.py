...
class FutureOptionShortPutITMExpiryRegressionAlgorithm(QCAlgorithm):
    def Initialize(self):
        ...
        self.esOption = self.AddFutureOptionContract(
            list(
                sorted(
                    [x for x in self.OptionChainProvider.GetOptionContractList(self.es19h21, self.Time) if x.ID.StrikePrice >= 3300.0 and x.ID.OptionRight == OptionRight.Put],
                    key=lambda x: x.ID.StrikePrice
                )
            )[0], Resolution.Minute).Symbol

        self.expectedContract = Symbol.CreateOption(self.es19h21, Market.CME, OptionStyle.American, OptionRight.Put, 3300.0, datetime(2021, 3, 19))
    ...
    def AssertFutureOptionOrderExercise(self, orderEvent: OrderEvent, future: Security, optionContract: Security):
        if "Assignment" in orderEvent.Message:
            if orderEvent.FillPrice != 3300.0:
                raise AssertionError("Option was not assigned at expected strike price (3300)")

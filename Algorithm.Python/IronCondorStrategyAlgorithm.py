...
class IronCondorStrategyAlgorithm(OptionStrategyFactoryMethodsBaseAlgorithm):
    ...
    def assert_strategy_position_group(self, position_group: IPositionGroup, option_symbol: Symbol):
        ...
        if long_put_position is None or long_put_position.quantity != 2:
            raise AssertionError(f"Expected long put position quantity to be 2. Actual: {getattr(long_put_position, 'quantity', None)}")
        ...
        if short_put_position is None or short_put_position.quantity != -2:
            raise AssertionError(f"Expected short put position quantity to be -2. Actual: {getattr(short_put_position, 'quantity', None)}")
        ...
        if short_call_position is None or short_call_position.quantity != -2:
            raise AssertionError(f"Expected short call position quantity to be -2. Actual: {getattr(short_call_position, 'quantity', None)}")
        ...
        if long_call_position is None or long_call_position.quantity != 2:
            raise AssertionError(f"Expected long call position quantity to be 2. Actual: {getattr(long_call_position, 'quantity', None)}")

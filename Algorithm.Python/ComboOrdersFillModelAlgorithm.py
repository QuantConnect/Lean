...
class ComboOrdersFillModelAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # Set the fill model
        self.spy.SetFillModel(CustomPartialFillModel(self))
        self.ibm.SetFillModel(CustomPartialFillModel(self))
...
class CustomPartialFillModel(FillModel):
    ...
    def FillOrderPartially(self, asset, order, quantity, fill):
    ...
    def ComboMarketFill(self, order, parameters):
        ...
        for kvp, fill in zip(sorted(parameters.SecuritiesForOrders, key=lambda x: x.Key.Id), fills):
            targetOrder = kvp.Key;
            security = kvp.Value;
            partialFills.append(self.FillOrderPartially(security, targetOrder, 50, fill))
    ...
    def ComboLimitFill(self, order, parameters):
        ...
        for kvp, fill in zip(sorted(parameters.SecuritiesForOrders, key=lambda x: x.Key.Id), fills):
            targetOrder = kvp.Key;
            security = kvp.Value;
            partialFills.append(self.FillOrderPartially(security, targetOrder, 20, fill))
    ...
    def ComboLegLimitFill(self, order, parameters):
        ...
        for kvp, fill in zip(sorted(parameters.SecuritiesForOrders, key=lambda x: x.Key.Id), fills):
            targetOrder = kvp.Key;
            security = kvp.Value;
            partialFills.append(self.FillOrderPartially(security, targetOrder, 10, fill))

    ...
    public class BacktestingBrokerage : Brokerage
    {
        ...
        public virtual void Scan()
        {
            ...
            lock (_needsScanLock)
            {
                ...
                // created on demand: Scan runs at least twice per time loop and most algorithms never place a group
                ...
                foreach (var kvp in _pending.OrderBySafe(x => x.Key))
                {
                    ...
                    if (order.GroupOrderManager != null && order.GroupOrderManager.ComboType != ComboType.Combo)
                    {
                        ...
                        if (!(processedGroupIds ??= new HashSet<int>()).Add(order.GroupOrderManager.Id))
                        {
                            continue;
                        }

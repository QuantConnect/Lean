...
{
    /// <summary>
    /// TDAmeritrade Class: IDataQueueUniverseProvider implementation
    /// </summary>
    public partial class TDAmeritradeBrokerage : IDataQueueUniverseProvider
    {
        /// <summary>
        ...
        public IEnumerable<Symbol> LookupSymbols(Symbol symbol, bool includeExpired, string securityCurrency = null)
        {
            ...
            Log.Trace($"TDAmeritradeBrokerage.LookupSymbols(): Requesting symbol list for {lookupName} ...");
            ...
            if (!includeExpired)
            {
                ...
                {
                    Log.Trace(
                        $"TDAmeritradeBrokerage.LookupSymbols(): Removed contract(s) for having expiry in the past: {string.Join(",", removedSymbols.Select(x => x.Value))}");
                }
            }

            Log.Trace($"TDAmeritradeBrokerage.LookupSymbols(): Returning {symbols.Count} contract(s) for {lookupName}");

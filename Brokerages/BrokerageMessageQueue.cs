...
{
    /// <summary>
    /// Owns the lock and buffer that <see cref="BrokerageConcurrentMessageHandler{T}"/> used to keep to itself.
    /// A message of any type can be enqueued here; <see cref="MessageReceived"/> raises it once it is this
    /// message's turn, and each subscriber picks out the type it cares about. This is what lets more than one
    /// source share a single lock: a brokerage's own stream messages and, separately, the order polling
    /// service's snapshots, so a stream message and a polled order update can never run at the same time, and
    /// neither can run while an order request holds <see cref="WithLockedStream"/>.
    /// </summary>
    public class BrokerageMessageQueue : IDisposable
    {
        private readonly Queue<object> _messageBuffer;
        private readonly ILock _lock;
        private readonly ManualResetEventSlim _messagesProcessedEvent;
        private readonly int _maxMessageBufferSize;

        /// <summary>
        /// Raised for every dequeued message, in arrival order, one at a time. A brokerage's stream handler
        /// and the order polling service each subscribe and filter for their own message type, e.g.
        /// <c>MessageReceived += message => { if (message is OrderResponse r) OnPolledOrder(r); };</c>
        /// </summary>
        ...
        public BrokerageMessageQueue(bool concurrencyEnabled = false)
        {
            _messageBuffer = new Queue<object>();
        ...
        private void ProcessMessages(object message = null)
        {
            try
            {
                if (message != null)

...
namespace QuantConnect.ToolBox.QuiverDataDownloader
{
    ...
    public class QuiverPoliticalBetaDataDownloader : QuiverDataDownloader
    {
        ...
        public QuiverPoliticalBetaDataDownloader(string destinationFolder)
        {
            _destinationFolder = Path.Combine(destinationFolder, "politicalbeta");
        ...
        public override bool Run()
        {
            ...
            try
            {
                ...
                Log.Trace($"QuiverPoliticalBetaDataDownloader.Run(): Start processing {count.ToStringInvariant()} companies");
                ...
                foreach (var company in companies)
                {
                    ...
                    {
                        Log.Error($"QuiverPoliticalBetaDataDownloader(): Defunct ticker {quiverTicker} is unable to be parsed. Continuing...");
                        continue;
                    }

                    // Begin processing ticker with a normalized value
                    Log.Trace($"QuiverPoliticalBetaDataDownloader.Run(): Processing {ticker}");
                    ...
                    tasks.Add(
                        HttpRequester($"historical/politicalbeta/{ticker}")
                            .ContinueWith(
                                y =>
                                {
                                    i++;
                                    ...
                                    if (y.IsFaulted)
                                    {
                                        Log.Error($"QuiverPoliticalBetaDataDownloader.Run(): Failed to get data for {company}");
                                        return;
                                    }
                                    ...
                                    var followers = JsonConvert.DeserializeObject<List<QuiverPoliticalBeta>>(result, JsonSerializerSettings);
                                    ...
                                    {
                                        Log.Trace($"QuiverPoliticalBetaDataDownloader.Run(): {percentageDone.ToStringInvariant("P2")} complete");
            ...
            Log.Trace($"QuiverPoliticalBetaDataDownloader.Run(): Finished in {stopwatch.Elapsed.ToStringInvariant(null)}");
            return true;
        }
    }
}
    ...
    public class QuiverSenateDataDownloader : QuiverDataDownloader
    {
        ...
        public override bool Run()
        {
            ...
            try
            {
                ...
                Log.Trace($"QuiverSenateDataDownloader.Run(): Start processing {count.ToStringInvariant()} companies");
                ...
                foreach (var company in companies)
                {
                    ...
                    {
                        Log.Error($"QuiverSenateDataDownloader(): Defunct ticker {quiverTicker} is unable to be parsed. Continuing...");
                        continue;
                    }

                    // Begin processing ticker with a normalized value
                    Log.Trace($"QuiverSenateDataDownloader.Run(): Processing {ticker}");
                    ...
                    tasks.Add(
                        HttpRequester($"historical/senatetrading/{ticker}")
                            .ContinueWith(
                                y =>
                                {
                                    i++;
                                    ...
                                    if (y.IsFaulted)
                                    {
                                        Log.Error($"QuiverSenateDataDownloader.Run(): Failed to get data for {company}");
                                        return;
                                    }
                                    ...
                                    var followers = JsonConvert.DeserializeObject<List<QuiverSenate>>(result, JsonSerializerSettings);
                                    ...
                                    {
                                        Log.Trace($"QuiverSenateDataDownloader.Run(): {percentageDone.ToStringInvariant("P2")} complete");
            ...
            Log.Trace($"QuiverSenateDataDownloader.Run(): Finished in {stopwatch.Elapsed.ToStringInvariant(null)}");
            return true;
        }
    }
}
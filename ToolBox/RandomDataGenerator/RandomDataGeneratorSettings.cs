...
namespace QuantConnect.ToolBox.RandomDataGenerator
{
    public class RandomDataGeneratorSettings
    {
        ...
        )
        {
            ...
            // --chain-symbol-count
            if (
                !int.TryParse(chainSymbolCountString, out chainSymbolCount)
                || chainSymbolCount <= 0
            )
            {
                chainSymbolCount = 10;
            ...
            else if (!Enum.TryParse(resolutionString, true, out resolution))
            {
                var validValues = string.Join(
                    ", ",
                    Enum.GetValues(typeof(Resolution)).Cast<Resolution>()
                );
                failed = true;
                Log.Error(
            ...
            if (string.IsNullOrEmpty(volatilityModelResolutionString))
            {
                ...
                !Enum.TryParse(volatilityModelResolutionString, true, out volatilityModelResolution)
            )
            {
                var validValues = string.Join(
                    ", ",
            ...
            {
                var validValues = string.Join(", ", Enum.GetValues(typeof(DataDensity)))
                    .Cast<DataDensity>();
                failed = true;
                Log.Error(
            ...
            if (string.IsNullOrEmpty(dividendEveryQuarterPercentageString))
            {
                ...
                !double.TryParse(
                    ...
                    out dividendEveryQuarterPercentage
                )
            )
            {
                failed = true;

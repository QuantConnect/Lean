...
namespace QuantConnect.ToolBox.AlgoSeekFuturesConverter
{
    ...
    public class AlgoSeekFuturesConverter
    {
        ...
        public AlgoSeekFuturesConverter(
        ...
        public void Convert()
        {
            ...
            var files = GetFilesInRawFolder()
                .Where(f =>
                    (f.Extension == ".gz" || f.Extension == ".bz2") && !f.Name.Contains("option")
                )
                .Select(remote =>
                    remote.CopyTo(Path.Combine(Path.GetTempPath(), remote.Name), true)
                )
                .ToList();

            Log.Trace(
                "AlgoSeekFuturesConverter.Convert(): Loading {0} AlgoSeekFuturesReader for {1} ",
                files.Count(),
                _referenceDate
            );
            ...
            Parallel.ForEach(
                ...
                file =>
                {
                    try
                    {
                        ...
                        var reader = new AlgoSeekFuturesReader(
                            csvFile,
                            symbolMultipliers,
                            _symbolFilter
                        );
                            ...
                            do
                            {
                                ...
                                if (!processors.TryGetValue(tick.Symbol, out symbolProcessors))
                                {
                                    symbolProcessors = new List<List<AlgoSeekFuturesProcessor>>(3)
                                    {
                                        {
                                            _resolutions
                                                .Select(x => new AlgoSeekFuturesProcessor(
                                                    tick.Symbol,
                                                    _referenceDate,
                                                    TickType.Trade,
                                                    x,
                                ...
                                if (Interlocked.Increment(ref totalLinesProcessed) % 1000000m == 0)
                                {
                                    var pro = (double)
                                        processors
                                            .Values.SelectMany(p => p.SelectMany(x => x))
                                            .Count();
                                    var symbols = (double)processors.Keys.Count();
                                    Log.Trace(
                                        "AlgoSeekFuturesConverter.Convert(): Processed {0,3}M ticks( {1}k / sec); Memory in use: {2} MB; Total progress: {3}%, Processor per symbol {4}",
                    ...
                    {
                        Log.Error(
                            "Exception caught! File: {0} Err: {1} Source {2} Stack {3}",
                            file,
                            err.Message,
                            err.Source,
                            err.StackTrace
        ...
        public void Package(DateTime date)
        {
            ...
            var files = Directory
                .EnumerateFiles(destination, dateMask + "*.csv", SearchOption.AllDirectories)
            ...
            Parallel.ForEach(
                ...
                //foreach (var file in files)
                {
                    try
                    {
                        ...
                        var filesToCompress = Directory.GetFiles(
                            file.Key,
                            "*.csv",

...
namespace QuantConnect.Report
{
    ...
    public static class Rolling
    {
        ...
        public static Series<DateTime, double> Beta(
            SortedList<DateTime, double> performancePoints,
            SortedList<DateTime, double> benchmarkPoints,
            int windowSize = 132
        )
        {
            var dailyDictionary = StatisticsBuilder.PreprocessPerformanceValues(
                performancePoints.Select(x => new KeyValuePair<DateTime, decimal>(
                    x.Key,
                    (decimal)x.Value
                ))
            );
            ...
            {
                var benchmarkReturnsDictionary = StatisticsBuilder.CreateBenchmarkDifferences(
                    benchmarkPoints.Select(x => new KeyValuePair<DateTime, decimal>(
                        x.Key,
                        (decimal)x.Value
                    )),
                    benchmarkPoints.Keys.First(),
                    benchmarkPoints.Keys.Last()
                );
            ...
            returns = returns
                .Join("benchmark", benchmarkReturns)
            ...
            var correlation = returns
                .Window(windowSize)
                .SelectValues(x =>
                    Correlation.Pearson(x["strategy"].Values, x["benchmark"].Values)
                );

            var portfolioStandardDeviation = dailyReturnsSeries
                .Window(windowSize)
                .SelectValues(s => s.StdDev());
            var benchmarkStandardDeviation = benchmarkReturns
                .Window(windowSize)
                .SelectValues(s => s.StdDev());
        ...
        public static Series<DateTime, double> Sharpe(
        ...
        )
        {
            var riskFreeRate = (double)
                _interestRateProvider.GetAverageRiskFreeRate(equityCurve.Keys);
            if (equityCurve.IsEmpty)
            {
                return equityCurve;
            }

            var dailyReturns = equityCurve
            ...
            foreach (var date in equityCurve.Keys)
            {
                ...
                rollingSharpeData.Add(
                    new KeyValuePair<DateTime, double>(
                        date,
                        Statistics.Statistics.SharpeRatio(
                            algoPerformanceLookback.Values.ToList(),
                            riskFreeRate,
                            tradingDayPerYear
                        )
                    )
            ...
            return new Series<DateTime, double>(
                rollingSharpeData.Select(kvp => kvp.Key),
                rollingSharpeData.Select(kvp => kvp.Value)
            );
        }
    }
}

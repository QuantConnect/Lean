...
namespace QuantConnect.Report.ReportElements
{
    ...
    public class SharpeRatioReportElement : ReportElement
    {
        ...
        /// </summary>
        public virtual decimal? BacktestResultValue =>
        ...
        public SharpeRatioReportElement(
            string name,
            string key,
        ...
        )
        {
            ...
            Name = name;
            Key = key;
            _tradingDaysPerYear = Convert.ToDouble(
                tradingDaysPerYear,
        ...
        public override string Render()
        {
            ...
            var performance = DeedleUtil.PercentChange(
                new Series<DateTime, double>(equityPoints).ResampleEquivalence(
                    date => date.Date,
                    s => s.LastValue()
                )
            );
            ...
            var trailingPerformance = performance
                .Where(series =>
                    series.Key >= sixMonthsAgo
                    && series.Key.DayOfWeek != DayOfWeek.Saturday
                    && series.Key.DayOfWeek != DayOfWeek.Sunday
                )
                .Values.ToList();

            var annualStandardDeviation =
                trailingPerformance.Count < 7
                    ? 0
            ...
            var annualPerformance = Statistics.Statistics.AnnualPerformance(
                trailingPerformance,
                _tradingDaysPerYear
            );
            var liveResultValue = Statistics.Statistics.SharpeRatio(
                annualPerformance,
        ...
        public virtual double GetAnnualStandardDeviation(
            List<double> trailingPerformance,
            double tradingDaysPerYear
        )
        {
            return Statistics.Statistics.AnnualStandardDeviation(
                trailingPerformance,
                tradingDaysPerYear
            );
        }

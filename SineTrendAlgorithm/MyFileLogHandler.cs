...
namespace QuantConnect.Logging
{
    ...
    public class MyFileLogHandler : ILogHandler
    {
        ...
        public MyFileLogHandler(string filepath)
        {
        ...
        {
            if (text.Contains("Debug Message >> ,"))
        ...
        private void WriteMessage(string text, string level)
        {
            lock (_lock)
            {
                ...
                // Add your formatting filter here.
                _writer.Value.WriteLine("{0}", text.Substring(text.IndexOf(",", System.StringComparison.Ordinal) + 1));
                _writer.Value.Flush();
            }
        }
    }

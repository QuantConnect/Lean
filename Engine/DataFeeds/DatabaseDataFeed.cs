    ...
    public class DatabaseDataFeed : IDataFeed
    {
        ...
        public void Run()
        {
            ...
            while (!_exitTriggered && IsActive)
            {
                ...
                var items = new SortedDictionary<DateTime, Dictionary<Security, List<BaseData>>>();
                ...
                for (var i = 0; i < Subscriptions.Count; i++)
                {
                    ...
                    {
                        Dictionary<Security, List<BaseData>> dataDictionary;
                        if (!items.TryGetValue(bar.EndTime, out dataDictionary))
                        {
                            dataDictionary = new Dictionary<Security, List<BaseData>>();
                            items[bar.EndTime] = dataDictionary;
                        }

                        List<BaseData> dataPoints;
                        if (!dataDictionary.TryGetValue(_securities[i], out dataPoints))
                        {
                            dataPoints = new List<BaseData>();
                            dataDictionary[_securities[i]] = dataPoints;
                        }
                ...
                Dictionary<Security, List<BaseData>> timeSlice;

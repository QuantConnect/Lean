...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class FileSystemDataFeed : IDataFeed
    {
        ...
        protected IEnumerator<BaseData> CreateEnumerator(
        ...
        private IEnumerator<BaseData> CreateDataEnumerator(
            SubscriptionRequest request,
        ...
        public virtual Subscription CreateSubscription(SubscriptionRequest request)
        {
            ...
            if (_algorithm.IsWarmingUp)
            {
                ...
                )
                {
                    ...
                    pivotTimeUtc = Time.GetStartTimeForTradeBars(
                            request.Security.Exchange.Hours,
                            _algorithm.StartDate.ConvertTo(
                                _algorithm.TimeZone,
                    ...
                    warmupEnumerator = CreateEnumerator(
                        warmupRequest,
                        _algorithm.Settings.WarmupResolution
                    );
                    // don't let future data past
                    warmupEnumerator = new FilterEnumerator<BaseData>(
                        warmupEnumerator,
                        data => data == null || data.EndTime <= warmupRequest.EndTimeLocal
                    );
                }

                var normalEnumerator = CreateEnumerator(
                    new SubscriptionRequest(request, startTimeUtc: pivotTimeUtc)
                );
                ...
                normalEnumerator = new FilterEnumerator<BaseData>(
                    normalEnumerator,
                    data => data == null || data.EndTime >= warmupRequest.EndTimeLocal
                );
        ...
        protected IEnumerator<BaseData> CreateUniverseEnumerator(
            SubscriptionRequest request,
            Func<
                SubscriptionRequest,
                Resolution?,
                IEnumerator<BaseData>
            > createUnderlyingEnumerator,
            Resolution? fillForwardResolution = null
        )
        {
            ...
            {
                factory = new TimeTriggeredUniverseSubscriptionEnumeratorFactory(
                    request.Universe as ITimeTriggeredUniverse,
            ...
            else if (request.Configuration.Type == typeof(ZipEntryName))
            {
                ...
                var resolution =
                    request.Configuration.Resolution == Resolution.Tick
                        ? Resolution.Second
                ...
                var result = new BaseDataSubscriptionEnumeratorFactory(
                    _algorithm.OptionChainProvider,
                    _algorithm.FutureChainProvider
                ...
                return TryAppendUnderlyingEnumerator(
                    request,
                    result,
                    createUnderlyingEnumerator,
        ...
        protected IEnumerator<BaseData> AddScheduleWrapper(
            SubscriptionRequest request,
        ...
        )
        {
            if (
                ...
                || !request.Universe.UniverseSettings.Schedule.Initialized
            )
            {
                return underlying;
            }

            var schedule = request.Universe.UniverseSettings.Schedule.Get(
                request.StartTimeLocal,
                request.EndTimeLocal
            );
            if (schedule != null)
            {
                return new ScheduledEnumerator(
                    underlying,
                    schedule,
                    timeProvider,
        ...
        /// </summary>
        protected IEnumerator<BaseData> TryAppendUnderlyingEnumerator(
            SubscriptionRequest request,
        ...
        )
        {
            if (
                ...
                && request.Configuration.Symbol.HasUnderlying
            )
            {
                var underlyingSymbol = request.Configuration.Symbol.Underlying;
                var underlyingMarketHours = _marketHoursDatabase.GetEntry(
                ...
                var underlyingRequests = new SubscriptionRequest(
                    request,
                ...
                underlying = new FilterEnumerator<BaseData>(
                    underlying,
                    data => data.DataType != MarketDataType.Auxiliary
                );

                parent = new SynchronizingBaseDataEnumerator(parent, underlying);
                // we aggregate both underlying and chain data
                parent = new BaseDataCollectionAggregatorEnumerator(
                    parent,
                    request.Configuration.Symbol
                );
                // only let through if underlying and chain data present
                parent = new FilterEnumerator<BaseData>(
                    parent,
                    data => (data as BaseDataCollection).Underlying != null
                );
        ...
        )
        {
            ...
            enumerator = TryAddFillForwardEnumerator(
                request,
                enumerator,
        ...
        /// </summary>
        protected IEnumerator<BaseData> TryAddFillForwardEnumerator(
            SubscriptionRequest request,
            IEnumerator<BaseData> enumerator,
            bool fillForward,
            Resolution? fillForwardResolution
        )
        {
            ...
            if (fillForward && request.Configuration.Resolution != Resolution.Tick)
            {
                ...
                var fillForwardSpan = _subscriptions.UpdateAndGetFillForwardResolution(
                    request.Configuration
                );

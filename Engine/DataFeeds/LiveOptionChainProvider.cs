...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class LiveOptionChainProvider : BacktestingOptionChainProvider
    {
        ...
        private const string CMEProductSlateURL =
            "https://www.cmegroup.com/CmeWS/mvc/ProductSlate/V2/List?pageNumber=1&sortAsc=false&sortField=rank&searchString="
            + CMESymbolReplace
            + "&pageSize=5";
        private const string CMEOptionsTradeDateAndExpirations =
            "https://www.cmegroup.com/CmeWS/mvc/Settlements/Options/TradeDateAndExpirations/"
            + CMEProductCodeReplace;
        private const string CMEOptionChainQuotesURL =
            "https://www.cmegroup.com/CmeWS/mvc/Quotes/Option/"
            + CMEProductCodeReplace
            + "/G/"
            + CMEProductExpirationReplace
            + "/ALL?_=";
        ...
        static LiveOptionChainProvider()
        {
            ...
            _client.DefaultRequestHeaders.AcceptLanguage.Add(
                new StringWithQualityHeaderValue("en-US", 0.5)
            );
        }
        ...
        public override IEnumerable<Symbol> GetOptionContractList(Symbol symbol, DateTime date)
        {
            ...
            if (date.Date >= DateTime.UtcNow.Date.AddDays(-5) || result.Count == 0)
            {
                ...
                if (
                    underlyingSymbol.SecurityType == SecurityType.Equity
                    || underlyingSymbol.SecurityType == SecurityType.Index
                )
                {
                    ...
                    foreach (
                        var optionSymbol in GetEquityIndexOptionContractList(
                                underlyingSymbol,
                ...
                else if (underlyingSymbol.SecurityType == SecurityType.Future)
                {
                    ...
                    foreach (
                        var optionSymbol in GetFutureOptionContractList(underlyingSymbol, date)
                            .Where(symbol => !IsContractExpired(symbol, date))
                    )
                    {
                        result.Add(optionSymbol);
        ...
        private IEnumerable<Symbol> GetFutureOptionContractList(
        ...
        )
        {
            ...
            while (++retries <= maxRetries)
            {
                try
                {
                    ...
                    var productResponse = _client
                        .GetAsync(
                            CMEProductSlateURL.Replace(
                                CMESymbolReplace,
                                futureContractSymbol.ID.Symbol
                            )
                        )
                    ...
                    var productResults =
                        JsonConvert.DeserializeObject<CMEProductSlateV2ListResponse>(
                            productResponse
                    ...
                    var futureProductId = productResults
                        .Products.Where(p =>
                            p.Globex == futureContractSymbol.ID.Symbol
                            && p.GlobexTraded
                            && p.Cleared == "Futures"
                        )
                        .Select(p => p.Id)
                    ...
                    var optionsTradesAndExpiries = CMEOptionsTradeDateAndExpirations.Replace(
                        CMEProductCodeReplace,
                        futureProductId.ToStringInvariant()
                    );

                    _cmeRateGate.WaitToProceed();

                    var optionsTradesAndExpiriesResponse = _client
                        .GetAsync(optionsTradesAndExpiries)
                        .SynchronouslyAwaitTaskResult();
                    optionsTradesAndExpiriesResponse.EnsureSuccessStatusCode();

                    var tradesAndExpiriesResponse = JsonConvert.DeserializeObject<
                        List<CMEOptionsTradeDatesAndExpiration>
                    >(
                    ...
                    var selectedOption = tradesAndExpiriesResponse.FirstOrDefault(x =>
                        !x.Daily && !x.Weekly && !x.Sto && x.OptionType == "AME"
                    );
                    ...
                    var canonicalFuture = Symbol.Create(
                        futureContractSymbol.ID.Symbol,
                        SecurityType.Future,
                        futureContractSymbol.ID.Market
                    );
                    var expiryFunction = FuturesExpiryFunctions.FuturesExpiryFunction(
                        canonicalFuture
                    );

                    var futureContractExpiration = selectedOption
                        .Expirations.Select(x => new KeyValuePair<CMEOptionsExpiration, DateTime>(
                            x,
                            expiryFunction(new DateTime(x.Expiration.Year, x.Expiration.Month, 1))
                        ))
                        .FirstOrDefault(x =>
                            x.Value.Year == futureContractSymbol.ID.Date.Year
                    ...
                    var optionChainQuotesResponseResult = _client.GetAsync(
                        CMEOptionChainQuotesURL
                            .Replace(
                                CMEProductCodeReplace,
                                selectedOption.ProductId.ToStringInvariant()
                            )
                            .Replace(CMEProductExpirationReplace, futureContractMonthCode)
                            + Math.Floor((DateTime.UtcNow - _epoch).TotalMilliseconds)
                                .ToStringInvariant()
                    );

                    optionChainQuotesResponseResult.Result.EnsureSuccessStatusCode();

                    var futureOptionChain = JsonConvert
                        .DeserializeObject<CMEOptionChainQuotes>(
                            optionChainQuotesResponseResult
                                .Result.Content.ReadAsStringAsync()
                                .SynchronouslyAwaitTaskResult()
                        )
                        .Quotes.DistinctBy(s => s.StrikePrice)
                        .ToList();
                    ...
                    var optionStrikePriceScaleFactor = CMEStrikePriceScalingFactors.GetScaleFactor(
                    ...
                    foreach (var optionChainEntry in futureOptionChain)
                    {
                        var futureOptionExpiry =
                            FuturesOptionsExpiryFunctions.GetFutureOptionExpiryFromFutureExpiry(
                                futureContractSymbol,
                                canonicalOption
                            );
                        var scaledStrikePrice =
                            optionChainEntry.StrikePrice / optionStrikePriceScaleFactor;
        ...
        private static IEnumerable<Symbol> GetEquityIndexOptionContractList(
            Symbol symbol,
        ...
        /// </summary>
        private static IEnumerable<Symbol> FindOptionContracts(
        ...
        )
        {
            ...
            var url =
                "https://www.quantconnect.com/api/v2/theocc/series-search?symbolType=U&symbol="
